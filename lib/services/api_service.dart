...
class ApiService {
  static const _b = 'http://127.0.0.1:5000';

  static Future<bool> isServerRunning() async {
    try {
      final r = await http.get(Uri.parse('$_b/')).timeout(const Duration(seconds: 3));
  ...
  static Future<File?> downloadFile(String id, String path) async {
    final r = await http.get(Uri.parse('$_b/download/$id')).timeout(const Duration(minutes: 2));
    if (r.statusCode == 200) {
      final f = File(path);
