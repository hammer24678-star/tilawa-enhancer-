...
def build_nr_sfm_v86(damage:'DamageProfile', silence:'Optional[SilenceProfile]'=None,
                     tier:str='TIER_PRISTINE', hum_notch:str='') -> 'List[str]':
    parts=[]
    ...
    if silence and silence.valid and tier in('TIER_DEGRADED','TIER_DAMAGED'):
        ...
        if w:
            ...
            return parts
    sfm_ratio=damage.sfm/(TARGET['sfm']+1e-6)
    if sfm_ratio>=5.0:   parts+=['afftdn=nr=20:nf=-55:tn=1','afftdn=nr=6:nf=-65:tn=1']
    elif sfm_ratio>=3.0: parts.append(f'afftdn=nr={int(np.interp(sfm_ratio,[3,5],[12,20]))}:nf=-58:tn=1')
    elif sfm_ratio>=2.0: parts.append(f'afftdn=nr={int(np.interp(sfm_ratio,[2,3],[6,12]))}:nf=-62:tn=1')
    elif sfm_ratio>=1.5: parts.append('afftdn=nr=4:nf=-68:tn=1')
    if damage.has_ringing and damage.rolloff_hz<17000:
        parts.append(f'lowpass=f={int(damage.rolloff_hz*0.97)}:poles=2')
    return parts
...
def enhance(input_path:str,output_path:str,
    ...
    silence_profile = measure_silence_floor(inp, SR)
    ...
    nr_parts=build_nr_sfm_v86(damage,silence_profile,tier_profile.tier,_hum_notch) if use_nr or has_ringing else (
    ...
    L(f"\n{'═'*70}")
    L(f"  FINAL REPORT — v8.6 ({elapsed:.0f}s)")
    L(f"{'═'*70}")
