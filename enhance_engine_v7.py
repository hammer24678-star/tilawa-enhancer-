...
def get_reference_fingerprint() -> ReferenceFingerprint:
    """
    v6.5: بصمة من 5 مقاطع موزعة — median spectrum أدق وأكثر استقراراً
    """
    import json
    cache_file = REF_CACHE

    if os.path.exists(cache_file):
        try:
            if os.path.getmtime(cache_file) >= os.path.getmtime(REF_PATH):
                with open(cache_file, 'r') as f:
                    d = json.load(f)
                fp = ReferenceFingerprint()
                fp.third_oct     = {float(k): v for k,v in d['third_oct'].items()}
                fp.a_weighted    = {float(k): v for k,v in d['a_weighted'].items()}
                fp.bark          = [(float(a), float(b)) for a,b in d['bark']]
                fp.rms           = d['rms'];  fp.peak  = d['peak']
                fp.crest         = d['crest']; fp.lra  = d['lra']
                fp.tilt_slope    = d['tilt_slope']
                fp.warmth_ratio  = d['warmth_ratio']
                fp.clarity_ratio = d['clarity_ratio']
                return fp
        except Exception:
            pass

    probe   = get_probe(REF_PATH)
    total_s = int(float(probe.get('format',{}).get('duration',300)))

    # v6.5: 5 مقاطع موزعة بالتساوي
    skips = [max(10, int(total_s * r)) for r in [0.10, 0.25, 0.45, 0.65, 0.82]]
    clips = [f'/tmp/ref65_s{i}.wav' for i in range(5)]

    procs = [
        subprocess.Popen(['ffmpeg','-y','-i',REF_PATH,
                          '-ss',str(sk),'-t','40',
    ...
    ref_audio = np.concatenate(segments) if segments else load(REF_PATH, skip=30, duration=120)
    fp = build_reference_fingerprint(ref_audio)
