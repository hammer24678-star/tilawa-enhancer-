...
_ADF_XFADE    = int(0.120 * 48000)  # v4: 120ms crossfade (was 40ms) — much smoother seams
...
def _df3(wav, samples, rt60, st):
    ...
    try:
        ...
        # §22.5 clean gate: restore original on already-clean chunks
        blended[c_s[:min_n]] = mono[:min_n][c_s[:min_n]]

        # ── 120ms sigmoid crossfade at every class boundary (v4: was cosine 40ms) ──
        # Sigmoid gives perceptually flatter transition — no "click" at midpoint
        ...
        for b in bounds:
            ...
            ln_ = e_cf - s_cf
            # Sigmoid: x from -6..+6 → smooth S-curve
