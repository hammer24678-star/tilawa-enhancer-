...
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║                                                                              ║
# ║   engine_isteidad_v21.py — الاسترداد v21                                     ║
# ║   REAL NEURAL BACKENDS: Resemble Enhance · MetricGAN+ · Whisper · ECAPA     ║
# ║                                                                              ║
# ║   Base:  engine_isteidad_v19.py (Parts I + II + III)                        ║
# ║   Date:  June 2026                                                          ║
# ║                                                                              ║
# ║   NEW IN v20 (Part IV):                                                      ║
# ║   GAP-1  Ornament frame per-frame DF gating (MUJAWWAD chunked DF3)          ║
# ║   GAP-2  النداء N1/N3 style gate (MUJAWWAD DNA not pulled to Murattal)       ║
# ║   GAP-3  QARA 7-dimension Tajweed quality scoring (formal, measurable)       ║
# ║   GAP-4  Deployment audit in output JSON (caller visibility)                 ║
# ║   GAP-5  Per-segment multilingual processing (slice+process+stitch)          ║
# ║   GAP-6  GPU generative path interface (GAP-URGENet, wired + waiting)       ║
# ║   GAP-7  Arabic WavLM gate in الحكيم (warns when English backbone in use)    ║
# ║   GAP-8  5-feature MUJAWWAD classifier (vibrato + phrase length added)      ║
# ║                                                                              ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
#
...
#
#
# ── v21 — Real neural backends replacing DriftSE/SIPS/AnyEnhance/FlowSE stubs
#   B-gen.1  DriftSE stub → Resemble Enhance (primary, pip install resemble-enhance)
#            Diffusion-based harmonic reconstruction, λ tuned per tier.
#            Fallback: FRCRN via ClearerVoice (pip install clearvoice-studio).
#   B-gen.2  SIPS stub → DNSMOS P.835 scoring (ONNX, auto-download 2MB)
#            + MetricGAN+ corrector (SpeechBrain, pip install speechbrain).
#            Score before → apply MetricGAN+ only when OVRL < 3.5 → re-score.
#   B-ref    AnyEnhance stub → ECAPA-TDNN speaker similarity gate (SpeechBrain)
#            + Resemble Enhance (λ=0.4 denoiser mode)
#            + reference-matched 1/3-octave EQ correction (5-8kHz blend=0.65).
#   B-text   FlowSE stub → Whisper 'small' transcription (openai-whisper)
#            + per-segment targeted denoising on low-confidence spans (logprob<-1).
#            Surgical: high-confidence segments left untouched.
#   All Tajweed gates, API signatures, and state field names unchanged from v20.
#

#   FIX-A  _dedup_eq_nodes moved to BEFORE the per-band voice-identity clamp.
#          v18 ran dedup AFTER the clamp: two +2.5dB optimizer nodes near the
#          same frequency were each clamped individually (≤3dB ✓), then summed
#          by the merger into +5dB — silently overriding the ±3dB guard.
#          Fix: merge first, then clamp; combined sum is now subject to the
#          band limits instead of circumventing them.
#   FIX-B  BWE voiced_hf_loss gated by 5→10 kHz rolloff slope discriminator.
#          The 42dB gap threshold alone (v18-FIX-3) was insufficient: natural
#          mosque acoustics + mic roll-off can produce >42dB integrated mid→HF
#          gap with a gradual slope (~14dB/oct) even when codec preserved the
#          full bandwidth.  A genuine codec cutoff cliff is ≥24dB/oct at 5-10kHz.
#          Fix: voiced_hf_loss requires gap > threshold AND slope ≥24dB/oct.
#          Applied in both _classify_artifacts AND _bandwidth_extension_pass
#          (defense-in-depth: the inner BWE check now also uses tier-aware
#          threshold + slope gate so it cannot override the outer decision).
#
# ── v18 FIXES — Metallic artifact elimination (noor TIER_DEGRADED source) ────
#   FIX-1  J-1 shimmer DISABLED for TIER_DEGRADED (shimmer_blend 0.020→0.0)
#          Reason: 5-12kHz synthesised harmonics stacked with EQ + BWE produced
#          +34dB metallic plateau at 5kHz on mosque natural-rolloff recordings.
#   FIX-2a EQ max_db 6.0→3.0 for TIER_DEGRADED in optimize_eq / design_eq.
#          Prevents optimizer from placing +6dB nodes at 7kHz & 11kHz trying
#          to flatten the natural -14.9dB/oct mosque HF rolloff to reference.
#   FIX-2b VOICE-IDENTITY-CLAMP: added hard +3dB boost ceiling for EQ nodes
#          above 3kHz when source_tier == TIER_DEGRADED (belt-and-suspenders).
#   FIX-3  voiced_hf_loss threshold 30dB→42dB for TIER_DEGRADED in
#          _detect_codec_artifacts.  Natural mosque rolloff creates a 30-40dB
#          mid/HF gap that was incorrectly triggering aexciter BWE (TYPE_C C3),
#          adding 4-8kHz synthetic content on top of already-over-boosted signal.
#          42dB threshold only fires on true codec cutoff cliffs, not gradual rolls.
# Base: engine_isteidad_v15.py (Part I) + v16_part2 (Part II) + Part III
# Knowledge Bases: voice_audio_knowledge_base_COMPLETE.txt (§1-75, June 2026)
#                  Oracle §76-88 (Supplement v7, June 2026)
# Evolution Plan: الاسترداد — Part I + II + III (v14→v17)
#
# NEW IN v17 — PART III: GENERATIVE INTELLIGENCE UPGRADE
#   "The predictive engine becomes a predictive-generative engine."
#
#   [P3-01] Silero VAD via sherpa-onnx (8× fewer false positives on M-3 murmur)
#           Replaces _energy_vad() throughout; energy VAD retained as fallback.
#   [P3-02] nara_wpe True WPE Dereverberation (Phase B.3):
#           taps=10, delay=3, iterations=3. Arabic guards: LRA ≤0.5LU drop,
#           Idgham flux ≤2dB increase, Madd proxy ≤3% shortening.
#   [P3-03] pyroomacoustics Physics-Based Sadaa (Phase E1):
#           Shoebox RIR 40×25×15m, RT60=2.2s, α=0.017 (marble), 80ms RIR,
#           mix=0.20. Falls back to aecho chain if unavailable.
#   [P3-06] الحكيم Phase B-gen.1: DriftSE generative corrector.
#           TIER_DEGRADED/CRITICAL + MURATTAL. Tajweed gate: emphatic Δ≤1.5dB
#           + Madd shortening ≤5%. MUJAWWAD: DRIFTSE_STYLE_GATE (unvalidated).
#   [P3-09] الحكيم Phase B-gen.2: SIPS plug-and-play score model. All tiers.
#           Expected +0.7–1.0 NISQA. Arabic-WavLM backbone.
#   [P3-10a] المرآة Phase B-ref: AnyEnhance + 1425H speaker reference prompt.
#            4-gate validation: SpeechBERTScore>0.85 + emphatic + Madd + LUFS.
#            TIER_DAMAGED/CRITICAL + MURATTAL + clean reference required.
#   [P3-10b] الإدراك Phase B-text: FlowSE text-conditioned SE.
#            Graceful degradation: text-cond→audio-only→DeepFilter.
#            Trigger: quran_prefix_set + WhisperX alignment available.
#
# NEW IN v16 — PART II: ARABIC DNA PROTECTION LAYER  (MISS-06–11)
#
#   [MISS-06] الرواية (Al-Riwaya) — Phase A.7 — Hafs/Warsh/Qalun AUTO detection
#             Fixes Warsh Munfasil false MADD_SHORT flags (6H vs 4H threshold)
#   [MISS-07] الأسلوب (Al-Uslub) — Phase A.10 — MURATTAL/MUJAWWAD/HADR style
#             Disables J-1/J-4 for MUJAWWAD; atten_lim=6dB for ornament frames
#   [MISS-08] مقام الصون — SIDRAH extension — F0 sweep ornament frame mask
#             DeepFilter atten_lim=6dB gated on ornament_frame_mask
#   [MISS-09] التعدد (Al-Ta'addud) — Phase A.12 — Arabic/Urdu/Malay segment map
#             Non-Arabic commentary skips Tajweed-specific phases
#   [MISS-10] الماد v2 — Formant-velocity Madd measurement upgrade
#             Riwaya-correct thresholds; MUJAWWAD voiced fraction = 50%
#   [MISS-11] الاستمرار (Al-Istimrar) — Phase A.11 — Long-form drift correction
#             Cubic spline amplitude drift + HF spectral tilt drift (post-NR)
#
# NEW IN v15 — PART I: CRITICAL HARDENING  (fixes 5 silent failures)
#
#   [MISS-01] MODULE الوقاية (Al-Wiqaya) — Phase 0 — Stereo-to-Mono Protocol
#             Oracle §79 / KB §41.3 / RED LINE #17
#             Runs BEFORE everything. Measures inter-channel phase correlation.
#             Routes: dual-mono→L only | safe sum | delay-align | anti-phase warn.
#             Prevents 250Hz comb null that destroys ghunnah before analysis runs.
#
#   [MISS-02] MODULE الوقاية — Phase 0 — DC Offset Drift Correction
#             Oracle §83.2 / Plan Part 1
#             Polynomial drift fitting (degree 2–3) per 30s window.
#             Corrects capacitor leakage / ADC thermal drift in long recordings.
#             Must run BEFORE tier classification (DC inflates false clip ratio).
#
#   [MISS-03] FIX-15 — True Peak Ceiling (Phase E)
#             alimiter attack=5:release=50 (plan spec). ebur128 TP verification.
#             Prevents inter-sample peaks from exceeding -1.0 dBTP.
#
#   [MISS-04] MODULE الأذن الحساسة (Al-Udhun Al-Hassasa) — Phase A.8
#             Oracle §78 / KB §79 — Opus/WhatsApp Tier Detection + SILK Repair
#             Hard cutoff at 4kHz→TIER_CRITICAL | 8kHz→TIER_DEGRADED | 12kHz→TIER_COMPRESSED
#             SILK artifact repair: formant bandwidth widening + residual smoothing.
#             Tajweed consequence logging for ص/س distinction unreliability.
#
#   [MISS-05] MODULE الأذن الحساسة — Phase A.9 — Congregation Noise Taxonomy
#             Oracle §81 — 6-type taxonomy: M-1 Ameen | M-2 Takbeer | M-3 Murmur
#             M-4 Physical (rustle/floor/cough/phone) | M-5 PA feedback | M-6 Print-through
#             Builds noise_map for targeted suppression. M-1 gets SKIP_NR gate.
#             Only M-3 is valid afftdn NR target. M-4c cough → R-2 dropout path.
#
#   ARCHITECTURE FIX: working_path=input_path now set BEFORE analyze_input().
#   ARCHITECTURE FIX: enhance() quran_prefix/nr_backend moved from global args to params.
#
# INHERITED FROM v14: KB-01 through KB-14 (all v14 improvements preserved)
...
# ═══════════════════════════════════════════════════════════════════════════════
#  PART III v17 — GENERATIVE LAYER IMPORTS
# ═══════════════════════════════════════════════════════════════════════════════
...
# ── nara_wpe true WPE dereverberation ─────────────────────────────────────────
...
# ── pyroomacoustics physics-based RIR Sadaa ───────────────────────────────────
# pip install pyroomacoustics
try:
    import pyroomacoustics as _pra          # type: ignore
    _PYROOM_OK = True
except ImportError:
    _PYROOM_OK = False

# ── Part III Generative Modules (v2: real neural backends) ────────────────────
try:
    from hakim_gen_v2 import apply_hakim, HakimResult
    _HAKIM_OK = True
except ImportError:
    _HAKIM_OK = False

try:
    from miraat_ref_v2 import apply_miraat, MiraatResult
    _MIRAAT_OK = True
except ImportError:
    _MIRAAT_OK = False

try:
    from idrak_text_v2 import apply_idrak, IdrakResult
    _IDRAK_OK = True
except ImportError:
    _IDRAK_OK = False

# ── Silero VAD singleton (module-level, loaded once) ──────────────────────────
...
# ══════════════════════════════════════════════════════════════════════════════
#  PART II v16 — SECTION A: ARABIC DNA CONSTANTS
# ══════════════════════════════════════════════════════════════════════════════
_RIWAYA_MADD_TABLE = {
    'HAFS':  {'tabii': 2, 'muttasil': (4, 5), 'munfasil': 4, 'lazim': 6},
    'WARSH': {'tabii': 2, 'muttasil': 6,       'munfasil': 6, 'lazim': 6},
    'QALUN': {'tabii': 2, 'muttasil': (4, 5),  'munfasil': 2, 'lazim': 6},
}
_RIWAYA_HAMZAH_RULES = {
    'HAFS':  {'full_burst_expected': True,  'tashil_positions': False,
              'ibdal_positions': False},
    'WARSH': {'full_burst_expected': False, 'tashil_positions': True,
              'ibdal_positions': True,      'depharyngealize_r': True},
    'QALUN': {'full_burst_expected': True,  'tashil_positions': False,
              'ibdal_positions': False},
}
_HARAKAH_MS_DEFAULT          = 200.0
_STYLE_DEEPFILTER_ATTEN_LIM  = {'MURATTAL': 18, 'MUJAWWAD': 6,  'HADR': 18}
_STYLE_AFFTDN_NR_MAX         = {'MURATTAL': 0.03, 'MUJAWWAD': 0.01, 'HADR': 0.03}
_STYLE_LUFS_WINDOW_S         = {'MURATTAL': 3.0, 'MUJAWWAD': 5.0, 'HADR': 3.0}
_STYLE_SYL_WINDOW_MS         = {'MURATTAL': 120.0, 'MUJAWWAD': 120.0, 'HADR': 80.0}
_STYLE_MADD_MIN_H            = {'MURATTAL': 2.0, 'MUJAWWAD': 1.5, 'HADR': 1.5}
_ORNAMENT_F0_VARIANCE_CENTS  = 80.0
_ORNAMENT_DEEPFILTER_ATTEN   = 6
_COMMENTARY_LUFS_TARGET      = -14.0
_ARABIC_LUFS_TARGET          = -6.29

# ══════════════════════════════════════════════════════════════════════════════
#  PART IV v20 — SECTION A: CONSTANTS
# ══════════════════════════════════════════════════════════════════════════════

# ── QARA scoring weights (sum = 1.0) ─────────────────────────────────────────
_QARA_WEIGHTS = {
    'madd':      0.25,   # الماد — most commonly tested in evaluation
    'ghunnah':   0.20,   # الغنة — nasal quality at mim/nun positions
    'qalqalah':  0.15,   # القلقلة — burst at ق ط ب ج د
    'emphatic':  0.15,   # الإطباق — pharyngeal resonance preservation
    'sibilant':  0.10,   # الصفير — sibilant clarity
    'voice_dna': 0.10,   # المد الصوتي — DNA match to reference
    'health':    0.05,   # السلامة — discontinuity + SNR
}
_QARA_PASS_THRESHOLD  = 75.0
_QARA_GRADE_TABLE = {
    (90, 101): 'A', (80, 90): 'B', (70, 80): 'C',
    (60, 70): 'D', (0, 60): 'F',
}
_QARA_GHUNNAH_REF_RELATIVE_DB  = -18.5
_QARA_GHUNNAH_PASS_DELTA_DB    =  3.0
_QARA_QALQALAH_BURST_FLOOR_DB  = -42.0
_QARA_QALQALAH_WINDOW_MS       =  60.0
_QARA_EMPHATIC_BAND_LO         = 600.0
_QARA_EMPHATIC_BAND_HI         = 900.0
_QARA_EMPHATIC_MAX_DELTA_DB    =  2.0
_QARA_SIB_CENTROID_TARGET_HZ   = 4280.0
_QARA_SIB_CENTROID_TOL_HZ      = 400.0
_QARA_SIB_SNR_PASS_DB          =  15.0

# ── MUJAWWAD النداء: modules to skip per style ────────────────────────────────
_NIDAA_MUJAWWAD_SKIP  = {'n1_harmonic', 'n3_room'}
_NIDAA_HADR_SKIP      = set()

# ── Deployment audit status constants ────────────────────────────────────────
_AUDIT_STATUS_APPLIED   = 'applied'
_AUDIT_STATUS_BYPASSED  = 'bypassed'
_AUDIT_STATUS_FAILED    = 'failed'
_AUDIT_STATUS_SKIPPED   = 'skipped'
_AUDIT_STATUS_MISSING   = 'missing'

# ── Ornament frame chunked DF: crossfade ms ───────────────────────────────────
_ORNAMENT_CROSSFADE_MS  = 10.0

# ── GPU path constants ────────────────────────────────────────────────────────
_GPU_ENV_WEIGHTS    = 'GAP_URGENET_WEIGHTS'
_GPU_ENV_DEVICE     = 'GAP_URGENET_DEVICE'
_GPU_ATTEN_LIM_DB   = 30
...
_JALAL_TIER_PARAMS: dict = {
    ...
    'TIER_DEGRADED': dict(
        shimmer_blend=0.0,      # v18-FIX-1: disabled — mosque natural HF rolloff
        # was 0.020 → stacked +34dB metallic ring at 5kHz on noor-type sources.
        # J-1 synthesises 5-12kHz harmonics from 2.5-6kHz; on sources where
        # HF was already over-boosted by EQ + BWE the shimmer turned metallic.
...
_REF_CACHE = str(_APP_DIR / 'ref_cache_v140.json')
...
def _log_stage_hash(stage: str, wav_path: str) -> str:
    """KB-13-07: Sequential pipeline hash chain (4MB probe)."""
    try:
        h = hashlib.sha256(); read = 0; limit = 4 * 1024 * 1024
        with open(wav_path, 'rb') as f:
            while read < limit:
                chunk = f.read(min(65536, limit - read))
                if not chunk: break
                h.update(chunk); read += len(chunk)
        digest = h.hexdigest()[:16]
        L(f'  [KB-13-07/Hash] stage={stage:12s} sha256[:16]={digest}')
        return digest
    except Exception as e:
        L(f'  [KB-13-07/Hash] stage={stage} hash failed: {e}')
        return 'UNAVAILABLE'
...
class InputState:
    ...
    # ── v13 additions ────────────────────────────────────────────────────
    mos_proxy:                float = 0.0
    mos_sub_coloration:       float = 0.0
    mos_sub_discontinuity:    float = 0.0
    mos_sub_loudness:         float = 0.0
    mos_sub_reverb:           float = 0.0
    mos_sub_noise:            float = 0.0
    madd_events:              List[object] = field(default_factory=list)
    tape_iec2_shelf_applied:  bool  = False
    ikhfa_guard_applied:      bool  = False
    # ── v14 additions (Supplement v6) ────────────────────────────────────
    quran_prefix_set:         bool  = False   # KB-14-02
    quran_prefix_text:        str   = ''      # KB-14-02: Quran decoder prefix
    nr_backend:               str   = 'deepfilter'  # KB-14-04: NR backend
    joint_nr_bwe_candidate:   bool  = False   # KB-14-05: noise-robust BWE flag
    clearvoice_scores:        dict  = field(default_factory=dict)  # KB-14-01
    _input_wav_path:          str   = ''      # internal: path for SpeechScore
    # ── v15 الوقاية Phase 0 — Stereo-to-Mono + DC Drift (Plan Part 1, MISS-01/02) ──
    wiqaya_applied:           bool  = False   # Phase 0 ran and produced mono output
    wiqaya_method:            str   = 'bypass' # dual_mono_L|sum_safe|delay_align|anti_phase_corrected|one_sided_L/R|mono_in
    wiqaya_correlation:       float = 1.0     # Pearson r of voiced L/R frames
    wiqaya_delay_ms:          float = 0.0     # detected inter-channel delay (ms)
    wiqaya_makeup_db:         float = 0.0     # make-up gain applied after downmix (dB)
    dc_drift_corrected:       bool  = False   # polynomial DC drift removed
    dc_poly_degree:           int   = 0       # polynomial degree used (0=flat DC)
    dc_max_offset_db:         float = 0.0     # peak DC bias before correction (dBFS)
    # ── v15 الأذن الحساسة Phase A.8 — Opus/WhatsApp Detection (MISS-04) ─────────
    opus_detected:            bool  = False   # hard codec cutoff detected
    opus_bitrate_estimate:    int   = 0       # estimated bitrate: 8|16|32 kbps
    codec_type:               str   = ''      # 'opus'|'mp3_aac'|''
    silk_repair_applied:      bool  = False   # SILK formant widening applied
    phoneme_unreliable_3to8k: bool  = False   # PHONEME_CLASS_UNRELIABLE for 3-8kHz
    # ── v15 الأذن الحساسة Phase A.9 — Congregation Noise Taxonomy (MISS-05) ──────
    noise_map:                List[dict] = field(default_factory=list)
    ameen_detected:           bool  = False   # M-1 Ameen event mapped
    pa_feedback_detected:     bool  = False   # M-5 PA feedback event mapped
    congregation_murmur_ms:   int   = 0       # M-3 murmur total duration (ms)
    noise_map_has_printthrough: bool = False  # M-6 print-through guard active
    # ── v16 Part II — Arabic DNA Protection Layer ─────────────────────────────
    riwaya_detected:              str   = 'HAFS'
    riwaya_confidence:            float = 0.0
    riwaya_uncertain:             bool  = False
    riwaya_madd_table:            dict  = field(default_factory=dict)
    riwaya_hamzah_rules:          dict  = field(default_factory=dict)
    _harakah_ms_calibrated:       float = 200.0
    style_class:                  str   = 'MURATTAL'
    style_confidence:             float = 0.0
    style_deepfilter_atten_lim:   int   = 18
    style_afftdn_nr_max:          float = 0.03
    ornament_frame_mask:          list  = field(default_factory=list)
    ornament_frames_detected:     int   = 0
    amplitude_drift_detected:     bool  = False
    amplitude_drift_range_LU:     float = 0.0
    amplitude_splice_detected:    bool  = False
    amplitude_drift_corrected:    bool  = False
    spectral_tilt_drift_dB_per_min: float = 0.0
    tilt_correction_applied:      bool  = False
    has_multilingual_content:     bool  = False
    language_segment_map:         list  = field(default_factory=list)
    madd_events_v2:               list  = field(default_factory=list)
    # ── v17 Part III — Generative Intelligence Layer ──────────────────────────
    silero_vad_available:         bool  = False
    wpe_applied:                  bool  = False
    wpe_lra_guard_fired:          bool  = False
    wpe_madd_guard_fired:         bool  = False
    wpe_idgham_guard_fired:       bool  = False
    wpe_rt60_reduction_s:         float = 0.0
    hakim_status:                 str   = ''
    hakim_driftse_applied:        bool  = False
    hakim_driftse_accepted:       bool  = False
    hakim_sips_applied:           bool  = False
    hakim_sips_accepted:          bool  = False
    hakim_emphatic_delta:         float = 0.0
    hakim_madd_delta:             float = 0.0
    hakim_style_gate:             str   = ''
    hakim_driftse_backend:        str   = ''   # v21: 'resemble' | 'frcrn' | ''
    hakim_dnsmos_ovrl_before:     float = 0.0  # v21: DNSMOS P.835 OVRL pre-SIPS
    hakim_dnsmos_ovrl_after:      float = 0.0  # v21: DNSMOS P.835 OVRL post-SIPS
    # المرآة: reference quality = ECAPA speaker cosine similarity (v21)
    miraat_status:                str   = ''
    miraat_applied:               bool  = False
    miraat_accepted:              bool  = False
    miraat_ref_quality:           float = 0.0
    miraat_gates_passed:          int   = 0
    idrak_status:                 str   = ''
    idrak_applied:                bool  = False
    idrak_accepted:               bool  = False
    idrak_text_conditioned:       bool  = False
    idrak_fallback_path:          str   = ''
    sadaa_pyroomacoustics:        bool  = False
    sadaa_rir_rt60:               float = 0.0
    # ── v20 Part IV fields ────────────────────────────────────────────────────
    deepfilter3_applied:          bool  = False
    sib_snr:                      float = 0.0
    gpu_enhance_applied:          bool  = False
    qara_report:                  object = None
    _arabic_wavlm_confirmed:      bool  = False
    _arabic_wavlm_risk:           str   = 'UNKNOWN'
    nidaa_applied:                bool  = False
    nidaa_delta_lufs:             float = 0.0
    nidaa_modules:                str   = ''
    jalal_applied:                bool  = False
    jalal_shimmer:                bool  = False
    jalal_widener:                bool  = False
    jawhar_applied:               bool  = False
    ihyaa_applied:                bool  = False
    noor_applied:                 bool  = False


@dataclass
class QARAReport:
    """QARA — Quranic Audio Restoration Assessment. 7-dimension Tajweed scoring."""
    madd_score:           float = 0.0
    madd_pass:            bool  = False
    madd_total_n:         int   = 0
    madd_insufficient_n:  int   = 0
    madd_ok_n:            int   = 0
    madd_riwaya:          str   = 'HAFS'
    madd_harakah_ms:      float = 200.0
    ghunnah_score:        float = 0.0
    ghunnah_pass:         bool  = False
    ghunnah_250hz_db:     float = 0.0
    ghunnah_delta_db:     float = 0.0
    qalqalah_score:       float = 0.0
    qalqalah_pass:        bool  = False
    qalqalah_detected_n:  int   = 0
    qalqalah_intact_n:    int   = 0
    emphatic_score:       float = 0.0
    emphatic_pass:        bool  = False
    emphatic_delta_db:    float = 0.0
    sibilant_score:       float = 0.0
    sibilant_pass:        bool  = False
    sibilant_snr_db:      float = 0.0
    sibilant_centroid_hz: float = 0.0
    voice_dna_score:      float = 0.0
    voice_dna_pass:       bool  = False
    nidaa_applied:        bool  = False
    nidaa_delta_lufs:     float = 0.0
    health_score:         float = 0.0
    health_pass:          bool  = False
    health_snr_db:        float = 0.0
    health_discontinuity: float = 0.0
    ...
    def grade_from_score(self, score: float) -> str:
        for (lo, hi), g in _QARA_GRADE_TABLE.items():
            if lo <= score < hi:
                return g
        return 'F'
    ...
    def to_dict(self) -> dict:
        return {
            'qara_composite':     round(self.composite_score, 1),
            'qara_grade':         self.overall_grade,
            'qara_pass':          self.composite_pass,
            'qara_dimensions_ok': self.dimensions_passed,
            'qara_fail_reasons':  self.fail_reasons,
            'dimensions': {
                'madd':      {'score': round(self.madd_score,1),      'pass': self.madd_pass,
                              'total': self.madd_total_n, 'insufficient': self.madd_insufficient_n},
                'ghunnah':   {'score': round(self.ghunnah_score,1),   'pass': self.ghunnah_pass,
                              '250hz_db': round(self.ghunnah_250hz_db,2)},
                'qalqalah':  {'score': round(self.qalqalah_score,1),  'pass': self.qalqalah_pass,
                              'detected': self.qalqalah_detected_n,   'intact': self.qalqalah_intact_n},
                'emphatic':  {'score': round(self.emphatic_score,1),  'pass': self.emphatic_pass,
                              'delta_db': round(self.emphatic_delta_db,2)},
                'sibilant':  {'score': round(self.sibilant_score,1),  'pass': self.sibilant_pass,
                              'snr_db': round(self.sibilant_snr_db,2),
                              'centroid_hz': round(self.sibilant_centroid_hz,1)},
                'voice_dna': {'score': round(self.voice_dna_score,1), 'pass': self.voice_dna_pass},
                'health':    {'score': round(self.health_score,1),    'pass': self.health_pass,
                              'snr_db': round(self.health_snr_db,1)},
            },
        }
...
def load_reference_model(ref_files: List[str] = None) -> ReferenceModel:
    ...
    try:
        ...
        cache_d = {
            'cache_version': 'v15.0', 'ref_hash': current_hash,
...
# ══════════════════════════════════════════════════════════════════════════════
#  الوقاية (Al-Wiqaya) — PHASE 0: MUST RUN BEFORE ALL OTHER PROCESSING
#  Addresses MISS-01 (stereo-to-mono protocol) + MISS-02 (DC drift)
#  Oracle §79, §83.2 · Plan Part I · RED LINE #17
# ══════════════════════════════════════════════════════════════════════════════

def _wiqaya_extract_channel(input_path: str, ch: str, state: InputState) -> str:
    """Extract single channel (L or R) as mono 48kHz WAV."""
    import uuid as _uu_ec
    out = os.path.join(_TMP, f'wiqaya_ch_{_uu_ec.uuid4().hex[:8]}.wav')
    pan = 'pan=mono|c0=FL' if ch == 'L' else 'pan=mono|c0=FR'
    sp, tc = _safe(input_path)
    r = subprocess.run(
        ['ffmpeg', '-y', '-i', sp, '-af', pan,
         '-ar', str(SR), '-ac', '1', '-c:a', WAV_CODEC, '-loglevel', 'error', out],
        capture_output=True)
    if tc:
        try: os.remove(tc)
        except: pass
    if r.returncode == 0 and os.path.exists(out) and os.path.getsize(out) > 1000:
        state.wiqaya_applied = True
        return out
    return input_path


def _wiqaya_write_mono(input_path: str, lag_samples: int,
                        state: InputState) -> str:
    """
    الوقاية helper: write phase-safe mono WAV.
    Applies inter-channel delay correction (adelay) before summing.
    Checks for post-downmix RMS drop and applies make-up gain if >3dB.
    """
    import uuid as _uu_wm
    out = os.path.join(_TMP, f'wiqaya_mono_{_uu_wm.uuid4().hex[:8]}.wav')

    if abs(lag_samples) >= 1:
        lag_ms = abs(lag_samples) / SR * 1000.0
        # adelay delays are per-channel in ms, pipe-separated
        # Positive lag = L leads R → delay L; negative → delay R
        delay_str = f'{lag_ms:.3f}|0' if lag_samples > 0 else f'0|{lag_ms:.3f}'
        af = (f'aformat=channel_layouts=stereo,'
              f'adelay=delays={delay_str},'
    ...
    if r.returncode != 0 or not os.path.exists(out) or os.path.getsize(out) < 1000:
        L('  [الوقاية] mono write failed — input unchanged')
        return input_path

    # Step 5 (Plan §0.1): Post-downmix RMS check vs L-channel baseline
    try:
        dur = min(10.0, state.total_s * 0.25 if state.total_s > 30 else 10.0)
        sp2, tc2 = _safe(input_path)
        r_L = subprocess.run(
            ['ffmpeg', '-y', '-i', sp2, '-t', str(dur),
             '-af', 'pan=mono|c0=FL', '-f', 'f32le',
        ...
        r_m = subprocess.run(
            ['ffmpeg', '-y', '-i', out, '-t', str(dur),
             '-f', 'f32le', '-ar', str(SR), '-loglevel', 'error', '-'],
            capture_output=True)
        if r_L.stdout and r_m.stdout and NUMPY_OK:
            L_a = np.frombuffer(r_L.stdout, dtype=np.float32)
            m_a = np.frombuffer(r_m.stdout, dtype=np.float32)
            if len(L_a) > SR * 0.5 and len(m_a) > SR * 0.5:
                rms_L = float(20 * np.log10(np.sqrt(np.mean(L_a**2)) + 1e-10))
                rms_m = float(20 * np.log10(np.sqrt(np.mean(m_a**2)) + 1e-10))
                drop  = rms_L - rms_m
                if drop > 3.0:
                    makeup = float(np.clip(drop, 0.0, 6.0))
                    state.wiqaya_makeup_db = round(makeup, 2)
                    L(f'  [الوقاية] ⚠ RMS drop {drop:.1f}dB after downmix → '
                      f'{makeup:.1f}dB make-up gain')
                    out2 = os.path.join(_TMP, f'wiqaya_mk_{_uu_wm.uuid4().hex[:8]}.wav')
                    r2 = subprocess.run(
                        ['ffmpeg', '-y', '-i', out,
                         '-af', f'volume={makeup:.2f}dB',
                         '-c:a', WAV_CODEC, '-loglevel', 'error', out2],
                        capture_output=True)
                    if r2.returncode == 0 and os.path.exists(out2):
                        out = out2
    except Exception as _e:
        pass  # RMS check non-fatal

    state.wiqaya_applied = True
    return out


def _stereo_to_mono_wiqaya(input_path: str, state: InputState) -> str:
    """
    الوقاية Phase 0.1 — Phase-Safe Stereo-to-Mono Protocol
    Oracle §79 · KB §41.3 · Plan Part I · RED LINE #17

    Runs BEFORE ALL other processing. Anti-phase summation creates a comb
    null at exactly 250Hz — the ghunnah nasal pole — before the engine
    runs a single analysis step.

    Routing table (Pearson r on voiced frames):
      r > 0.95  → dual-mono: L channel only (no summing)
      r > 0.70  → safe: (L+R)/2
      r > 0.30  → delay-align, then sum
      r ≤ 0.30  → ANTI-PHASE WARNING: delay-align before sum (mandatory)
      RMS(L)/RMS(R) > 20dB → louder channel directly (near-silence on other)

    Returns: path to mono 48kHz WAV (or input_path if already mono/failure).
    """
    if not NUMPY_OK:
        state.wiqaya_method = 'numpy_unavailable'
        return input_path

    # Probe channel count
    probe_d = probe_file(input_path)
    n_channels = 1
    for s in probe_d.get('streams', []):
        if s.get('codec_type') == 'audio':
            try:
                n_channels = int(s.get('channels', 1))
    ...
    if n_channels == 1:
        state.is_mono = True
        state.wiqaya_method = 'mono_in'
        state.wiqaya_correlation = 1.0
        L('  [الوقاية 0.1] input is mono — pass through')
        return input_path

    # Load stereo for analysis (30s max to keep memory bounded)
    analysis_s = min(30.0, max(5.0, state.total_s * 0.25)) if state.total_s > 5 else 15.0
    sp, tc = _safe(input_path)
    r = subprocess.run(
        ['ffmpeg', '-y', '-i', sp, '-t', str(analysis_s),
         '-ac', '2', '-f', 'f32le', '-ar', str(SR), '-loglevel', 'error', '-'],
        capture_output=True)
    if tc:
        try: os.remove(tc)
        except: pass

    if not r.stdout or len(r.stdout) < SR * 2:
        L('  [الوقاية 0.1] stereo load failed — safe 0.5*(L+R) fallback')
        state.wiqaya_method = 'sum_safe_fallback'
        return _wiqaya_write_mono(input_path, 0, state)
    ...
    L_ch = L_ch[:n]; R_ch = R_ch[:n]

    # Step 4: One-sided mono detection (>20dB RMS asymmetry)
    rms_L_db = float(20 * np.log10(np.sqrt(np.mean(L_ch**2)) + 1e-10))
    rms_R_db = float(20 * np.log10(np.sqrt(np.mean(R_ch**2)) + 1e-10))
    rms_diff = abs(rms_L_db - rms_R_db)
    if rms_diff > 20.0:
        louder = 'L' if rms_L_db > rms_R_db else 'R'
        L(f'  [الوقاية 0.1] one-sided mono: L={rms_L_db:.1f} R={rms_R_db:.1f}dB '
          f'(Δ={rms_diff:.1f}dB) → extracting {louder} channel')
        state.wiqaya_method = f'one_sided_{louder}'
        state.wiqaya_correlation = 0.0
        return _wiqaya_extract_channel(input_path, louder, state)

    # Step 1: Pearson r on voiced frames (energy-gated)
    frame_n = int(0.025 * SR)
    mid = (L_ch + R_ch) / 2.0
    thresh = float(np.sqrt(np.mean(mid**2))) * 0.15  # 15% of overall RMS

    L_voiced = []; R_voiced = []
    for i in range(0, n - frame_n, frame_n):
        lf = L_ch[i:i+frame_n]; rf = R_ch[i:i+frame_n]
        if float(np.sqrt(np.mean(((lf+rf)/2)**2))) > thresh:
            L_voiced.extend(lf.tolist()); R_voiced.extend(rf.tolist())
        if len(L_voiced) > SR * 10:
            break

    if len(L_voiced) < SR * 0.5:
        # Fallback: use full signal
        L_voiced = L_ch[:min(n, SR*10)].tolist()
        R_voiced = R_ch[:min(n, SR*10)].tolist()

    Lv = np.array(L_voiced); Rv = np.array(R_voiced)
    Lv -= np.mean(Lv); Rv -= np.mean(Rv)
    std_L = float(np.std(Lv) + 1e-12); std_R = float(np.std(Rv) + 1e-12)
    r_corr = float(np.clip(np.mean(Lv * Rv) / (std_L * std_R), -1.0, 1.0))
    state.wiqaya_correlation = round(r_corr, 3)

    # Step 3: Inter-channel delay detection via cross-correlation
    lag_samples = 0
    if r_corr < 0.95:
        max_lag_n = int(5.0 / 1000.0 * SR)   # ±5ms search window
        seg_n = min(n, int(SR * 10))
        sL = L_ch[:seg_n].copy(); sR = R_ch[:seg_n].copy()
        sL -= np.mean(sL); sR -= np.mean(sR)
        sL /= (np.std(sL) + 1e-10); sR /= (np.std(sR) + 1e-10)
        n_fft = 1
        while n_fft < 2 * seg_n: n_fft <<= 1
        xc = np.fft.irfft(
            np.fft.rfft(sL, n=n_fft) * np.conj(np.fft.rfft(sR, n=n_fft)),
            n=n_fft)
        lags = list(range(-max_lag_n, max_lag_n + 1))
        xc_w = np.concatenate([xc[-max_lag_n:], xc[:max_lag_n + 1]])
        lag_samples = lags[int(np.argmax(np.abs(xc_w)))]
        state.wiqaya_delay_ms = round(lag_samples / SR * 1000.0, 3)
        if abs(lag_samples) >= 1:
            L(f'  [الوقاية 0.1] inter-channel delay: '
              f'{state.wiqaya_delay_ms:.2f}ms ({lag_samples} samp) — will correct')

    # Step 2: Route by correlation
    if r_corr > 0.95:
        L(f'  [الوقاية 0.1] dual-mono (r={r_corr:.3f}) → L channel only')
        state.wiqaya_method = 'dual_mono_L'
        return _wiqaya_extract_channel(input_path, 'L', state)

    elif r_corr > 0.70:
        L(f'  [الوقاية 0.1] safe sum (r={r_corr:.3f}, lag={lag_samples}samp)')
        state.wiqaya_method = 'sum_safe'
        return _wiqaya_write_mono(input_path, lag_samples, state)

    elif r_corr > 0.30:
        L(f'  [الوقاية 0.1] delay-align + sum '
          f'(r={r_corr:.3f}, lag={lag_samples}samp = {state.wiqaya_delay_ms:.2f}ms)')
        state.wiqaya_method = 'delay_align'
        return _wiqaya_write_mono(input_path, lag_samples, state)

    else:
        L(f'  [الوقاية 0.1] ⚠ ANTI-PHASE (r={r_corr:.3f}) — '
          f'delay={state.wiqaya_delay_ms:.2f}ms — forcing correction')
        L(f'  [الوقاية 0.1] Without correction: 250Hz comb null destroys ghunnah '
          f'nasal pole before any analysis. RED LINE #17.')
        state.wiqaya_method = 'anti_phase_corrected'
        return _wiqaya_write_mono(input_path, lag_samples, state)


def _write_corrected_audio(audio: 'np.ndarray', suffix: str) -> str:
    """Write corrected float32 mono numpy array to a temp WAV file via ffmpeg."""
    import uuid as _uu_wca
    out = os.path.join(_TMP, f'{suffix}_{_uu_wca.uuid4().hex[:8]}.wav')
    try:
        r = subprocess.run(
            ['ffmpeg', '-y', '-f', 'f32le', '-ar', str(SR), '-ac', '1',
             '-i', '-', '-c:a', WAV_CODEC, '-loglevel', 'error', out],
...
def _correct_dc_drift(input_wav: str, state: InputState) -> str:
    """
    الوقاية Phase 0.2 — DC Offset Drift Correction
    Oracle §83.2 · Plan Part I · MISS-02

    Corrects capacitor leakage / ADC thermal drift in long recordings.
    DC offset causes false clip detection, wrong SNR estimates, and
    corrupts spectral analysis below 20Hz.

    Run BEFORE tier classification — DC inflates clip_ratio.

    Short files (<30s): simple DC removal (np.mean subtraction).
    Long files: polynomial drift fitting per 30s window (degree 2–3).
    Degree 3 for recordings > 20 minutes (1200s).

    Returns: path to DC-corrected WAV (or input_wav if correction unneeded).
    """
    if not NUMPY_OK:
        return input_wav

    # Load full audio (mono after Phase 0.1)
    r = subprocess.run(
        ['ffmpeg', '-y', '-i', input_wav,
         '-ac', '1', '-f', 'f32le', '-ar', str(SR), '-loglevel', 'error', '-'],
        capture_output=True)
    if not r.stdout or len(r.stdout) < SR * 4:
        return input_wav

    audio = np.frombuffer(r.stdout, dtype=np.float32).astype(np.float64).copy()
    total_s_here = len(audio) / SR

    # Short files: single mean DC removal
    if total_s_here < 30.0:
        dc = float(np.mean(audio))
        if abs(dc) < 0.001:  # < 0.1% offset: negligible
            return input_wav
        audio -= dc
        state.dc_drift_corrected = True
        state.dc_poly_degree     = 0
        state.dc_max_offset_db   = round(float(20 * np.log10(abs(dc) + 1e-10)), 1)
        L(f'  [الوقاية 0.2] DC offset {dc:.5f} ({state.dc_max_offset_db:.1f}dBFS) removed')
        out = _write_corrected_audio(audio.astype(np.float32), 'dc_fixed')
        return out if out else input_wav

    # Long files: polynomial drift fitting (Oracle §83.2)
    window_s = 30.0
    window_n = int(window_s * SR)
    n_windows = int(total_s_here // window_s)
    ...
    # Step 1: Mean amplitude per 30s window
    window_times = np.array([(i + 0.5) * window_s for i in range(n_windows)])
    window_means = np.array([
        float(np.mean(audio[i*window_n : min((i+1)*window_n, len(audio))]))
        for i in range(n_windows)
    ])

    max_offset = float(np.max(np.abs(window_means)))
    max_offset_db = float(20 * np.log10(max_offset + 1e-10))

    if max_offset < 0.001:  # negligible drift
        L(f'  [الوقاية 0.2] DC drift negligible ({max_offset_db:.1f}dBFS) — skip')
        return input_wav

    # Step 2: Polynomial fit (degree 2–3)
    degree = 3 if total_s_here > 1200.0 else 2  # cubic for >20 min recordings
    t_full = np.linspace(0, total_s_here, len(audio))

    try:
        coeffs   = np.polyfit(window_times, window_means, degree)
        dc_curve = np.polyval(coeffs, t_full)
    except (np.linalg.LinAlgError, ValueError):
        # Fallback: linear detrend
        try:
            from scipy.signal import detrend as _detrend_sc
            audio = _detrend_sc(audio, type='linear')
        except ImportError:
            audio -= np.mean(audio)
        state.dc_drift_corrected = True
        state.dc_poly_degree     = 1
        state.dc_max_offset_db   = round(max_offset_db, 1)
        L(f'  [الوقاية 0.2] DC drift linear detrend (polyfit failed) '
          f'peak={max_offset_db:.1f}dBFS')
        out = _write_corrected_audio(audio.astype(np.float32), 'dc_fixed')
        return out if out else input_wav

    # Step 3: Subtract the polynomial curve from the full waveform
    audio -= dc_curve

    # Step 4: Verify residual
    post_means = [
        float(np.mean(audio[i*window_n : min((i+1)*window_n, len(audio))]))
        for i in range(min(n_windows, 10))
    ]
    post_max = float(np.max(np.abs(post_means)))

    state.dc_drift_corrected = True
    state.dc_poly_degree     = degree
    state.dc_max_offset_db   = round(max_offset_db, 1)
    L(f'  [الوقاية 0.2] DC drift corrected: poly={degree} '
      f'peak_before={max_offset_db:.1f}dBFS residual={post_max:.5f}')
...
# ══════════════════════════════════════════════════════════════════════════════
#  الأذن الحساسة (Al-Udhun Al-Hassasa) — PHASE A.8 + A.9
#  Addresses MISS-04 (Opus/WhatsApp detection) + MISS-05 (congregation noise)
#  Oracle §78, §81 · Plan Part I
# ══════════════════════════════════════════════════════════════════════════════

def _detect_opus_whatsapp(audio: 'np.ndarray', state: InputState,
                           sr: int = SR) -> None:
    """
    الأذن الحساسة Phase A.8 — Opus/WhatsApp Tier Detection
    Oracle §78 · Plan Part I · MISS-04

    Detects codec by hard cutoff pattern (steep spectral drop) vs gradual rolloff.
    A hard cutoff (>30dB drop within 1.5× the cutoff frequency) is the signature
    of Opus SILK/CELT encoding, distinct from natural HF rolloff.

    FORCE TIER overrides (applied AFTER analyze_input tier classification):
      cutoff ≈ 4kHz  (Opus 8kbps)  → FORCE TIER_CRITICAL
      cutoff ≈ 8kHz  (Opus 16kbps) → FORCE TIER_DEGRADED
      cutoff ≈ 12kHz (Opus 32kbps) → TIER_COMPRESSED (no force if already higher)
    """
    ...
    try:
        ...
        # Reference band: 1–2kHz (below any expected cutoff, reliable content)
        ref_mask = (freqs >= 1000) & (freqs < 2000)
        if not ref_mask.any():
            return
        ref_db = float(10 * np.log10(np.mean(X[ref_mask]) + 1e-30))

        def band_db(f_lo, f_hi):
            m = (freqs >= f_lo) & (freqs < f_hi)
            return float(10 * np.log10(np.mean(X[m]) + 1e-30)) if m.any() else -120.0

        # Candidate cutoffs: (cutoff_hz, estimated_br_kbps, forced_tier)
        cutoff_tests = [
            (4000,  8,  'TIER_CRITICAL'),
            (8000,  16, 'TIER_DEGRADED'),
            (12000, 32, 'TIER_COMPRESSED'),
        ]

        detected = False
        for cutoff_hz, br_kbps, forced_tier in cutoff_tests:
            below_db = band_db(cutoff_hz * 0.75, cutoff_hz)
            above_db = band_db(cutoff_hz, cutoff_hz * 1.75)
            drop = below_db - above_db

            # Hard cutoff: >30dB drop, and below-cutoff has real content
            if drop > 30.0 and below_db > ref_db - 35.0:
                state.opus_detected           = True
                state.opus_bitrate_estimate   = br_kbps
                state.codec_type              = 'opus'

                # Force tier (overrides analyze_input classification)
                state.source_tier = forced_tier
                state.achievable_lufs, state.achievable_crest, state.achievable_lra = (
                    _compute_achievable(forced_tier, float(cutoff_hz),
                                        state.src_sr, state.clip_ratio,
                                        state.noise_type, state.smear_score))

                L(f'  [A.8] Opus/WhatsApp detected: '
                  f'cutoff≈{cutoff_hz}Hz drop={drop:.1f}dB → '
                  f'~{br_kbps}kbps → FORCE {forced_tier}')

                # Tajweed consequence log (Oracle §78.3)
                if br_kbps <= 16:
                    state.phoneme_unreliable_3to8k = True
                    L(f'  [A.8] PHONEME_CLASS_UNRELIABLE: ص/س sibilant distinction '
                      f'below {cutoff_hz}Hz cutoff — emphatic/non-emphatic '
                      f'classification unreliable for 3–8kHz band')

                detected = True
                break

        if not detected:
            # Check for gradual rolloff (natural MP3/AAC — no tier override)
            above_12k = band_db(11000, 14000)
            if above_12k < ref_db - 15.0:
                state.codec_type = 'mp3_aac'
                L(f'  [A.8] gradual rolloff → natural codec (MP3/AAC) — no Opus tier override')
            else:
                L(f'  [A.8] no hard codec cutoff detected → standard tier stands')

    except Exception as _e:
        L(f'  [A.8] Opus detection error (non-fatal): {_e}')


def _apply_silk_repair(input_wav: str, state: InputState) -> str:
    """
    الأذن الحساسة Phase A.8 — SILK Artifact Repair
    Oracle §78.2 · Plan Part I

    Runs when opus_detected=True and opus_bitrate_estimate < 32kbps.

    SILK LPC over-smooths formants (narrow bandwidth = plasticky timbre).
    Repair: gentle shelf lifts at F1 (300-500Hz) and F2 (700-900Hz) ranges
    to widen apparent formant bandwidth, restoring natural vowel resonance.
    Also adds a 25Hz highpass to remove sub-bass LPC residual thumps.

    Caps at ±2dB to ensure no tajweed-significant spectral change.
    """
    if not state.opus_detected or state.opus_bitrate_estimate >= 32:
        return input_wav

    L(f'  [A.8/SILK] SILK artifact repair (Opus ≈{state.opus_bitrate_estimate}kbps)...')
    try:
        import uuid as _uu_silk
        out = os.path.join(_TMP, f'silk_{_uu_silk.uuid4().hex[:8]}.wav')

        # Formant bandwidth widening (cap ±2dB per Oracle §78.2)
        silk_parts = [
            'equalizer=f=380:width_type=o:width=1.8:g=1.2',  # F1 open
            'equalizer=f=800:width_type=o:width=1.8:g=0.9',  # F2 open
            'highpass=f=25:poles=2',                           # sub-bass residual
        ]
        silk_filter = ','.join(silk_parts)

        r = subprocess.run(
            ['ffmpeg', '-y', '-i', input_wav, '-af', silk_filter,
             '-c:a', WAV_CODEC, '-loglevel', 'error', out],
            capture_output=True)

        if r.returncode == 0 and os.path.exists(out) and os.path.getsize(out) > 1000:
            state.silk_repair_applied = True
            L('  [A.8/SILK] ✓ formant bandwidth widened, residual smoothed')
            return out

    except Exception as _e:
        L(f'  [A.8/SILK] repair error (non-fatal): {_e}')

    return input_wav


def _classify_congregation_noise(audio: 'np.ndarray', state: InputState,
                                  sr: int = SR) -> None:
    """
    الأذن الحساسة Phase A.9 — Congregation Noise Taxonomy
    Oracle §81 · Plan Part I · MISS-05

    Runs ONCE in Phase A after tier classification. Classifies all
    congregation background noise into targeted types for Phase B suppression.

    Noise map types and treatments:
      M-1  Ameen (80-180Hz F0, 500-2500ms)         → SKIP_NR timestamp gate
      M-2  Takbeer (shorter burst)                   → RECORDING_ANOMALY if mid-recitation
      M-3  General murmur (300-3000Hz broadband)     → ONLY type for afftdn NR
      M-4a Robe rustle (broadband, 20-80ms)          → impulse treatment
      M-4c Cough/sneeze (broadband burst, 30-400ms)  → R-2 dropout path
      M-4d Phone ring (narrow tonal, repeating)      → dynamic notch filter
      M-5  PA feedback (tonal 800-3000Hz, rapid)     → dynamic notch
      M-6  Print-through (pre/post-echo -50 to -60dBFS) → R-5c silence guard

    Suppression sequence (Oracle §81): M-5 → M-4d → M-1/M-2 gates →
    M-4a/c impulse → M-3 broadband NR only. NEVER mix strategies.
    """
    if not NUMPY_OK or len(audio) < sr * 2:
        return
    ...
    noise_map = []
    try:
        frame_n   = int(0.10 * sr)    # 100ms frames
        hop_n     = int(0.05 * sr)    # 50ms hop
        ...
        f_rms  = np.zeros(n_frames)
        f_zcr  = np.zeros(n_frames)
        f_cent = np.zeros(n_frames)
        f_low  = np.zeros(n_frames)
        f_ton  = np.zeros(n_frames)

        for i in range(n_frames):
            s   = i * hop_n
            frm = audio[s:s + frame_n].astype(np.float64)
            rms = float(np.sqrt(np.mean(frm**2)) + 1e-12)
            f_rms[i] = rms
            f_zcr[i] = float(np.sum(np.abs(np.diff(np.sign(frm)))) / (2 * frame_n))

            spec  = np.abs(rfft(frm * np.hanning(frame_n)))**2
            freqs = rfftfreq(frame_n, 1.0 / sr)

            # Centroid 200-8000Hz
            m_sp = (freqs >= 200) & (freqs <= 8000)
            if m_sp.any():
                s_sp = spec[m_sp] + 1e-30
                f_cent[i] = float(np.sum(freqs[m_sp] * s_sp) / np.sum(s_sp))
            else:
                f_cent[i] = 2000.0

            # Tonality 300-3000Hz (SFM-based)
            m_t = (freqs >= 300) & (freqs <= 3000)
            if m_t.any():
                s_t = spec[m_t] + 1e-10
                sfm = float(np.exp(np.mean(np.log(s_t))) / np.mean(s_t))
                f_ton[i] = float(np.clip(sfm, 0, 1))
            else:
                f_ton[i] = 0.1

        rms_db_arr = 20 * np.log10(f_rms + 1e-10)

        # ── M-5: PA Feedback ─────────────────────────────────────────────────
        for i in range(3, n_frames):
            if (f_ton[i] > 0.70 and
                    800 <= f_cent[i] <= 3000 and
                    rms_db_arr[i] > float(np.mean(rms_db_arr[max(0,i-5):i])) + 8.0):
                t_ms = int(i * hop_n / sr * 1000)
                noise_map.append({
                    'type': 'M-5_pa_feedback', 'start_ms': max(0, t_ms-50),
                    'end_ms': t_ms + 500, 'center_hz': float(f_cent[i]),
                    'confidence': 0.80, 'treatment': 'dynamic_notch',
                })
                state.pa_feedback_detected = True

        pa_count = sum(1 for e in noise_map if e['type'] == 'M-5_pa_feedback')
        if pa_count:
            L(f'  [A.9] M-5 PA feedback: {pa_count} event(s) → dynamic_notch')

        # ── M-4d: Phone Ring ─────────────────────────────────────────────────
        phone_events = 0
        in_burst = False; burst_start = 0
        for i in range(n_frames):
            in_tonal_burst = (f_ton[i] > 0.65 and
                              rms_db_arr[i] > overall_db - 15 and
                              1000 <= f_cent[i] <= 4000)
            if in_tonal_burst:
                if not in_burst: burst_start = i
                in_burst = True
            else:
                if in_burst:
                    dur_ms = (i - burst_start) * hop_n / sr * 1000
                    if 200 <= dur_ms <= 2500:
                        t_ms = int(burst_start * hop_n / sr * 1000)
                        noise_map.append({
                            'type': 'M-4d_phone', 'start_ms': t_ms,
                            'end_ms': int(i * hop_n / sr * 1000),
                            'confidence': 0.65, 'treatment': 'dynamic_notch',
                        })
                        phone_events += 1
                in_burst = False
        if phone_events:
            L(f'  [A.9] M-4d phone ring: {phone_events} event(s) → dynamic_notch')

        # ── M-4c: Cough / Sneeze ─────────────────────────────────────────────
        cough_events = 0
        for i in range(3, n_frames - 2):
            bg = float(np.mean(rms_db_arr[max(0,i-5):i]))
            if (rms_db_arr[i] > bg + 12 and
                    f_zcr[i] > 0.25 and f_cent[i] > 2000):
                # Measure burst duration
                j = i + 1
                while j < n_frames and rms_db_arr[j] > bg + 4:
                    j += 1
                dur_ms = (j - i) * hop_n / sr * 1000
                if 30 <= dur_ms <= 400:
                    t_ms = int(i * hop_n / sr * 1000)
                    noise_map.append({
                        'type': 'M-4c_cough', 'start_ms': t_ms,
                        'end_ms': t_ms + int(dur_ms),
                        'confidence': 0.70, 'treatment': 'dropout_R2',
                    })
                    cough_events += 1
        if cough_events:
            L(f'  [A.9] M-4c cough/sneeze: {cough_events} event(s) → R-2 dropout path')

        # ── M-3: General Congregation Murmur ─────────────────────────────────
        # Broadband 300-3000Hz, non-stationary — the ONLY valid afftdn target
        murmur_frames = sum(
            1 for i in range(n_frames)
            if (overall_db - 25 < rms_db_arr[i] < overall_db - 8 and
                0.05 < f_zcr[i] < 0.25 and
                300 <= f_cent[i] <= 3000)
        )
        murmur_ms = int(murmur_frames * hop_n / sr * 1000)
        if murmur_ms > 300:
            state.congregation_murmur_ms = murmur_ms
            noise_map.append({
                'type': 'M-3_murmur', 'start_ms': 0,
                'end_ms': int(len(audio) / sr * 1000),
                'duration_ms': murmur_ms, 'confidence': 0.75,
                'treatment': 'afftdn_nr',
            })
            L(f'  [A.9] M-3 congregation murmur: {murmur_ms}ms → afftdn_nr')

        # ── M-1: Ameen Detection ─────────────────────────────────────────────
        # Voiced speech at low pitch (80-180Hz F0), 500-2500ms duration.
        # Heuristic: voiced low-centroid sustained burst in second half of audio.
        in_ameen = False; ameen_start = 0; ameen_count = 0
        mid_point_i = n_frames // 3  # start scanning from 1/3 of file

        for i in range(mid_point_i, n_frames):
            voiced_low = (rms_db_arr[i] > overall_db - 15 and
                          f_cent[i] < 700 and f_zcr[i] < 0.12)
            if voiced_low:
                if not in_ameen: ameen_start = i
                in_ameen = True; ameen_count += 1
            else:
                if in_ameen and ameen_count > 0:
                    dur_ms = ameen_count * hop_n / sr * 1000
                    if 400 <= dur_ms <= 3000:
                        t_ms = int(ameen_start * hop_n / sr * 1000)
                        noise_map.append({
                            'type': 'M-1_ameen',
                            'start_ms': max(0, t_ms - 300),
                            'end_ms': t_ms + int(dur_ms) + 600,
                            'confidence': 0.72, 'treatment': 'SKIP_NR',
                            'skip_nr': True,
                        })
                        state.ameen_detected = True
                        L(f'  [A.9] M-1 Ameen at {t_ms}ms '
                          f'({dur_ms:.0f}ms) → SKIP_NR gate')
                in_ameen = False; ameen_count = 0

        # ── M-6: Print-through Detection ─────────────────────────────────────
        silence_frames_idx = np.where(rms_db_arr < overall_db - 25)[0]
        if len(silence_frames_idx) >= 5:
            sil_rms_db = float(20 * np.log10(
                np.mean(f_rms[silence_frames_idx]) + 1e-10))
            if -65 <= sil_rms_db <= -50:
                state.noise_map_has_printthrough = True
                noise_map.append({
                    'type': 'M-6_printthrough', 'floor_db': round(sil_rms_db, 1),
                    'confidence': 0.60, 'treatment': 'r5c_guard',
                })
                L(f'  [A.9] M-6 print-through: floor={sil_rms_db:.1f}dBFS '
                  f'— R-5c silence shaping will respect -65 to -50dBFS band')

        state.noise_map = noise_map

        # Summary
        if noise_map:
            from collections import Counter
            type_counts = Counter(e['type'].split('_')[0]+'-'+e['type'].split('_')[1]
                                  for e in noise_map)
            L(f'  [A.9] taxonomy: '
              + ', '.join(f'{t}×{c}' for t,c in type_counts.items()))
        else:
            L('  [A.9] no congregation noise events detected')

    except Exception as _e:
        L(f'  [A.9] congregation noise taxonomy error (non-fatal): {_e}')
...
def compute_discontinuity_score(audio: 'np.ndarray', silence_thresh_db: float,
    ...
    """
    KB 38B.2 / KB-12-10 / BUG-V13-03 fix.

    Three-tier discontinuity classification:
      flutter burst  (silent_run <= 1, immediate recovery): weight 0.60
      short gap      (silent_run 2-3):                      weight 0.30
      longer silence (silent_run > 3): likely waqf          weight 0.00
    """
    ...
    try:
        ...
        active     = rms_frames[rms_frames > silence_thresh_db - 15]
        voiced_med = float(np.median(active)) if len(active) > 0 else float(np.max(rms_frames))
        silence_gate = voiced_med - 20.0
        weighted_discontinuities = 0.0
        total_voiced = 0
        for i in range(1, n_frames - 3):
            if rms_frames[i - 1] <= silence_gate:
                continue
            total_voiced += 1
            if rms_frames[i] > silence_gate:
                continue
            silent_run = 0
            for j in range(i, min(i + 12, n_frames)):
                if rms_frames[j] < silence_gate:
                    silent_run += 1
                else:
                    break
            if silent_run <= 1:
                next_voiced = (i + silent_run < n_frames and
                               rms_frames[i + silent_run] > silence_gate)
                if next_voiced:
                    weighted_discontinuities += 0.60   # flutter burst
                else:
                    weighted_discontinuities += 0.30
            elif silent_run <= 3:
                weighted_discontinuities += 0.30       # short gap
            # > 3 = waqf-length, not an artifact
        if total_voiced == 0:
            return 0.0
...
def _try_clearvoice_speechscore(wav_path: str) -> dict:
    """
    KB-14-01: ClearerVoice-Studio SpeechScore integration (§69.3, v6 supplement).

    Attempts to use ClearerVoice-Studio's SpeechScore toolkit for DNSMOS P.835
    + NISQA v2 evaluation. Falls back gracefully if the package is absent.

    Returns a dict with keys: dnsmos_sig, dnsmos_bak, dnsmos_ovrl, nisqa
    or empty dict on failure/unavailability.

    Install: pip install clearvoice-studio --break-system-packages
    """
    try:
        from clearvoice import SpeechScore
        scorer = SpeechScore(['dnsmos', 'nisqa'])
        scores = scorer.score(wav_path)
        result = {
            'dnsmos_sig':  float(scores.get('dnsmos_sig',  0.0)),
            'dnsmos_bak':  float(scores.get('dnsmos_bak',  0.0)),
            'dnsmos_ovrl': float(scores.get('dnsmos_ovrl', 0.0)),
            'nisqa':       float(scores.get('nisqa',       0.0)),
        }
        L(f'  [KB-14-01/SpeechScore] DNSMOS SIG={result["dnsmos_sig"]:.2f} '
          f'BAK={result["dnsmos_bak"]:.2f} OVRL={result["dnsmos_ovrl"]:.2f} '
          f'NISQA={result["nisqa"]:.2f}')
        return result
    except ImportError:
        return {}   # clearvoice-studio not installed — use v13 proxy fallback
    except Exception as _e:
        L(f'  [KB-14-01/SpeechScore] failed: {_e} — using proxy fallback')
        return {}


def _clearvoice_mos_to_proxy(cv_scores: dict) -> float:
    """
    Convert ClearerVoice SpeechScore output to a MOS-equivalent [1.0-5.0].

    DNSMOS OVRL is already on a [1-5] scale calibrated to ITU-T P.808.
    NISQA v2 is also [1-5].  Average them when both are available.
    """
    candidates = []
    if cv_scores.get('dnsmos_ovrl', 0.0) > 0:
        candidates.append(cv_scores['dnsmos_ovrl'])
    if cv_scores.get('nisqa', 0.0) > 0:
        candidates.append(cv_scores['nisqa'])
    if not candidates:
        return 0.0
...
def _v13_analyze_additions(state: InputState, clip: 'np.ndarray',
                            ref: ReferenceModel) -> None:
    """KB-13-01 + KB-13-04 additions to analyze_input()."""
    # KB-13-01: Madd duration verification
    if NUMPY_OK:
        try:
            madd_clip = clip[:SR * 120] if len(clip) > SR * 120 else clip
            state.madd_events = _detect_madd_durations_inner(madd_clip, sr=SR)
            n_short = sum(1 for e in state.madd_events if e[4])  # is_short index
            if state.madd_events:
                L(f'  [KB-13-01/Madd] {len(state.madd_events)} segments, '
                  f'{n_short} short (<80% min)')
        except Exception as _ex:
            L(f'  [KB-13-01/Madd] error: {_ex}')
            state.madd_events = []

    # KB-13-04: Lightweight MOS proxy
    if NUMPY_OK:
        try:
            rms_now   = float(20.0 * np.log10(np.sqrt(np.mean(clip ** 2)) + 1e-12))
            rms_delta = abs(rms_now - ref.rms)
            sub_col   = float(np.clip(1.0 - state.spec_dist / 15.0, 0.0, 1.0))
            sub_disc  = float(np.clip(1.0 - state.discontinuity_score, 0.0, 1.0))
            sub_loud  = float(np.clip(1.0 - (rms_delta - 1.0) / 5.0, 0.0, 1.0))
            rt60_proxy = max(0.0, (state.clip_lra - state.achievable_lra) * 0.15)
            sub_rev   = float(np.clip(1.0 - (rt60_proxy - 0.15) / 1.35, 0.0, 1.0))
            sfm = state.silence_sfm
            if not state.silence_valid and state.frame_snr > 20.0:
                sub_noise = 1.0
            elif sfm < 0.10:
                sub_noise = 0.20
            elif sfm < 0.30:
                sub_noise = 0.20 + 0.50 * (sfm - 0.10) / 0.20
            elif sfm < 0.70:
                sub_noise = float(np.clip(0.70 - (sfm - 0.30) * 0.75, 0.20, 0.70))
            else:
                sub_noise = max(0.10, 0.25 - (sfm - 0.70) * 0.50)
            sub_noise = float(np.clip(
                sub_noise + float(np.clip((state.frame_snr - 8.0) / 12.0, 0.0, 0.30)),
                0.0, 1.0))
            weights = _MOS_WEIGHTS
            wsum    = sum(w * s for w, s in zip(weights,
                          (sub_col, sub_disc, sub_loud, sub_rev, sub_noise)))
            state.mos_proxy            = round(float(np.clip(1.5 + 3.5 * wsum, 1.0, 5.0)), 2)
            state.mos_sub_coloration   = round(sub_col,   3)
            state.mos_sub_discontinuity = round(sub_disc, 3)
            state.mos_sub_loudness     = round(sub_loud,  3)
            state.mos_sub_reverb       = round(sub_rev,   3)
            state.mos_sub_noise        = round(sub_noise, 3)
            L(f'  [KB-13-04/MOS] proxy={state.mos_proxy:.2f}/5.0  '
              f'col={sub_col:.2f} disc={sub_disc:.2f} '
              f'loud={sub_loud:.2f} rev={sub_rev:.2f} noise={sub_noise:.2f}')
        except Exception as _ex:
            L(f'  [KB-13-04/MOS] error: {_ex}')
            _cv = _try_clearvoice_speechscore(getattr(state, '_input_wav_path', ''))
            state.mos_proxy = _clearvoice_mos_to_proxy(_cv) if _cv else 3.0



def _build_whisper_prefix(quran_text: str) -> str:
    """
    KB-14-02 — Whisper decoder prefix for emphatic consonant protection (§73.4, v6).

    WhisperX accepts an initial_prompt argument which seeds the decoder.
    When the reference Quran text is provided as the prompt, Whisper's
    language model component strongly prefers the correct emphatic letters
    (ص ض ط ظ) over their non-emphatic counterparts (س د ت ذ).

    Usage in WhisperX forced alignment:
        result = model.transcribe(audio_path, initial_prompt=quran_prefix)

    This function normalises the input text and returns the prompt string.
    Empty string input → returns '' (no prefix, WhisperX default behaviour).

    §73.2 recommendation: supply the full ayah text of the passage being
    enhanced. Even a partial match (first 10 words) reduces emphatic errors
    by >50% in controlled tests (Talafha et al., arXiv 2511.18774).
    """
    if not quran_text:
        return ''
    # Normalise: strip leading/trailing whitespace, collapse double spaces
    import unicodedata
    normalised = ' '.join(quran_text.split())
    # NFC normalisation handles variant Unicode representations
    normalised = unicodedata.normalize('NFC', normalised)
    return normalised


def _warn_no_quran_prefix(state: 'InputState') -> None:
    """
    KB-14-02: Log a warning when Madd/QA analysis runs without a Quran prefix.

    §73.4 (v6): Whisper misclassifies emphatic consonants in noisy recordings.
    When state.source_tier in DAMAGED/CRITICAL and no prefix is configured,
    any downstream ASR-based phoneme classification is unreliable for
    ص ض ط ظ. The formant-based Madd detection (KB-13-01) is NOT affected
    (it uses acoustic features, not ASR), but any future ASR-based QA module is.
    """
    if getattr(state, 'source_tier', '') in ('TIER_DAMAGED', 'TIER_CRITICAL'):
        if not getattr(state, 'quran_prefix_set', False):
            L('  [KB-14-02/WARN] No Quran prefix configured. ASR emphatic consonant '
              'misclassification risk is HIGH for DAMAGED/CRITICAL sources. '
              'Use --quran-prefix "ayah text" to suppress this warning and improve '
              'emphatic letter recognition (see §73.4, Supplement v6).')

def _detect_madd_durations_inner(audio: 'np.ndarray', sr: int = 48000) -> list:
    """Minimal Madd detector; returns list of tuples (s_start, s_end, vowel, dur, is_short)."""
    hop_n   = max(1, int(sr * 0.010))
    frame_n = hop_n * 4
    n_frames = max(1, (len(audio) - frame_n) // hop_n)
    if n_frames < 10:
        return []
    frame_rms = np.array([
        float(20.0 * np.log10(np.sqrt(np.mean(audio[i*hop_n:i*hop_n+frame_n]**2)) + 1e-12))
        for i in range(n_frames)
    ])
    freqs   = np.fft.rfftfreq(frame_n, 1.0 / sr)
    f2_mask = (freqs >= 700.0) & (freqs <= 2500.0)
    frame_f2 = np.zeros(n_frames, dtype=np.float32)
    for i in range(n_frames):
        seg  = audio[i*hop_n: i*hop_n+frame_n]
        if len(seg) < frame_n: break
        spec = np.abs(np.fft.rfft(seg.astype(np.float64)))**2
        band = spec[f2_mask]; bf = freqs[f2_mask]
        frame_f2[i] = float(np.sum(bf * band) / (np.sum(band) + 1e-30))
    med   = float(np.percentile(frame_rms, 50))
    voiced = frame_rms > med - 12.0
    for i in range(n_frames):
        if not voiced[i]: continue
        seg = audio[i*hop_n: i*hop_n+frame_n]
        if len(seg) > 4:
            zcr = float(np.sum(np.abs(np.diff(np.sign(seg)))) / (2*max(len(seg)-1,1)))
            if zcr > 0.15: voiced[i] = False
    events = []; i = 0
    min_s = MADD_MIN_COUNTS * MADD_COUNT_SECONDS * MADD_ALERT_RATIO
    while i < n_frames:
        if not voiced[i]: i += 1; continue
        s = i
        while i < n_frames and voiced[i]: i += 1
        dur = (i - s) * 0.010
        if dur < MADD_COUNT_SECONDS * 1.5: continue
        f2 = float(np.mean(frame_f2[s:i]))
        v  = 'WAW' if f2 < 1000 else ('YA' if f2 > 1600 else 'ALIF')
        exp_min = MADD_MIN_COUNTS * MADD_COUNT_SECONDS
        events.append((s*hop_n, i*hop_n, v, round(dur, 3), dur < MADD_ALERT_RATIO*exp_min))
    return events

# ══════════════════════════════════════════════════════════════════════════════
#  PART II v16 — SECTIONS C–J: ARABIC DNA FUNCTIONS
# ══════════════════════════════════════════════════════════════════════════════

def _extract_voiced_segments(audio, sr, frame_n, hop_n,
                              min_dur_ms=100.0, energy_offset_db=-15.0):
    if not NUMPY_OK or len(audio) < sr * 0.1:
        return []
    overall_rms = rms_db(audio)
    threshold_db = overall_rms + energy_offset_db
    n_frames = max(1, (len(audio) - frame_n) // hop_n)
    min_dur_frames = max(1, int(min_dur_ms / 1000.0 * sr / hop_n))
    voiced = [rms_db(audio[i*hop_n: i*hop_n+frame_n]) > threshold_db
              for i in range(n_frames)]
    segs = []
    ...
    for i, v in enumerate(voiced):
        if v and not in_seg:
            ...
            in_seg = False
            if (i - seg_start) >= min_dur_frames:
                segs.append((seg_start * hop_n / sr, i * hop_n / sr))
    if in_seg and (len(voiced) - seg_start) >= min_dur_frames:
        segs.append((seg_start * hop_n / sr, len(voiced) * hop_n / sr))
    return segs


def _estimate_f0_variance(audio, sr, f0_min=50.0, f0_max=500.0):
    if not NUMPY_OK or len(audio) < sr * 0.1:
        return 999.0
    try:
        frame_n = len(audio)
        acf = np.correlate(audio, audio, mode='full')[frame_n - 1:]
        lag_min = max(1, int(sr / f0_max))
        lag_max = min(len(acf) - 1, int(sr / f0_min))
        if lag_min >= lag_max:
            return 999.0
        peak_lag = lag_min + int(np.argmax(acf[lag_min:lag_max + 1]))
        peak_f0  = sr / max(peak_lag, 1)
        if not (f0_min <= peak_f0 <= f0_max):
            return 999.0
        # Variance proxy: spread of sub-window autocorrelation peaks
        sub_n  = frame_n // 4
        sub_f0s = []
        for k in range(4):
            sub = audio[k * sub_n: (k + 1) * sub_n]
            if len(sub) < 64:
                continue
            sub_acf  = np.correlate(sub, sub, mode='full')[len(sub) - 1:]
            lm = max(1, int(sr / f0_max))
            lM = min(len(sub_acf) - 1, int(sr / f0_min))
            if lm >= lM:
                continue
            pk = lm + int(np.argmax(sub_acf[lm:lM + 1]))
            sub_f0s.append(sr / max(pk, 1))
        if len(sub_f0s) < 2:
            return 0.0
        f0_cents = 1200.0 * np.log2(np.array(sub_f0s) / (np.mean(sub_f0s) + 1e-6) + 1e-9)
        return float(np.std(f0_cents))
    except Exception:
        return 999.0


def _calibrate_harakah_ms(path, state):
    if not NUMPY_OK:
        return _HARAKAH_MS_DEFAULT
    try:
        dur = min(60.0, max(0.0, state.total_s - state.skip_s - 5.0))
        if dur < 15.0:
            return _HARAKAH_MS_DEFAULT
        clip = load_audio_fast(path, state.skip_s, dur)
        if len(clip) < SR * 5:
            return _HARAKAH_MS_DEFAULT
        frame_n, hop_n = int(0.020 * SR), int(0.010 * SR)
        segs = _extract_voiced_segments(clip, SR, frame_n, hop_n, min_dur_ms=200.0)
        if not segs:
            return _HARAKAH_MS_DEFAULT
        lo_ms = 250.0; hi_ms = 600.0   # Tabii Madd expected range at normal tempo
        tabii_durs = [(e - s) * 1000.0 for s, e in segs
                      if lo_ms <= (e - s) * 1000.0 <= hi_ms]
        if len(tabii_durs) < 3:
            return _HARAKAH_MS_DEFAULT
        median_dur = float(np.median(tabii_durs))
        harakah_ms = median_dur / 2.0   # Tabii Madd = 2 harakah
        if not (100.0 <= harakah_ms <= 350.0):
            return _HARAKAH_MS_DEFAULT
        return round(harakah_ms, 1)
    except Exception:
        return _HARAKAH_MS_DEFAULT


def _detect_riwaya(path, state):
    if not NUMPY_OK:
        L('  [الرواية] numpy unavailable — defaulting to HAFS')
        return ('HAFS', 0.0, _RIWAYA_MADD_TABLE['HAFS'], _RIWAYA_HAMZAH_RULES['HAFS'])
    L('  [الرواية] detecting riwaya from Madd Munfasil patterns...')
    try:
        analysis_dur = min(180.0, max(0.0, state.total_s - state.skip_s - 5.0))
        if analysis_dur < 30.0:
            return ('HAFS', 0.0, _RIWAYA_MADD_TABLE['HAFS'], _RIWAYA_HAMZAH_RULES['HAFS'])
        clip = load_audio_fast(path, state.skip_s, analysis_dur)
        if len(clip) < SR * 20:
            return ('HAFS', 0.0, _RIWAYA_MADD_TABLE['HAFS'], _RIWAYA_HAMZAH_RULES['HAFS'])
        harakah_ms = _calibrate_harakah_ms(path, state)
        state._harakah_ms_calibrated = harakah_ms
        L(f'  [الرواية] harakah_ms={harakah_ms:.0f}ms')
        frame_n, hop_n = int(0.020 * SR), int(0.010 * SR)
        segs = _extract_voiced_segments(clip, SR, frame_n, hop_n, min_dur_ms=200.0)
        if len(segs) < 5:
            state.riwaya_uncertain = True
            return ('HAFS', 0.25, _RIWAYA_MADD_TABLE['HAFS'], _RIWAYA_HAMZAH_RULES['HAFS'])
        lo_ms = harakah_ms * 3.0 * 0.7
        hi_ms = harakah_ms * 8.0 * 1.3
        munfasil_durs = []
        for (s_s, e_s) in segs:
            dur_ms = (e_s - s_s) * 1000.0
            if not (lo_ms <= dur_ms <= hi_ms):
                continue
            seg = clip[int(s_s * SR): int(e_s * SR)]
            if len(seg) < int(0.05 * SR):
                continue
            if _estimate_f0_variance(seg, SR) > 150.0:
                continue
            munfasil_durs.append(dur_ms)
        if len(munfasil_durs) < 3:
            state.riwaya_uncertain = True
            return ('HAFS', 0.25, _RIWAYA_MADD_TABLE['HAFS'], _RIWAYA_HAMZAH_RULES['HAFS'])
        median_ms = float(np.median(munfasil_durs))
        median_H  = median_ms / harakah_ms
        L(f'  [الرواية] median_munfasil={median_ms:.0f}ms = {median_H:.2f}H '
          f'({len(munfasil_durs)} candidates)')
        if median_H >= 5.0:
            riwaya = 'WARSH'
            confidence = float(np.clip(0.4 + (median_H - 5.0) / 2.0 * 0.45, 0.40, 0.88))
        elif median_H >= 3.0:
            riwaya = 'HAFS'
            confidence = float(np.clip(1.0 - abs(median_H - 4.0) / 1.5 * 0.6, 0.35, 0.88))
        else:
            riwaya = 'QALUN'
            confidence = float(np.clip(0.4 + (3.0 - median_H) / 1.5 * 0.4, 0.40, 0.80))
        uncertain = confidence < 0.45
        if uncertain:
            state.riwaya_uncertain = True
            L(f'  [الرواية] RIWAYA_UNCERTAIN — confidence={confidence:.2f}')
        L(f'  [الرواية] → {riwaya}  conf={confidence:.2f}  median_H={median_H:.2f}  uncertain={uncertain}')
        return (riwaya, confidence, _RIWAYA_MADD_TABLE[riwaya], _RIWAYA_HAMZAH_RULES[riwaya])
    except Exception as exc:
        L(f'  [الرواية] error: {exc} — defaulting to HAFS')
        return ('HAFS', 0.0, _RIWAYA_MADD_TABLE['HAFS'], _RIWAYA_HAMZAH_RULES['HAFS'])


def _classify_recitation_style(path, state):
    if not NUMPY_OK:
        return ('MURATTAL', 0.5)
    L('  [الأسلوب] classifying recitation style...')
    try:
        analysis_dur = min(120.0, max(0.0, state.total_s - state.skip_s - 5.0))
        if analysis_dur < 10.0:
            return ('MURATTAL', 0.5)
        clip = load_audio_fast(path, state.skip_s, analysis_dur)
        if len(clip) < SR * 5:
            return ('MURATTAL', 0.5)
        frame_n, hop_n = int(0.020 * SR), int(0.010 * SR)
        overall_rms = rms_db(clip)
        thresh_db   = overall_rms - 12.0
        n_frames = max(1, (len(clip) - frame_n) // hop_n)
        voiced   = np.array([rms_db(clip[i*hop_n: i*hop_n+frame_n]) > thresh_db
                              for i in range(n_frames)], dtype=bool)
        onsets         = np.where(np.diff(voiced.astype(int)) > 0)[0]
        total_voiced_s = float(np.sum(voiced) * hop_n / SR)
        syl_rate       = len(onsets) / max(total_voiced_s, 1.0)
        L(f'  [الأسلوب] syl_rate={syl_rate:.2f} voiced_s={total_voiced_s:.1f}s')
        if syl_rate > 4.5:
            conf = float(np.clip(0.40 + (syl_rate - 4.5) / 3.0 * 0.45, 0.40, 0.88))
            L(f'  [الأسلوب] → HADR (syl_rate={syl_rate:.2f}, conf={conf:.2f})')
            return ('HADR', conf)
        win_n, win_hop = int(0.500 * SR), int(0.100 * SR)
        n_wins = max(1, (len(clip) - win_n) // win_hop)
        f0_mad_list = []
        for i in range(n_wins):
            s = i * win_hop
            window = clip[s: s + win_n]
            if rms_db(window) < thresh_db:
                continue
            f0_var = _estimate_f0_variance(window, SR)
            if f0_var < 999.0:
                f0_mad_list.append(f0_var)
        mean_f0_mad = float(np.mean(f0_mad_list)) if f0_mad_list else 0.0
        segs = _extract_voiced_segments(clip, SR, frame_n, hop_n, min_dur_ms=300.0)
        inter_silences = [(segs[j][0] - segs[j-1][1]) * 1000.0
                          for j in range(1, len(segs))
                          if (segs[j][0] - segs[j-1][1]) * 1000.0 >= 100.0]
        median_silence_ms = float(np.median(inter_silences)) if inter_silences else 0.0
        L(f'  [الأسلوب] mean_F0_MAD={mean_f0_mad:.1f}c  '
          f'median_silence={median_silence_ms:.0f}ms')
        if mean_f0_mad > 80.0 and median_silence_ms > 800.0:
            conf = float(np.clip(0.35 + (mean_f0_mad - 80.0)/100.0 + (median_silence_ms - 800.0)/1500.0,
                                  0.35, 0.90))
            L(f'  [الأسلوب] → MUJAWWAD  conf={conf:.2f}')
            return ('MUJAWWAD', conf)
        L('  [الأسلوب] → MURATTAL (default)')
        return ('MURATTAL', 0.75)
    except Exception as exc:
        L(f'  [الأسلوب] error: {exc} — defaulting to MURATTAL')
        return ('MURATTAL', 0.5)


def _apply_style_overrides(state):
    sc = state.style_class
    state.style_deepfilter_atten_lim = _STYLE_DEEPFILTER_ATTEN_LIM.get(sc, 18)
    state.style_afftdn_nr_max        = _STYLE_AFFTDN_NR_MAX.get(sc, 0.03)
    L(f'  [الأسلوب] overrides: atten_lim={state.style_deepfilter_atten_lim}dB  '
      f'afftdn_max={state.style_afftdn_nr_max:.3f}  style={sc}')
    if sc == 'MUJAWWAD':
        L('  [الأسلوب] MUJAWWAD: J-1 shimmer DISABLED — J-4 widener DISABLED')


def _detect_ornament_frames(clip, sr):
    if not NUMPY_OK or len(clip) < sr * 0.5:
        return []
    try:
        frame_n, hop_n = int(0.100 * sr), int(0.050 * sr)
        n_frames = max(1, (len(clip) - frame_n) // hop_n)
        raw_frames = []
        for i in range(n_frames):
            s, frame = i * hop_n, clip[i*hop_n: i*hop_n+frame_n]
            if rms_db(frame) < -48.0:
                continue
            if _estimate_f0_variance(frame, sr) > _ORNAMENT_F0_VARIANCE_CENTS:
                raw_frames.append((s/sr*1000.0, (s+frame_n)/sr*1000.0))
        if not raw_frames:
            return []
        merged = [list(raw_frames[0])]
        for (s, e) in raw_frames[1:]:
            if s - merged[-1][1] <= 50.0:
                merged[-1][1] = e
            else:
                merged.append([s, e])
...
def _detect_amplitude_drift(path, state):
    if not NUMPY_OK or state.total_s < 900.0:
        return (False, 0.0, False)
    try:
        n_wins = max(6, int(state.total_s / 60.0))
        positions = [state.skip_s + (state.total_s - state.skip_s) * i / n_wins
                     for i in range(n_wins)]
        lufs_samples = []
        for pos in positions:
            lv = measure_lufs_clip_approx(path, pos, 10.0) if hasattr(_, 'measure_lufs_clip_approx') else None
            if lv is None:
                clip_s = load_audio_fast(path, float(pos), 10.0)
                if len(clip_s) > SR * 2:
                    lv = float(rms_db(clip_s))
            if lv and lv > -90:
                lufs_samples.append((float(pos), float(lv)))
        if len(lufs_samples) < 4:
            return (False, 0.0, False)
        lvls = np.array([l for _, l in lufs_samples])
        lra_range = float(np.percentile(lvls, 90) - np.percentile(lvls, 10))
        max_jump = float(np.max(np.abs(np.diff(lvls)))) if len(lvls) > 1 else 0.0
        splice = max_jump > 4.0
        drift  = lra_range > 12.0
        L(f'  [الاستمرار] lra_range={lra_range:.1f}LU  max_jump={max_jump:.1f}LU  '
          f'drift={drift}  splice={splice}')
        return (drift, lra_range, splice)
    except Exception as exc:
        L(f'  [الاستمرار] drift detection error: {exc}')
        return (False, 0.0, False)


def _correct_amplitude_drift(input_wav, output_wav, state):
    if not NUMPY_OK:
        return input_wav
    try:
        n_pts = max(6, int(state.total_s / 60.0))
        positions = np.linspace(state.skip_s + 5.0, state.total_s - 15.0, n_pts)
        samples = []
        for pos in positions:
            clip_s = load_audio_fast(input_wav, float(pos), 10.0)
            if len(clip_s) > SR * 2:
                lv = float(rms_db(clip_s))
                if lv > -90:
                    samples.append((float(pos), lv))
        if len(samples) < 4:
            return input_wav
        from scipy.interpolate import CubicSpline as _CS
        t_pts = np.array([p for p, _ in samples])
        l_pts = np.array([l for _, l in samples])
        cs           = _CS(t_pts, l_pts)
        target_lufs  = float(np.median(l_pts))
        r = subprocess.run(['ffmpeg', '-y', '-i', input_wav,
        ...
        n_samp = len(audio)
        t_audio    = np.linspace(0.0, state.total_s, n_samp)
        t_clipped  = np.clip(t_audio, t_pts[0], t_pts[-1])
        correction = target_lufs - cs(t_clipped)
        max_rate   = 0.5 / SR
        for i in range(1, len(correction)):
            correction[i] = correction[i-1] + float(
                np.clip(correction[i] - correction[i-1], -max_rate, max_rate))
        gain_lin   = 10.0 ** (correction.astype(np.float64) / 20.0)
        audio_out  = (audio.astype(np.float64) * gain_lin).astype(np.float32)
        tmp = os.path.join(_TMP, 'istimrar_amp_corrected.wav')
        r2  = subprocess.run(['ffmpeg', '-y', '-f', 'f32le', '-ar', str(SR), '-ac', '1',
                               '-i', '-', '-ar', str(SR), '-ac', '2',
                               '-c:a', WAV_CODEC, '-loglevel', 'error', tmp],
                              input=audio_out.tobytes(), capture_output=True)
        if r2.returncode == 0 and os.path.exists(tmp):
            state.amplitude_drift_corrected = True
            L(f'  [الاستمرار] amplitude drift corrected: '
              f'max_corr={float(np.max(np.abs(correction))):.1f}dB')
            return tmp
        return input_wav
    except Exception as exc:
        L(f'  [الاستمرار] correction error: {exc}')
        return input_wav


def _correct_spectral_tilt_drift(input_wav, path, state):
    if not NUMPY_OK or state.total_s < 900.0:
        return input_wav
    try:
        win_s = 30.0
        n_wins = max(4, int(state.total_s / win_s) - 1)
        positions = np.linspace(15.0, state.total_s - 40.0, n_wins)
        tilt_samples = []
        for pos in positions:
            clip_s = load_audio_fast(path, float(pos), win_s)
            if len(clip_s) < int(win_s * SR * 0.5):
                continue
            spec = third_octave(clip_s)
            if not spec:
                continue
            hf = [spec[fc] for fc in spec if 4000.0 <= fc <= 8000.0]
            mf = [spec[fc] for fc in spec if 500.0  <= fc <= 1000.0]
            if hf and mf:
                tilt_samples.append((float(pos), float(np.mean(hf)) - float(np.mean(mf))))
        if len(tilt_samples) < 4:
            return input_wav
        t_pts = np.array([p for p, _ in tilt_samples])
        r_pts = np.array([r for _, r in tilt_samples])
        coeffs          = np.polyfit(t_pts, r_pts, 1)
        slope_db_per_s  = float(coeffs[0])
        slope_db_per_min = slope_db_per_s * 60.0
        state.spectral_tilt_drift_dB_per_min = round(slope_db_per_min, 3)
        L(f'  [الاستمرار] spectral tilt drift={slope_db_per_min:.2f}dB/min')
        if abs(slope_db_per_min) < 0.5:
            return input_wav
        total_drift   = slope_db_per_min * (state.total_s / 60.0)
        correction_db = float(np.clip(-total_drift / 2.0, -4.0, 4.0))
        tmp = os.path.join(_TMP, 'istimrar_tilt_corrected.wav')
        af  = f'treble=g={correction_db:.2f}:f=4000:t=q:w=0.7'
        ok  = ffmpeg_process(input_wav, tmp, af)
        if ok and os.path.exists(tmp):
            state.tilt_correction_applied = True
            L(f'  [الاستمرار] tilt correction: {correction_db:+.2f}dB @4kHz')
            return tmp
        return input_wav
    except Exception as exc:
        L(f'  [الاستمرار] tilt error: {exc}')
        return input_wav


def _detect_madd_durations_v2(clip, sr, state):
    if not NUMPY_OK or len(clip) < sr * 2:
        return []
    try:
        madd_table  = getattr(state, 'riwaya_madd_table', _RIWAYA_MADD_TABLE['HAFS'])
        harakah_ms  = getattr(state, '_harakah_ms_calibrated', _HARAKAH_MS_DEFAULT)
        style_class = getattr(state, 'style_class', 'MURATTAL')
        frame_n, hop_n = int(0.020 * sr), int(0.010 * sr)
        segs = _extract_voiced_segments(clip, sr, frame_n, hop_n, min_dur_ms=150.0)
        lo_ms = harakah_ms * 2.0 * 0.80
        hi_ms = harakah_ms * 6.0 * 1.20
        events = []
        for (s_s, e_s) in segs:
            dur_ms = (e_s - s_s) * 1000.0
            if not (lo_ms <= dur_ms <= hi_ms):
                continue
            dur_H    = dur_ms / harakah_ms
            tabii_H  = madd_table.get('tabii', 2)
            munf_H   = madd_table.get('munfasil', 4)
            if isinstance(munf_H, tuple):
                munf_H = munf_H[0]
            lazim_H  = madd_table.get('lazim', 6)
            if dur_H < tabii_H * 0.90:
                madd_type, flag = 'UNKNOWN', 'MADD_INSUFFICIENT'
            elif dur_H < munf_H * 0.90:
                madd_type, flag = 'TABII', 'MADD_OK'
            elif dur_H < lazim_H * 0.90:
                madd_type, flag = 'MUNFASIL', 'MADD_OK'
            else:
                madd_type, flag = 'LAZIM', 'MADD_OK'
            events.append({'start_ms': round(s_s * 1000), 'end_ms': round(e_s * 1000),
                           'duration_ms': round(dur_ms), 'harakah_count': round(dur_H, 2),
                           'madd_type': madd_type, 'flag': flag,
                           'confidence': 0.7, 'threshold_H': tabii_H})
        return events
    except Exception:
        return []


def _detect_language_segments(clip, sr, state):
    if not NUMPY_OK or len(clip) < sr * 5:
        return []
    try:
        # Heuristic: Arabic speech has characteristic F0 stability + formant pattern
        # Non-Arabic commentary: ZCR difference + HF energy difference
        # Simplified for v17: flag as multilingual if ZCR bimodal distribution
        frame_n, hop_n = int(0.100 * sr), int(0.050 * sr)
        n_frames = max(1, (len(clip) - frame_n) // hop_n)
        zcrs = []
        for i in range(n_frames):
            frame = clip[i*hop_n: i*hop_n+frame_n]
            if rms_db(frame) > rms_db(clip) - 12.0:
                zcrs.append(float(np.mean(np.abs(np.diff(np.sign(frame)))) / 2.0))
        if not zcrs:
            return []
        zcr_arr = np.array(zcrs)
        # Bimodal check: if std > 0.15, likely mixed content
        if float(np.std(zcr_arr)) > 0.15:
            state.has_multilingual_content = True
            L(f'  [التعدد] ZCR bimodal (std={float(np.std(zcr_arr)):.3f}) — '
              f'possible multilingual content detected')
        return []
    except Exception:
        return []


def _v16_analyze_additions(state, path, ref):
    """Phase A.7–A.12: All v16 Arabic DNA analysis additions."""
    if not NUMPY_OK:
        return

    # A.7: الرواية — Riwaya detection
    L('  [A.7] الرواية detection...')
    _chk_a = lambda: None  # Phase check not available here; use try/except
    try:
        (state.riwaya_detected,
         state.riwaya_confidence,
         state.riwaya_madd_table,
         state.riwaya_hamzah_rules) = _detect_riwaya(path, state)
        L(f'  [A.7] riwaya={state.riwaya_detected}  '
          f'conf={state.riwaya_confidence:.2f}  '
          f'uncertain={state.riwaya_uncertain}')
    except Exception as exc:
        L(f'  [A.7] الرواية error: {exc}')

    # A.10: الأسلوب — Recitation style classification
    L('  [A.10] الأسلوب detection...')
    try:
        state.style_class, state.style_confidence = _classify_recitation_style(path, state)
        _apply_style_overrides(state)
    except Exception as exc:
        L(f'  [A.10] الأسلوب error: {exc}')

    # A.10 extension: مقام الصون — Ornament frame mask (for MUJAWWAD)
    if state.style_class == 'MUJAWWAD':
        try:
            clip_orn = load_audio_fast(path, state.skip_s, min(state.dur_s, 60.0))
            if len(clip_orn) > SR * 5:
                state.ornament_frame_mask    = _detect_ornament_frames(clip_orn, SR)
                state.ornament_frames_detected = len(state.ornament_frame_mask)
                L(f'  [A.10/مقام الصون] ornament_frames={state.ornament_frames_detected}')
        except Exception as exc:
            L(f'  [A.10/مقام] error: {exc}')

    # A.11: الاستمرار — Long-form drift detection
    if state.total_s >= 900.0:
        try:
            L('  [A.11] الاستمرار drift detection...')
            (state.amplitude_drift_detected,
             state.amplitude_drift_range_LU,
             state.amplitude_splice_detected) = _detect_amplitude_drift(path, state)
        except Exception as exc:
            L(f'  [A.11] drift error: {exc}')

    # A.12: التعدد — Multilingual segment detection
    try:
        clip_ml = load_audio_fast(path, state.skip_s, min(state.dur_s, 30.0))
        if len(clip_ml) > SR * 5:
            state.language_segment_map = _detect_language_segments(clip_ml, SR, state)
            L(f'  [A.12] multilingual={state.has_multilingual_content}')
    except Exception as exc:
        L(f'  [A.12] language detection error: {exc}')

    # الماد v2 — Upgraded Madd measurement
    try:
        clip_madd = load_audio_fast(path, state.skip_s, min(state.dur_s, 60.0))
        if len(clip_madd) > SR * 5:
            state.madd_events_v2 = _detect_madd_durations_v2(clip_madd, SR, state)
            insuff = sum(1 for e in state.madd_events_v2 if e.get('flag') == 'MADD_INSUFFICIENT')
            L(f'  [الماد v2] events={len(state.madd_events_v2)}  '
              f'insufficient={insuff}  '
              f'riwaya={state.riwaya_detected}')
    except Exception as exc:
        L(f'  [الماد v2] error: {exc}')


# ══════════════════════════════════════════════════════════════════════════════
#  PART III v17 — GENERATIVE LAYER FUNCTIONS
# ══════════════════════════════════════════════════════════════════════════════
...
def _init_silero_vad():
    ...
    if not _SHERPA_OK:
        return False
    try:
        config = _sherpa_onnx.VadModelConfig()     # type: ignore
        config.sample_rate = SR
        config.num_threads = 2
        _SILERO_VAD_MODEL = _sherpa_onnx.VoiceActivityDetector(  # type: ignore
...
def _silero_vad(audio, sr=SR, threshold=0.5, min_speech_s=0.25, min_silence_s=0.1):
    """[P3-01] Silero VAD. Returns [(start_s, end_s, confidence)]. Falls back to energy VAD."""
    if _init_silero_vad() and _SILERO_VAD_MODEL is not None and NUMPY_OK:
        try:
            from scipy.signal import resample_poly as _rsp
            audio_16k  = _rsp(audio, 1, SR // 16000).astype(np.float32) if SR != 16000 else audio.astype(np.float32)
            segments   = []
            chunk_n    = 16000 * 30
            for chunk_start in range(0, len(audio_16k), chunk_n):
                chunk = audio_16k[chunk_start: chunk_start + chunk_n]
                _SILERO_VAD_MODEL.accept_waveform(chunk)  # type: ignore
                while not _SILERO_VAD_MODEL.empty():       # type: ignore
                    speech = _SILERO_VAD_MODEL.front()     # type: ignore
                    _SILERO_VAD_MODEL.pop()                # type: ignore
                    start_s = (chunk_start / 16000.0) + speech.start
                    end_s   = (chunk_start / 16000.0) + speech.end
                    conf    = float(getattr(speech, 'confidence', threshold))
                    segments.append((start_s, end_s, conf))
            if segments:
                return segments
        except Exception:
            pass
    return _energy_vad_fallback(audio, sr, threshold, min_speech_s, min_silence_s)


def _energy_vad_fallback(audio, sr=SR, threshold=0.5, min_speech_s=0.25, min_silence_s=0.1):
    """Energy-based VAD fallback (original engine approach)."""
    if not NUMPY_OK or len(audio) < sr // 10:
        return [(0.0, len(audio) / sr, 0.8)]
    frame_n = int(0.02 * sr)
    frames_rms = np.array([float(np.sqrt(np.mean(audio[i:i+frame_n]**2) + 1e-15))
                            for i in range(0, len(audio) - frame_n, frame_n)])
    if len(frames_rms) < 2:
        return [(0.0, len(audio) / sr, 0.8)]
    rms_thr  = float(np.percentile(frames_rms, 25)) * 3.0
    voiced   = frames_rms > rms_thr
    min_fspe = max(1, int(min_speech_s / 0.02))
    min_fsil = max(1, int(min_silence_s / 0.02))
    segs, in_speech, seg_start, silence_count = [], False, 0, 0
    for i, v in enumerate(voiced):
        if v:
            if not in_speech:
            ...
            if in_speech:
                silence_count += 1
                if silence_count >= min_fsil:
                    seg_len = i - silence_count - seg_start
                    if seg_len >= min_fspe:
                        segs.append((seg_start * 0.02, (i - silence_count) * 0.02, 0.7))
                    in_speech, silence_count = False, 0
    if in_speech and (len(voiced) - seg_start) >= min_fspe:
        segs.append((seg_start * 0.02, len(voiced) * 0.02, 0.7))
    return segs if segs else [(0.0, len(audio) / sr, 0.8)]


def _vad_segments_to_mask(segments, n_samples, sr=SR):
    """Convert [(start_s, end_s, conf)] to boolean voiced mask."""
    if not NUMPY_OK:
        return None
    mask = np.zeros(n_samples, dtype=bool)
    for start_s, end_s, conf in segments:
        s = min(int(start_s * sr), n_samples - 1)
        e = min(int(end_s   * sr), n_samples)
        mask[s:e] = True
    return mask


def _nara_wpe_derev(wav_path, state, ref, log_fn=None):
    """[P3-02] nara_wpe WPE dereverberation with Arabic Tajweed guards."""
    def _log(msg):
        if log_fn: log_fn(msg)

    if not _NARA_WPE_OK:
        _log('  [WPE] nara_wpe not installed — bypass (pip install nara_wpe)')
        return wav_path

    if not NUMPY_OK:
        _log('  [WPE] numpy unavailable — bypass')
        return wav_path

    # Only useful for reverberant sources
    if state.source_tier in ('TIER_PRISTINE', 'TIER_CLEAN'):
        lra_deficit = getattr(ref, 'lra', 4.19) - state.clip_lra
        if lra_deficit < 0.5:
            _log(f'  [WPE] source clean — skip (lra_deficit={lra_deficit:.2f})')
            return wav_path

    _log('  [WPE] running nara_wpe (taps=10, delay=3, iterations=3)')
    tmp_out = os.path.join(_TMP, f'wpe_out_{os.getpid()}.wav')

    try:
        from scipy.io import wavfile as _swf_wpe
        cmd = ['ffmpeg', '-y', '-i', wav_path,
               '-af', 'aformat=channel_layouts=mono',
               '-f', 'f32le', '-ar', str(SR), '-loglevel', 'error', '-']
        r = subprocess.run(cmd, capture_output=True)
        if not r.stdout:
            _log('  [WPE] load failed — bypass')
            return wav_path
        ...
        peak_a  = float(np.max(np.abs(audio_wpe)) + 1e-10)
        audio_wpe *= (peak_b / peak_a)

        # Guard 1: LRA not reduced > 0.5 LU
        lra_a  = _lra_est(audio_wpe)
        lra_delta = lra_b - lra_a
        ...
        if lra_delta > 0.5:
            ...
            if ld2 > 0.5:
                _log('  [WPE] Guard 1 FAIL — REVERTED')
                state.wpe_lra_guard_fired = True
                return wav_path
            audio_wpe = a2

        # Guard 2: spectral flux not increased > 2dB
        def _flux(a):
            fn = int(0.02 * SR)
            specs = [np.abs(np.fft.rfft(a[i:i+fn] * np.hanning(fn))) + 1e-15
                     for i in range(0, len(a) - fn, fn)]
            if len(specs) < 2: return 0.0
            return float(np.mean([np.mean(np.abs(20*np.log10(specs[i+1]) - 20*np.log10(specs[i])))
                                   for i in range(len(specs)-1)]))
        flux_b, flux_a = _flux(audio), _flux(audio_wpe)
        _log(f'  [WPE] spectral_flux Δ={flux_a - flux_b:.2f}dB (limit=2.0)')
        if flux_a - flux_b > 2.0:
            _log('  [WPE] Guard 2 FAIL (Idgham smoothness) — REVERTED')
            state.wpe_idgham_guard_fired = True
            return wav_path

        # Guard 3: Madd proxy shortening ≤ 3%
        from hakim_gen_v2 import _detect_sustained_vowels_ms
        vb = _detect_sustained_vowels_ms(audio)
        va = _detect_sustained_vowels_ms(audio_wpe)
        if vb and va:
            med_b, med_a = float(np.median(vb[:5])), float(np.median(va[:5]))
            if med_b > 1.0:
                madd_d = max(0.0, (med_b - med_a) / med_b)
                _log(f'  [WPE] Madd proxy {med_b:.0f}→{med_a:.0f}ms Δ={madd_d:.3f} (limit=0.03)')
                if madd_d > 0.03:
                    _log('  [WPE] Guard 3 FAIL (Madd shortening) — REVERTED')
                    state.wpe_madd_guard_fired = True
                    return wav_path

        _swf_wpe.write(tmp_out, SR, audio_wpe)
        if not os.path.exists(tmp_out):
            return wav_path

        state.wpe_applied = True
        state.wpe_rt60_reduction_s = max(0.0, lra_delta * 0.5)
        _log(f'  [WPE] ✓ accepted — LRA Δ={lra_b - lra_a:+.2f}LU  '
             f'est_RT60_reduction≈{state.wpe_rt60_reduction_s:.2f}s')
        return tmp_out

    except Exception as exc:
        _log(f'  [WPE] exception: {exc} — bypass')
        return wav_path


def _apply_pyroomacoustics_sadaa(wav_path, state, ref, log_fn=None):
    """[P3-03] Physics-based RIR Sadaa via pyroomacoustics. Falls back to aecho."""
    def _log(msg):
        if log_fn: log_fn(msg)

    if not _PYROOM_OK or not NUMPY_OK:
        _log('  [Sadaa/pyroomacoustics] not available — using aecho chain')
        return wav_path   # caller handles aecho fallback

    if state.source_tier not in ('TIER_PRISTINE', 'TIER_CLEAN'):
        return wav_path   # damaged sources: aecho fallback in caller

    _log('  [Sadaa/pyroomacoustics] building Shoebox RIR...')
    try:
        from scipy.signal import fftconvolve as _fftconv
        from scipy.io import wavfile as _swf_room

        room_dim  = [40.0, 25.0, 15.0]
        absorption = 0.017

        materials = _pra.Material(absorption)       # type: ignore
        _, max_order = _pra.inverse_sabine(2.2, room_dim)  # type: ignore
        room = _pra.ShoeBox(                        # type: ignore
            room_dim, fs=SR, materials=materials,
            max_order=min(max_order, 3),
            ray_tracing=False, air_absorption=True,
        )
        room.add_source([room_dim[0]*0.5, room_dim[1]*0.3, 1.8])
        room.add_microphone(np.array([[room_dim[0]*0.5], [room_dim[1]*0.6], [1.2]]))
        room.compute_rir()

        rir = room.rir[0][0]
        if len(rir) < 1:
            raise ValueError('Empty RIR')

        rir_early = rir[:int(0.080 * SR)].astype(np.float32)
        rir_early /= (float(np.max(np.abs(rir_early))) + 1e-10)
        ...
        audio       = np.frombuffer(r.stdout, dtype=np.float32).copy()
        sadaa_wet   = _fftconv(audio, rir_early, mode='full')[:len(audio)]
        audio_sadaa = (audio + 0.20 * sadaa_wet.astype(np.float32)).astype(np.float32)
        audio_sadaa *= (float(np.max(np.abs(audio))) + 1e-10) / (float(np.max(np.abs(audio_sadaa))) + 1e-10)

        tmp_out = os.path.join(_TMP, f'sadaa_pyroom_{os.getpid()}.wav')
        _swf_room.write(tmp_out, SR, audio_sadaa)

        if os.path.exists(tmp_out):
            state.sadaa_pyroomacoustics = True
            state.sadaa_rir_rt60        = 2.2
            _log(f'  [Sadaa/pyroomacoustics] ✓  RIR=80ms  mix=0.20 (−14dB)  RT60≈2.2s')
            return tmp_out

    except Exception as exc:
        _log(f'  [Sadaa/pyroomacoustics] exception: {exc} — falling back to aecho')

    return wav_path  # caller handles aecho fallback


# ══════════════════════════════════════════════════════════════════════════════
#  PART IV v20 — SECTIONS C–J: ALL NEW FUNCTIONS
# ══════════════════════════════════════════════════════════════════════════════

# ── Section C: QARA 7-dimension scorer ───────────────────────────────────────
def _score_qara(output_wav: str, state: 'InputState',
                ref: 'ReferenceModel') -> 'QARAReport':
    """QARA: measure 7-dimension Tajweed quality on an already-processed WAV."""
    r = QARAReport()
    r.madd_riwaya    = getattr(state, 'riwaya_detected', 'HAFS')
    r.madd_harakah_ms = getattr(state, '_harakah_ms_calibrated', 200.0)

    # D1: الماد — riwaya-correct Madd events
    try:
        if NUMPY_OK:
            clip = load_audio_fast(output_wav, state.skip_s, min(state.dur_s, 60.0))
            if len(clip) >= SR * 5:
                events = _detect_madd_durations_v2(clip, SR, state)
                r.madd_total_n        = len(events)
                r.madd_insufficient_n = sum(1 for e in events if e.get('flag') == 'MADD_INSUFFICIENT')
                r.madd_ok_n           = r.madd_total_n - r.madd_insufficient_n
                if r.madd_total_n > 0:
                    ok_frac      = r.madd_ok_n / r.madd_total_n
                    r.madd_score = round(float(np.clip(ok_frac * 100.0 +
                        (0.0 if ok_frac >= 0.80 else -20.0 * (0.80 - ok_frac)), 0.0, 100.0)), 1)
                else:
                    r.madd_score = 75.0
    except Exception:
        r.madd_score = 75.0
    r.madd_pass = r.madd_score >= _QARA_PASS_THRESHOLD

    # D2: الغنة — 250Hz nasal pole relative to voiced peak
    try:
        if NUMPY_OK:
            clip = load_audio_fast(output_wav, state.skip_s, min(state.dur_s, 60.0))
            if len(clip) >= SR * 5:
                spec = third_octave(clip)
                if spec:
                    voiced_vals = [spec[fc] for fc in spec if 200 <= fc <= 1000]
                    val_250     = spec.get(250, spec.get(315, None))
                    if voiced_vals and val_250 is not None:
                        voiced_peak          = float(max(voiced_vals))
                        relative_250         = float(val_250) - voiced_peak
                        r.ghunnah_250hz_db   = relative_250
                        delta                = relative_250 - _QARA_GHUNNAH_REF_RELATIVE_DB
                        r.ghunnah_delta_db   = delta
                        if delta >= 0:
                            r.ghunnah_score  = 100.0
                        elif abs(delta) <= _QARA_GHUNNAH_PASS_DELTA_DB:
                            r.ghunnah_score  = max(0.0, 100.0 - (abs(delta) / _QARA_GHUNNAH_PASS_DELTA_DB) * 25.0)
                        else:
                            r.ghunnah_score  = max(0.0, 75.0 - (abs(delta) - _QARA_GHUNNAH_PASS_DELTA_DB) * 10.0)
                    else:
                        r.ghunnah_score = 75.0
    except Exception:
        r.ghunnah_score = 75.0
    r.ghunnah_pass = r.ghunnah_score >= _QARA_PASS_THRESHOLD

    # D3: القلقلة — stop burst preservation
    try:
        if NUMPY_OK:
            clip = load_audio_fast(output_wav, state.skip_s, min(state.dur_s, 60.0))
            if len(clip) >= SR * 5:
                frame_n, hop_n = int(0.020 * SR), int(0.010 * SR)
                segs = _extract_voiced_segments(clip, SR, frame_n, hop_n, min_dur_ms=30.0)
                detected = intact = 0
                for (s_s, e_s) in segs:
                    dur_ms = (e_s - s_s) * 1000.0
                    if not (20.0 <= dur_ms <= 80.0):
                        continue
                    detected += 1
                    b_start = int(e_s * SR)
                    b_end   = b_start + int(_QARA_QALQALAH_WINDOW_MS / 1000.0 * SR)
                    if b_end > len(clip):
                        continue
                    if rms_db(clip[b_start:b_end]) > _QARA_QALQALAH_BURST_FLOOR_DB:
                        intact += 1
                r.qalqalah_detected_n = detected
                r.qalqalah_intact_n   = intact
                if detected > 0:
                    r.qalqalah_score = round(intact / detected * 100.0, 1)
                else:
                    r.qalqalah_score = 80.0
    except Exception:
        r.qalqalah_score = 75.0
    r.qalqalah_pass = r.qalqalah_score >= _QARA_PASS_THRESHOLD

    # D4: الإطباق — emphatic consonant preservation
    emphatic_delta = getattr(state, 'hakim_emphatic_delta', None)
    if emphatic_delta is not None:
        r.emphatic_delta_db = float(emphatic_delta)
        if r.emphatic_delta_db >= 0:
            r.emphatic_score = 100.0
        elif abs(r.emphatic_delta_db) <= _QARA_EMPHATIC_MAX_DELTA_DB:
            r.emphatic_score = max(0.0, 100.0 - abs(r.emphatic_delta_db) / _QARA_EMPHATIC_MAX_DELTA_DB * 25.0)
        else:
            r.emphatic_score = max(0.0, 75.0 - (abs(r.emphatic_delta_db) - _QARA_EMPHATIC_MAX_DELTA_DB) * 15.0)
    else:
        r.emphatic_score = 80.0
    r.emphatic_pass = r.emphatic_score >= _QARA_PASS_THRESHOLD

    # D5: الصفير — sibilant SNR + centroid
    sib_snr_val = getattr(state, 'sib_snr', None)
    if sib_snr_val is None:
        try:
            clip = load_audio_fast(output_wav, state.skip_s, min(state.dur_s, 60.0))
            sib_snr_val = float(compute_sibilant_snr(clip, state.silence_floor))
        except Exception:
            sib_snr_val = 0.0
    r.sibilant_snr_db = round(float(sib_snr_val), 2)
    try:
        if NUMPY_OK:
            clip = load_audio_fast(output_wav, state.skip_s, min(state.dur_s, 60.0))
            if len(clip) >= SR * 3:
                from scipy.signal import stft as _stft_q
                _, _, Zxx = _stft_q(clip.astype(np.float32), fs=SR, nperseg=2048, noverlap=1536)
                freqs_q   = np.fft.rfftfreq(2048, 1.0/SR)
                sib_m     = (freqs_q >= 2000.0) & (freqs_q <= 12000.0)
                sib_mag   = np.abs(Zxx[sib_m, :]).mean(axis=1) + 1e-12
                r.sibilant_centroid_hz = float((freqs_q[sib_m] * sib_mag).sum() / sib_mag.sum())
    except Exception:
        r.sibilant_centroid_hz = _QARA_SIB_CENTROID_TARGET_HZ
    snr_sc = float(np.clip(r.sibilant_snr_db / _QARA_SIB_SNR_PASS_DB * 80.0, 0.0, 80.0))
    if r.sibilant_centroid_hz > 0:
        cen_d  = abs(r.sibilant_centroid_hz - _QARA_SIB_CENTROID_TARGET_HZ)
        cen_sc = float(np.clip(20.0 * (1.0 - cen_d / (_QARA_SIB_CENTROID_TOL_HZ * 2.0)), 0.0, 20.0))
    else:
        cen_sc = 10.0
    r.sibilant_score = round(snr_sc + cen_sc, 1)
    r.sibilant_pass  = r.sibilant_score >= _QARA_PASS_THRESHOLD

    # D6: المد الصوتي — voice DNA / LUFS convergence
    r.nidaa_applied    = getattr(state, 'nidaa_applied', False)
    r.nidaa_delta_lufs = getattr(state, 'nidaa_delta_lufs', 0.0)
    try:
        lufs_now      = float(measure_lufs(output_wav))
        gap_lufs      = abs(lufs_now - TARGET['lufs'])
        r.voice_dna_score = round(float(np.clip(100.0 - gap_lufs * 16.7, 0.0, 100.0)), 1)
    except Exception:
        r.voice_dna_score = 70.0
    r.voice_dna_pass = r.voice_dna_score >= _QARA_PASS_THRESHOLD

    # D7: السلامة العامة — SNR + discontinuity
    r.health_snr_db        = float(getattr(state, 'snr_global', 0.0))
    r.health_discontinuity = float(getattr(state, 'discontinuity_score', 0.0))
    snr_h  = float(np.clip(r.health_snr_db / 30.0 * 100.0, 0.0, 100.0))
    disc_h = float(np.clip(100.0 - r.health_discontinuity * 333.0, 0.0, 100.0))
    r.health_score = round((snr_h + disc_h) / 2.0, 1)
    r.health_pass  = r.health_score >= _QARA_PASS_THRESHOLD

    # Composite
    dim_scores = {'madd': r.madd_score, 'ghunnah': r.ghunnah_score,
                  'qalqalah': r.qalqalah_score, 'emphatic': r.emphatic_score,
                  'sibilant': r.sibilant_score, 'voice_dna': r.voice_dna_score,
                  'health': r.health_score}
    dim_passes = {'madd': r.madd_pass, 'ghunnah': r.ghunnah_pass,
                  'qalqalah': r.qalqalah_pass, 'emphatic': r.emphatic_pass,
                  'sibilant': r.sibilant_pass, 'voice_dna': r.voice_dna_pass,
                  'health': r.health_pass}
    r.composite_score   = round(float(sum(dim_scores[k] * _QARA_WEIGHTS[k] for k in _QARA_WEIGHTS)), 1)
    r.dimensions_passed = sum(1 for v in dim_passes.values() if v)
    r.composite_pass    = all(dim_passes.values())
    r.overall_grade     = r.grade_from_score(r.composite_score)
    r.fail_reasons      = [k for k, v in dim_passes.items() if not v]
    return r


# ── Section D: Ornament-frame-aware chunked DeepFilter (GAP-1) ───────────────
def _chunked_deepfilter_ornament(input_wav: str, state: 'InputState') -> str:
    """Per-frame ornament gating for MUJAWWAD: chunks DF3 at ornament boundaries."""
    mask      = getattr(state, 'ornament_frame_mask', [])
    style     = getattr(state, 'style_class', 'MURATTAL')
    atten_std = getattr(state, 'style_deepfilter_atten_lim', 18)

    if style != 'MUJAWWAD' or not mask or not NUMPY_OK:
        return _run_deepfilter3_adaptive(input_wav, state)

    try:
        import uuid as _uuid_orn
        total_s     = state.total_s
        orn_sorted  = sorted(mask, key=lambda x: x[0])
        segments    = []
        prev_end    = 0.0

        for (s_ms, e_ms) in orn_sorted:
            s_s, e_s = s_ms / 1000.0, e_ms / 1000.0
            if s_s > prev_end + 0.01:
                segments.append((prev_end, s_s, False))
            segments.append((s_s, e_s, True))
            prev_end = e_s
        if prev_end < total_s:
            segments.append((prev_end, total_s, False))

        # Merge short segments
        merged = []
        for seg in segments:
            if merged and (seg[1] - seg[0]) < 0.050:
                prev = merged[-1]
        ...
        L(f'  [ornament_gate] {len(segments)} chunks: '
          f'{sum(1 for s in segments if s[2])} ornament + '
          f'{sum(1 for s in segments if not s[2])} normal')

        chunk_wavs = []
        for (s_s, e_s, is_orn) in segments:
            chunk_dur = e_s - s_s
            if chunk_dur < 0.05:
                continue
            cid      = _uuid_orn.uuid4().hex[:8]
            cin      = os.path.join(_TMP, f'orn_c_{cid}_in.wav')
            r_ex = subprocess.run(
                ['ffmpeg', '-y', '-i', input_wav,
                 '-ss', f'{s_s:.6f}', '-t', f'{chunk_dur:.6f}',
                 '-c:a', WAV_CODEC, '-loglevel', 'error', cin],
                capture_output=True)
            if r_ex.returncode != 0 or not os.path.exists(cin):
                return _run_deepfilter3_adaptive(input_wav, state)

            _orig = state.style_deepfilter_atten_lim
            state.style_deepfilter_atten_lim = _ORNAMENT_DEEPFILTER_ATTEN if is_orn else atten_std
            try:
                cr = _run_deepfilter3_adaptive(cin, state)
                chunk_wavs.append(cr if os.path.exists(cr) else cin)
            except Exception:
                chunk_wavs.append(cin)
            finally:
                state.style_deepfilter_atten_lim = _orig

        if not chunk_wavs:
            return _run_deepfilter3_adaptive(input_wav, state)

        concat_txt = os.path.join(_TMP, f'orn_concat_{_uuid_orn.uuid4().hex[:8]}.txt')
        stitched   = os.path.join(_TMP, f'orn_stitch_{_uuid_orn.uuid4().hex[:8]}.wav')
        with open(concat_txt, 'w') as cf:
            for cw in chunk_wavs:
                cf.write(f"file '{cw}'\n")
        r_cat = subprocess.run(
            ['ffmpeg', '-y', '-f', 'concat', '-safe', '0',
             '-i', concat_txt, '-c:a', WAV_CODEC, '-loglevel', 'error', stitched],
            capture_output=True)
        if r_cat.returncode == 0 and os.path.exists(stitched):
            state.deepfilter3_applied = True
            return stitched

    except Exception as exc:
        L(f'  [ornament_gate] error: {exc}')

    return _run_deepfilter3_adaptive(input_wav, state)


# ── Section E: MUJAWWAD النداء style gate (GAP-2) ────────────────────────────
def _nidaa_apply_style_gate(state: 'InputState', report: dict) -> dict:
    """Flag N1/N3 as wrong-target if MUJAWWAD — instructs apply_nidaa to skip them."""
    style        = getattr(state, 'style_class', 'MURATTAL')
    skip_modules = _NIDAA_MUJAWWAD_SKIP if style == 'MUJAWWAD' else _NIDAA_HADR_SKIP
    report       = dict(report)
    report['rollback_modules'] = list(skip_modules)
    if skip_modules and any(report.get(m, False) for m in ['n1_harmonic', 'n3_room']):
        ran_wrong = [m for m in ['n1_harmonic', 'n3_room']
                     if m in skip_modules and report.get(m, False)]
        report['style_gate_warning'] = ran_wrong
        L(f'  [نداء/v20] WARNING: {style} — modules {ran_wrong} ran but should be skipped. '
          f'(Patch apply_nidaa() to honour _NIDAA_MUJAWWAD_SKIP for full fix)')
    return report


# ── Section F: Deployment audit (GAP-4) ──────────────────────────────────────
def _build_module_audit(state: 'InputState') -> dict:
    """Complete module deployment audit: applied/skipped/failed/missing for every component."""
    def _s(applied_flag, module_ok=True, reason=''):
        if not module_ok:   return {'status': _AUDIT_STATUS_MISSING,   'reason': 'module not installed'}
        if applied_flag:    return {'status': _AUDIT_STATUS_APPLIED}
        if reason:          return {'status': _AUDIT_STATUS_BYPASSED,  'reason': reason}
        return              {'status': _AUDIT_STATUS_SKIPPED}

    sc   = getattr(state, 'style_class', 'MURATTAL')
    audit = {
        'riwaya':         {'status': _AUDIT_STATUS_APPLIED, 'riwaya': getattr(state, 'riwaya_detected', 'HAFS'),
                           'confidence': round(getattr(state, 'riwaya_confidence', 0.0), 3),
                           'uncertain': getattr(state, 'riwaya_uncertain', False)},
        'style':          {'status': _AUDIT_STATUS_APPLIED, 'style': sc,
                           'confidence': round(getattr(state, 'style_confidence', 0.0), 3)},
        'deepfilter3':    {'status': (_AUDIT_STATUS_APPLIED if getattr(state, 'deepfilter3_applied', False)
                                       else (_AUDIT_STATUS_MISSING if not DEEPFILTER_OK and not DEEPFILTER_CLI_OK
                                             else _AUDIT_STATUS_SKIPPED)),
                           'ornament_gating': sc == 'MUJAWWAD' and bool(getattr(state, 'ornament_frame_mask', []))},
        'wpe':            {'status': (_AUDIT_STATUS_APPLIED if getattr(state, 'wpe_applied', False)
                                       else (_AUDIT_STATUS_MISSING if not _NARA_WPE_OK
                                             else _AUDIT_STATUS_SKIPPED)),
                           'rt60_reduction_s': round(getattr(state, 'wpe_rt60_reduction_s', 0.0), 2)},
        'hakim_driftse':  {'status': (_AUDIT_STATUS_APPLIED if getattr(state, 'hakim_driftse_accepted', False)
                                       else (_AUDIT_STATUS_MISSING if not _HAKIM_OK
                                             else (_AUDIT_STATUS_FAILED if getattr(state, 'hakim_driftse_applied', False)
                                                   else _AUDIT_STATUS_SKIPPED))),
                           'emphatic_delta': round(getattr(state, 'hakim_emphatic_delta', 0.0), 3)},
        'hakim_sips':     _s(getattr(state, 'hakim_sips_accepted', False),   _HAKIM_OK),
        'miraat':         _s(getattr(state, 'miraat_accepted', False),        _MIRAAT_OK),
        'idrak':          _s(getattr(state, 'idrak_accepted', False),         _IDRAK_OK),
        'noor':           _s(getattr(state, 'noor_applied', False),           NOOR_OK),
        'jalal':          {'status': (_AUDIT_STATUS_APPLIED if getattr(state, 'jalal_applied', False)
                                       else _AUDIT_STATUS_SKIPPED),
                           'j1_shimmer': getattr(state, 'jalal_shimmer', False),
                           'j4_widener': getattr(state, 'jalal_widener', False),
                           'mujawwad_gate': sc == 'MUJAWWAD'},
        'nidaa':          {'status': (_AUDIT_STATUS_APPLIED if getattr(state, 'nidaa_applied', False)
                                       else _AUDIT_STATUS_SKIPPED),
                           'n1_n3_skipped': sc == 'MUJAWWAD'},
        'gpu_path':       {'status': (_AUDIT_STATUS_APPLIED if getattr(state, 'gpu_enhance_applied', False)
                                       else (_AUDIT_STATUS_MISSING if not _gpu_path_available()
                                             else _AUDIT_STATUS_SKIPPED)),
                           'available': _gpu_path_available()},
        'arabic_wavlm':   {'confirmed': getattr(state, '_arabic_wavlm_confirmed', False),
                           'risk':      getattr(state, '_arabic_wavlm_risk', 'UNKNOWN')},
    }
    counts = {}
    ...
    audit['_summary'] = {k: counts.get(k, 0) for k in
                          (_AUDIT_STATUS_APPLIED, _AUDIT_STATUS_SKIPPED,
                           _AUDIT_STATUS_BYPASSED, _AUDIT_STATUS_FAILED, _AUDIT_STATUS_MISSING)}
    return audit


# ── Section G: Per-segment multilingual processing (GAP-5) ───────────────────
def _process_multilingual_segments(input_wav, state, ref, arabic_fn, non_arabic_fn):
    """Slice + process + stitch audio at language boundaries (replaces binary gate)."""
    if not state.has_multilingual_content:
        return input_wav
    lang_map = getattr(state, 'language_segment_map', [])
    if not lang_map or not NUMPY_OK:
        return input_wav
    try:
        import uuid as _uuid_ml
        job_id = _uuid_ml.uuid4().hex[:8]
        chunk_paths = []
        for idx, (start_ms, end_ms, lang, mode) in enumerate(lang_map):
            if (end_ms - start_ms) < 500.0:
                continue
            s_s = start_ms / 1000.0; dur_s = (end_ms - start_ms) / 1000.0
            cin = os.path.join(_TMP, f'ml_{job_id}_s{idx}_in.wav')
            r_ex = subprocess.run(
                ['ffmpeg', '-y', '-i', input_wav, '-ss', f'{s_s:.6f}', '-t', f'{dur_s:.6f}',
                 '-c:a', WAV_CODEC, '-loglevel', 'error', cin], capture_output=True)
            if r_ex.returncode != 0:
                chunk_paths.append((input_wav, s_s, dur_s, mode)); continue
            try:
                processed = arabic_fn(cin, state, ref) if mode == 'arabic' else non_arabic_fn(cin, state, ref)
            except Exception:
                processed = cin
            chunk_paths.append((processed if os.path.exists(processed) else cin, s_s, dur_s, mode))
        if not chunk_paths:
            return input_wav
        if len(chunk_paths) == 1:
            return chunk_paths[0][0]
        concat_txt = os.path.join(_TMP, f'ml_{job_id}_concat.txt')
        stitched   = os.path.join(_TMP, f'ml_{job_id}_stitched.wav')
        with open(concat_txt, 'w') as cf:
            for (cpath, _, _, _) in chunk_paths:
                cf.write(f"file '{cpath}'\n")
        r_cat = subprocess.run(
            ['ffmpeg', '-y', '-f', 'concat', '-safe', '0',
             '-i', concat_txt, '-c:a', WAV_CODEC, '-loglevel', 'error', stitched],
            capture_output=True)
        if r_cat.returncode == 0 and os.path.exists(stitched):
            L(f'  [التعدد/v20] ✓ stitched {len(chunk_paths)} segments')
            return stitched
    except Exception as exc:
        L(f'  [التعدد/v20] error: {exc}')
    return input_wav


def _non_arabic_segment_process(chunk_wav, state, ref):
    """Non-Arabic segment: gentle DeepFilter + LUFS -14."""
    try:
        import uuid as _u_na
        _fk = type('NS', (), {'style_deepfilter_atten_lim': 18, 'style_class': 'MURATTAL',
                               'ornament_frame_mask': []})()
        nr  = _run_deepfilter3_adaptive(chunk_wav, _fk)
        if os.path.exists(nr):
            chunk_wav = nr
        lufs_now = float(measure_lufs(chunk_wav))
        if abs(lufs_now - _COMMENTARY_LUFS_TARGET) > 0.3:
            lo = os.path.join(_TMP, f'non_ar_lufs_{_u_na.uuid4().hex[:8]}.wav')
            r  = subprocess.run(
                ['ffmpeg', '-y', '-i', chunk_wav, '-af',
                 f'volume={_COMMENTARY_LUFS_TARGET - lufs_now:.3f}dB',
                 '-c:a', WAV_CODEC, '-loglevel', 'error', lo], capture_output=True)
            if r.returncode == 0 and os.path.exists(lo):
                return lo
    except Exception:
        pass
    return chunk_wav
...
# ── Section H: GPU generative path interface (GAP-6) ─────────────────────────
def _gpu_path_available() -> bool:
    """Check if GAP-URGENet GPU path is ready (torch + CUDA + weights + package)."""
    try:
        import torch   # type: ignore
        if not torch.cuda.is_available():
            return False
        weights = os.environ.get(_GPU_ENV_WEIGHTS, '')
        if not weights or not os.path.exists(weights):
            return False
        import gap_urgenet  # type: ignore  # noqa
        return True
    except ImportError:
        return False


def _run_gpu_enhance(input_wav: str, state: 'InputState',
                      ref: 'ReferenceModel') -> Tuple[str, dict]:
    """GPU generative path: GAP-URGENet with Tajweed safety gates."""
    rep = {'status': 'SKIPPED', 'reason': '', 'gates_passed': 0,
           'emphatic_delta': 0.0, 'madd_delta_frac': 0.0}
    if not _gpu_path_available():
        rep['reason'] = 'GPU path unavailable (see Section L-1)'
        return input_wav, rep
    if state.source_tier not in ('TIER_DEGRADED', 'TIER_DAMAGED', 'TIER_CRITICAL'):
        rep['reason'] = f'tier={state.source_tier} does not need GPU path'
        return input_wav, rep
    if state.style_class != 'MURATTAL':
        rep['status'] = 'BYPASSED'
        rep['reason'] = f'style={state.style_class}: GPU path not validated for non-Murattal'
        return input_wav, rep
    try:
        import torch; from gap_urgenet import GAP  # type: ignore
        device  = os.environ.get(_GPU_ENV_DEVICE, 'cuda:0')
        weights = os.environ.get(_GPU_ENV_WEIGHTS)
        model   = GAP.load(weights, device=device); model.eval()
        import uuid as _uid_gpu
        gpu_out = os.path.join(_TMP, f'gpu_out_{_uid_gpu.uuid4().hex[:8]}.wav')
        with torch.no_grad():
            model.enhance_file(input_wav, gpu_out, atten_lim_db=_GPU_ATTEN_LIM_DB)
        if not os.path.exists(gpu_out):
            rep['status'] = 'FAILED'; rep['reason'] = 'no output'; return input_wav, rep
        # Tajweed gates
        gates = 0
        try:
            c_in  = load_audio_fast(input_wav, state.skip_s, min(state.dur_s, 30.0))
            c_out = load_audio_fast(gpu_out,   state.skip_s, min(state.dur_s, 30.0))
            s_i, s_o = third_octave(c_in), third_octave(c_out)
            if s_i and s_o:
                emp_i = float(np.mean([s_i[f] for f in s_i if 600 <= f <= 900] or [0.0]))
                emp_o = float(np.mean([s_o[f] for f in s_o if 600 <= f <= 900] or [0.0]))
                rep['emphatic_delta'] = round(emp_o - emp_i, 3)
                if abs(rep['emphatic_delta']) <= 1.5:
                    gates += 1
                else:
                    rep['status'] = 'FAILED'; rep['reason'] = f'emphatic Δ={rep["emphatic_delta"]:.2f}dB'
                    return input_wav, rep
        except Exception: gates += 1
        rep['gates_passed'] = gates
        rep['status'] = 'OK'
        state.gpu_enhance_applied = True
        return gpu_out, rep
    except Exception as exc:
        rep['status'] = 'FAILED'; rep['reason'] = str(exc)
        return input_wav, rep


# ── Section I: Arabic WavLM confirmation gate (GAP-7) ────────────────────────
def _check_arabic_wavlm_in_hakim() -> dict:
    """Check if الحكيم is using Arabic-pretrained WavLM or English WavLM."""
    result = {'arabic_wavlm_confirmed': False, 'backbone_lang': 'unknown',
              'nisqa_risk': 'UNKNOWN', 'action_required': False}
    try:
        import hakim_gen_v2 as _hk
        # v21: الحكيم v2 uses Resemble Enhance + MetricGAN+ — no WavLM backbone.
        # Report which neural backend is actually available.
        resemble_ok  = getattr(_hk, '_RESEMBLE_OK',  False)
        frcrn_ok     = getattr(_hk, '_FRCRN_OK',     False)
        metricgan_ok = getattr(_hk, '_METRICGAN_OK', False)
        if resemble_ok or frcrn_ok:
            result['arabic_wavlm_confirmed'] = True
            result['backbone_lang']  = ('resemble_enhance' if resemble_ok else 'frcrn')
            result['nisqa_risk']     = 'NONE (WavLM replaced by Resemble/FRCRN+MetricGAN+)'
        else:
            result['backbone_lang']  = 'none_installed'
            result['nisqa_risk']     = 'no neural backend installed'
            result['action_required']= True
    except ImportError:
        result['nisqa_risk']      = 'N/A (الحكيم not installed)'
        result['action_required'] = False
    except Exception as exc:
        result['nisqa_risk']      = f'check_error: {exc}'
        result['action_required'] = True
    return result


# ── Section J: 5-feature MUJAWWAD classifier (GAP-8) ─────────────────────────
def _estimate_vibrato_rate(clip: 'np.ndarray', sr: int) -> float:
    """Estimate vibrato rate (Hz) from amplitude envelope modulation."""
    if not NUMPY_OK or len(clip) < sr:
        return 0.0
    try:
        from scipy.signal import hilbert as _hilbert
        env = np.abs(_hilbert(clip.astype(np.float64)))
        env = (env - env.mean()) / (env.std() + 1e-10)
        N   = len(env)
        acf = np.correlate(env, env, mode='full')[N-1:]
        lag_min = max(1, int(sr / 10.0))   # 10 Hz max vibrato
        lag_max = min(N-1, int(sr / 2.0))  # 2 Hz min vibrato
        if lag_min >= lag_max:
            return 0.0
        peak_lag = lag_min + int(np.argmax(acf[lag_min:lag_max+1]))
        return float(sr / max(peak_lag, 1))
    except Exception:
        return 0.0


def _estimate_mean_phrase_length_s(clip: 'np.ndarray', sr: int) -> float:
    """Estimate mean phrase length from inter-breath pause pattern."""
    if not NUMPY_OK or len(clip) < sr * 2:
        return 0.0
    try:
        frame_n = int(0.020 * sr); hop_n = frame_n
        thresh  = rms_db(clip) - 15.0
        voiced  = [rms_db(clip[i*hop_n:i*hop_n+frame_n]) > thresh
                   for i in range(max(1, (len(clip)-frame_n)//hop_n))]
        pause_lim  = int(0.300 * sr / hop_n)
        phrase_durs = []
        p_start = 0; pause_count = 0; in_phrase = False
        for i, v in enumerate(voiced):
            if v:
                if not in_phrase:
                    p_start, in_phrase = i, True
                pause_count = 0
            else:
                if in_phrase:
                    pause_count += 1
                    if pause_count >= pause_lim:
                        phrase_durs.append((i - pause_count - p_start) * 0.020)
                        in_phrase = False; pause_count = 0
        if in_phrase:
            phrase_durs.append((len(voiced) - p_start) * 0.020)
        return float(np.mean(phrase_durs)) if phrase_durs else 0.0
    except Exception:
        return 0.0


def _classify_recitation_style_v20(path: str, state: 'InputState') -> Tuple[str, float]:
    """5-feature MUJAWWAD classifier (v20 upgrade: vibrato rate + phrase length added)."""
    if not NUMPY_OK:
        return ('MURATTAL', 0.5)
    try:
        analysis_dur = min(120.0, max(0.0, state.total_s - state.skip_s - 5.0))
        ...
        if len(clip) < SR * 5:
            return ('MURATTAL', 0.5)
        frame_n  = int(0.020 * SR); hop_n = frame_n
        thresh   = rms_db(clip) - 12.0
        n_frames = max(1, (len(clip) - frame_n) // hop_n)
        voiced   = np.array([rms_db(clip[i*hop_n:i*hop_n+frame_n]) > thresh for i in range(n_frames)], bool)
        onsets   = np.where(np.diff(voiced.astype(int)) > 0)[0]
        tot_v_s  = float(np.sum(voiced) * hop_n / SR)
        syl_rate = len(onsets) / max(tot_v_s, 1.0)

        if syl_rate > 4.5:
            return ('HADR', float(np.clip(0.40 + (syl_rate - 4.5) / 3.0 * 0.50, 0.40, 0.95)))

        win_n  = int(0.500 * SR); win_hop = int(0.100 * SR)
        f0_mads = [_estimate_f0_variance(clip[i*win_hop:i*win_hop+win_n], SR)
                   for i in range(max(1, (len(clip)-win_n)//win_hop))
                   if rms_db(clip[i*win_hop:i*win_hop+win_n]) > thresh]
        mean_f0_mad   = float(np.mean(f0_mads)) if f0_mads else 0.0
        vibrato_hz    = _estimate_vibrato_rate(clip, SR)
        mean_phrase_s = _estimate_mean_phrase_length_s(clip, SR)
        segs          = _extract_voiced_segments(clip, SR, frame_n, hop_n, min_dur_ms=300.0)
        silences      = [(segs[j][0]-segs[j-1][1])*1000.0 for j in range(1, len(segs))
                          if (segs[j][0]-segs[j-1][1])*1000.0 > 100.0]
        median_sil_ms = float(np.median(silences)) if silences else 0.0

        muj_votes = {
            'f0_mad':     (mean_f0_mad,    0.30), 'silence': (median_sil_ms, 0.20),
            'vibrato':    (vibrato_hz,     0.25), 'phrase':  (mean_phrase_s, 0.15),
            'syl_rate_q': (syl_rate,       0.10),
        }
        def _vote(key, val):
            if key == 'f0_mad':     return float(np.clip((val -  60.0) /  60.0, 0.0, 1.0))
            if key == 'silence':    return float(np.clip((val - 500.0) / 600.0, 0.0, 1.0))
            if key == 'vibrato':    return float(np.clip((val -   2.0) /   3.0, 0.0, 1.0))
            if key == 'phrase':     return float(np.clip((val -   5.0) /   5.0, 0.0, 1.0))
            if key == 'syl_rate_q': return float(np.clip((2.5  - val) /   1.0, 0.0, 1.0))
            return 0.0

        weighted = sum(_vote(k, v) * w for k, (v, w) in muj_votes.items())
        L(f'  [الأسلوب v20] F0_MAD={mean_f0_mad:.1f}c  vibrato={vibrato_hz:.2f}Hz  '
          f'phrase={mean_phrase_s:.1f}s  silence={median_sil_ms:.0f}ms  '
          f'MUJAWWAD_vote={weighted:.3f}')
        if weighted >= 0.45:
            conf = float(np.clip(0.45 + (weighted - 0.45) * 1.5, 0.45, 0.95))
            return ('MUJAWWAD', conf)
        return ('MURATTAL', 0.80)
    except Exception as exc:
        L(f'  [الأسلوب v20] error: {exc}')
        return ('MURATTAL', 0.5)


def _v20_analyze_additions(state: 'InputState', input_path: str,
                            ref: 'ReferenceModel') -> None:
    """v20 additions to analyze_input (called after _v16_analyze_additions)."""
    # Re-run style classification with 5-feature model if borderline confidence
    conf = getattr(state, 'style_confidence', 0.0)
    if conf < 0.55:
        L(f'\n  [v20] style conf={conf:.2f} < 0.55 — re-running 5-feature classifier')
        try:
            new_style, new_conf = _classify_recitation_style_v20(input_path, state)
            if new_conf > conf:
                L(f'  [v20] style: {state.style_class}({conf:.2f}) → {new_style}({new_conf:.2f})')
                state.style_class      = new_style
                state.style_confidence = new_conf
                _apply_style_overrides(state)
        except Exception as exc:
            L(f'  [v20] style re-classification error: {exc}')
    # Arabic WavLM confirmation check
    wlm = _check_arabic_wavlm_in_hakim()
    state._arabic_wavlm_confirmed = wlm['arabic_wavlm_confirmed']
    state._arabic_wavlm_risk      = wlm['nisqa_risk']
    if wlm.get('action_required'):
        L(f'  [v20/WavLM] ⚠ Arabic WavLM not confirmed in الحكيم — '
          f'backbone={wlm["backbone_lang"]}  risk={wlm["nisqa_risk"]}')
...
def analyze_input(path: str, ref: ReferenceModel) -> InputState:
    ...
    # KB-14-05: Joint noise-robust BWE candidate detection (§70.4, v6)
    # Fires when: source is band-limited (detected cutoff < 16kHz) AND noisy
    # (source_tier DAMAGED or CRITICAL). Sequential NR→BWE is sub-optimal here;
    # a joint noise-robust BWE model (Vocos-BWE §47 or Lin et al. 2024) would
    # process both defects simultaneously and avoid NR over-suppression of
    # the limited-bandwidth spectral content.
    if (getattr(state, 'source_tier', '') in ('TIER_DAMAGED', 'TIER_CRITICAL') and
            getattr(state, 'd01_codec', 0.0) > 0.25 and
            getattr(state, 'nr_confidence', 0.0) > 0.4):
        state.joint_nr_bwe_candidate = True
        L('  [KB-14-05/BWE] Joint noise-robust BWE candidate detected. '
          'Source has simultaneous band-limitation + noise. '
          'Sequential NR→BWX is sub-optimal; Vocos-BWE (§47) would handle both '
          'in one pass. Add --nr-backend=sfmse when SFMSE pip package lands.')

    _v13_analyze_additions(state, clip, ref)
    _compute_confidence_vectors(state, ref)

    # ── v16 Part II: Arabic DNA additions ──────────────────────────────────────
    try:
        _v16_analyze_additions(state, path, ref)
    except Exception as _v16e:
        L(f'  [v16_analyze] error (non-fatal): {_v16e}')

    # v20 V-1: 5-feature style classifier + Arabic WavLM gate
    try:
        _v20_analyze_additions(state, path, ref)
    except Exception as _v20e:
        L(f'  [v20_analyze] error (non-fatal): {_v20e}')

    # ── v17 Part III: Silero VAD availability check ───────────────────────────
    state.silero_vad_available = _SHERPA_OK and _init_silero_vad()
    if state.silero_vad_available:
        L('  [v17/Silero] Silero VAD available ✓')
...
def _declip_pass(input_path: str, state: InputState) -> str:
    """
    FIX-13/14 + BUG-V13-02: Memory-safe chunked declipping.

    v12 loaded the entire file (up to 1.38GB for 2h Quran) causing OOM on S22.
    v13 caps each chunk to DECLIP_MAX_S=600s and concatenates via ffmpeg.
    """
    if not SCIPY_OK:
        return input_path
    try:
    ...
    CLIP_THRESH = 0.995
    CONTEXT     = 40
    CHUNK_S     = DECLIP_MAX_S

    # Fast probe on analysis window
    probe = load_audio_fast(input_path, state.skip_s, min(state.dur_s, 30.0))
    if len(probe) < SR * 3:
        return input_path
    if int(np.sum(np.abs(probe) >= CLIP_THRESH)) == 0:
        return input_path
    del probe

    total_chunks  = max(1, int(state.total_s / CHUNK_S) + 1)
    chunk_outputs = []

    for cidx in range(total_chunks):
        skip_s = cidx * CHUNK_S
        dur_s  = min(CHUNK_S, state.total_s - skip_s)
        if dur_s <= 0:
            break

        audio = load_audio_fast(input_path, skip_s=skip_s, duration_s=dur_s)
        if len(audio) < SR * 3:
            _write_chunk(audio, cidx, chunk_outputs)
            continue
        ...
        L(f'  [declip chunk {cidx+1}/{total_chunks}] {clipped_n} samples clipped')
        fixed      = audio.copy()
        ...
        while i < len(audio):
            if is_clipped[i]:
                ...
                if (i - left) >= 5 and (right - j) >= 5:
                    xc = list(range(left, i)) + list(range(j, right))
                    yc = audio[xc]
                    try:
                        cs = CubicSpline(xc, yc)
                        fixed[i:j] = np.clip(cs(np.arange(i, j)).astype(np.float32), -1.0, 1.0)
                        repairs += 1
                    except Exception:
                        pass
                i = max(j, i + 1)
                ...
                i += 1
        L(f'  [declip chunk {cidx+1}] {repairs} regions repaired')
        _write_chunk(fixed if repairs > 0 else audio, cidx, chunk_outputs)
    ...
    concat_list = os.path.join(_TMP, 'v130_declip_concat.txt')
    try:
        with open(concat_list, 'w') as f:
            for p in chunk_outputs:
                f.write(f"file '{p}'\n")
        out_path = os.path.join(_TMP, 'v130_declip_full.wav')
        r = subprocess.run(
            ['ffmpeg', '-y', '-f', 'concat', '-safe', '0',
             '-i', concat_list, '-c:a', WAV_CODEC, '-loglevel', 'error', out_path],
            capture_output=True)
        if r.returncode == 0 and os.path.exists(out_path):
            L(f'  [declip] {len(chunk_outputs)} chunks merged OK')
            return out_path
    except Exception as e:
        L(f'  [declip] concat failed: {e}')
    finally:
...
def _write_chunk(audio: 'np.ndarray', cidx: int, out_list: list) -> None:
    """Write one float32 mono chunk to a temp WAV and append path to out_list."""
    tmp = os.path.join(_TMP, f'v130_declip_chunk_{cidx}.wav')
...
def _run_frcrn_nr(input_wav: str, output_wav: str) -> bool:
    """
    KB-14-04 — ClearerVoice FRCRN NR backend (§69.5, Supplement v6).

    FRCRN (Full-band Recurrent CNN) is a real-time-capable SE model from
    ClearerVoice-Studio with 3M reported production uses. Benchmarked on
    DNS-2020 at 16 kHz; performs comparably to DeepFilterNet on clean-speech
    preservation metrics.

    This function is called when --nr-backend=frcrn instead of the default
    afftdn/DeepFilterNet chain. It writes the enhanced audio to output_wav.

    Installation: pip install clearvoice-studio --break-system-packages

    Returns True on success, False on failure (caller falls back to afftdn).
    """
    try:
        from clearvoice import ClearVoice
        model = ClearVoice(task='speech_enhancement', model_names=['FRCRN_SE_16K'])
        output = model(input=input_wav, online_write=False)
        import soundfile as sf
        import numpy as np
        # ClearVoice returns a numpy array at 16kHz; resample to 48kHz for engine
        try:
            ...
            up   = SR // g
            down = 16000 // g
            out_48k = resample_poly(output, up, down).astype(np.float32)
        except ImportError:
            out_48k = output.astype(np.float32)
        sf.write(output_wav, out_48k, SR, subtype='PCM_24')
        L(f'  [KB-14-04/FRCRN] enhanced → {output_wav}')
        return True
    except ImportError:
        L('  [KB-14-04/FRCRN] clearvoice-studio not installed — falling back to afftdn')
        return False
    except Exception as _e:
        L(f'  [KB-14-04/FRCRN] error: {_e} — falling back to afftdn')
        return False


def _run_sfmse_nr(input_wav: str, output_wav: str) -> bool:
    """
    KB-14-04 — SFMSE one-step flow-matching NR backend (§68.5, Supplement v6).

    SFMSE (Shortcut Flow Matching for Speech Enhancement, Zhou et al. 2025)
    achieves SGMSE+ quality in a single Neural Function Evaluation.
    RTF = 0.013 on consumer GPU; ~0.05 on CPU (competitive with DeepFilterNet2).

    Step-invariant: the same model handles NFE=1 (fast) or NFE=4 (quality)
    without retraining — set sfmse_nfe=1 for batch archival, 4 for critical files.

    Source: github.com/liduojia1/MeanFlowSE (closest open-source equivalent)
    NOTE: SFMSE itself may not have a pip package yet as of June 2026.
    This function provides the infrastructure; fill in the model loading once
    a pip-installable checkpoint is available.

    Returns True on success, False on fallback.
    """
    try:
        # Placeholder: import the SFMSE model once available
        # from sfmse import SFMSEEnhancer
        # model = SFMSEEnhancer(nfe=1)
        # model.enhance(input_wav, output_wav)
        L('  [KB-14-04/SFMSE] SFMSE not yet pip-installable (June 2026). '
          'Falling back to afftdn. Monitor github.com/liduojia1/MeanFlowSE.')
        return False
    except Exception as _e:
        L(f'  [KB-14-04/SFMSE] error: {_e} — falling back')
        return False

def _ikhfa_guard_filter(nr_depth: int, state: InputState) -> str:
    """KB-13-06: Re-boost 250-400Hz after afftdn to protect ikhfa nasality."""
    if nr_depth <= _IKHFA_NR_DEPTH_CAP:
        return ''
    if state.source_tier not in ('TIER_DAMAGED', 'TIER_CRITICAL'):
        return ''
    boost_db = round(min((nr_depth - _IKHFA_NR_DEPTH_CAP) * _IKHFA_BOOST_FACTOR, 3.0), 1)
    if boost_db < 0.3:
        return ''
    state.ikhfa_guard_applied = True
    L(f'  [KB-13-06/Ikhfa] +{boost_db}dB @ 315Hz ikhfa guard (nr_depth={nr_depth})')
    return f'equalizer=f=315:width_type=o:width=0.7:g=+{boost_db}'
...
def _build_nr_filter(state: InputState, ref: ReferenceModel,
    ...
    ikhfa_filt = _ikhfa_guard_filter(nr_depth, state)
    if ikhfa_filt:
        filters.append(ikhfa_filt)
    return ','.join(filters)
...
def nr_pass(input_path: str, state: InputState, ref: ReferenceModel,
    ...
    tmp_nr_orig = os.path.join(_TMP, 'v100_nr.wav')  # v14
    # KB-14-04: NR backend routing
    if getattr(state, 'nr_backend', 'deepfilter') == 'frcrn':
        _frcrn_out = os.path.join(_TMP, 'v140_frcrn_nr.wav')
        if _run_frcrn_nr(input_path, _frcrn_out):
            nr_report['applied'] = True
            nr_report['backend'] = 'frcrn'
            return _frcrn_out, nr_report
    elif getattr(state, 'nr_backend', 'deepfilter') == 'sfmse':
        _sfmse_out = os.path.join(_TMP, 'v140_sfmse_nr.wav')
        if _run_sfmse_nr(input_path, _sfmse_out):
            nr_report['applied'] = True
            nr_report['backend'] = 'sfmse'
            return _sfmse_out, nr_report
...
def _detect_codec_artifacts(state: 'InputState') -> Tuple[int, Dict]:
    ...
    if state.full_spectrum:
        ...
        if mid_vals and hf_vals:
            ...
            # v18-FIX-3: TIER_DEGRADED mosque recordings have a NATURAL steep HF
            # rolloff (-14.9dB/oct from mosque acoustics + MP3 cutoff at ~14kHz).
            # The 30dB gap threshold then fires incorrectly and sends the signal
            # through aexciter BWE, adding synthetic 4-8kHz content on top of
            # an already over-processed output.  A genuine codec cutoff produces
            # a *cliff* not a slope; raise the threshold to 42dB for TIER_DEGRADED
            # so only actual bandwidth truncation (≈20dB/oct cliff) triggers BWE.
            _vhfl_thresh = 42.0 if getattr(state, 'source_tier', '') == 'TIER_DEGRADED' else 30.0
            # v19-FIX-B: slope discriminator — mosque acoustic rolloff vs codec cliff.
            # A large mid→HF gap alone is insufficient: natural mosque acoustics +
            # mic HF roll-off can produce >42dB integrated gap even when the codec
            # preserved full bandwidth.  The two cases differ in *slope*:
            #   Natural acoustic slope : ~12–20 dB/octave (gradual)
            #   Codec cutoff cliff     :  ≥24 dB/octave   (brick-wall)
            # Measure the 5kHz→10kHz window (exactly 1 octave in log space).
            # Only flag voiced_hf_loss when both the gap AND the slope exceed their
            # respective thresholds.  Default to cliff=True when bands are absent
            # so missing spectrum data never silently disables BWE on real loss.
            _hf_slope_db_oct = 999.0   # conservative fallback: treat as cliff
            if (state.full_spectrum
                    and 5000 in state.full_spectrum
                    and 10000 in state.full_spectrum):
                _hf_slope_db_oct = (state.full_spectrum[5000]
                                    - state.full_spectrum[10000])  # dB per octave
            _is_codec_cliff = (_hf_slope_db_oct >= 24.0)
            # >_vhfl_thresh gap AND cliff-like slope = genuine BW loss
            voiced_hf_loss = (mid_mean - hf_mean) > _vhfl_thresh and _is_codec_cliff
...
def _bandwidth_extension_pass(input_path: str, state: 'InputState') -> Tuple[str, Dict]:
    ...
    if cutoff >= 14000 and state.full_spectrum:
        ...
        # v19-FIX-B (guard): mirror the tier-aware threshold + slope check from
        # _classify_artifacts so this inner check cannot override it.
        # Previous code used a hardcoded 30dB gap regardless of tier; on a
        # TIER_DEGRADED mosque clip the 42dB threshold was already raised but
        # this inner check still fired at 30dB, forcing cutoff=5000 and BWE.
        _bwp_thresh = 42.0 if getattr(state, 'source_tier', '') == 'TIER_DEGRADED' else 30.0
        _bwp_gap = (float(_np2.mean(mid_v)) - float(_np2.mean(hf_v))
                    if (mid_v and hf_v) else 0.0)
        _bwp_slope = 999.0   # fallback: treat as cliff (conservative)
        if 5000 in state.full_spectrum and 10000 in state.full_spectrum:
            _bwp_slope = state.full_spectrum[5000] - state.full_spectrum[10000]
        _bwp_is_cliff = (_bwp_slope >= 24.0)
        if mid_v and hf_v and _bwp_gap > _bwp_thresh and _bwp_is_cliff:
            ...
            L(f'  [T2-C3] voiced HF loss detected — using effective cutoff={cutoff:.0f}Hz '
              f'(gap={_bwp_gap:.0f}dB slope={_bwp_slope:.0f}dB/oct)')
        else:
            return input_path, report
    elif cutoff >= 14000:
        return input_path, report
...
# ── BUG-V14-01: TYPE_C constants (were MISSING → NameError on every CODEC file)
# C-1 Pre-echo suppression
_C_PREECHO_LOOKBACK_S: float = 0.030   # 30ms = 3 × 10ms frames
_C_PREECHO_ATTEN_DB:   float = 6.0    # 6dB attenuation in pre-echo zone
_C_PREECHO_THRESH_DB:  float = 10.0   # energy must be >10dB below masker

# C-2 Spectral NLM denoising
_C_NLM_SIGMA_REL:  float = 0.15   # relative noise sigma (15% of mean mag)
_C_NLM_T_RADIUS:   int   = 4      # ±4 frames temporal search radius
_C_NLM_F_PATCH:    int   = 3      # ±3 bins spectral patch half-width
_C_NLM_ITERS:      int   = 2      # 2 NLM iterations (speed vs quality)
_C_NLM_H_FLOOR:    float = 1e-6   # h² floor prevents div-by-zero

# C-3 Bandwidth extension
_C_BWX_MIN_CUTOFF_HZ: float = 16000.0  # skip BWX if cutoff >= 16kHz
_C_BWX_MAX_GAIN_DB:   float = 6.0      # max extension gain at cutoff
_C_BWX_TAPER_OCTAVE:  float = 1.0      # fade to 0dB over 1 octave above cutoff

# ── v13 new constants ─────────────────────────────────────────────────────────
DECLIP_MAX_S: float = 600.0   # BUG-V13-02: max seconds per declip chunk
MADD_COUNT_SECONDS: float = 0.145   # KB-13-01: 1 harakah at Sheikh's speed
MADD_MIN_COUNTS:    int   = 2
MADD_ALERT_RATIO:   float = 0.80
_TAFASSHI_CENTROID_LO: float = 4000.0
_TAFASSHI_CENTROID_HI: float = 8000.0
_TAFASSHI_ZCR_THRESH:  float = 0.25
_TAFASSHI_GAIN_FLOOR:  float = 0.55
_IDGHAM_SLOPE_DB_THRESH: float = 4.0
_IDGHAM_ZCR_CEIL:        float = 0.12
_MOS_WEIGHTS: tuple = (0.30, 0.25, 0.20, 0.15, 0.10)
_IKHFA_NR_DEPTH_CAP: int   = 4
_IKHFA_BOOST_FACTOR: float = 0.50
...
# ── MMSE-LSA / OMLSA / IMCRA algorithm constants ─────────────────────────────
_BETA_NOISE:       float = 0.98     # noise PSD smoothing coefficient (IMCRA)
_IMCRA_MIN_WIN:    int   = 8        # minimum statistics window length (frames)
_IMCRA_BIAS:       float = 1.66     # IMCRA bias correction factor
_TSNR_GAIN_ALPHA:  float = 0.92     # a posteriori SNR gain smoothing
_TSNR_VAD_THRESH:  float = 1.5      # mean γ threshold to classify voiced frame
_TSNR_VOICE_HOLD:  int   = 5        # voiced hold-over frames after VAD drop
...
def _v16_mmse_lsa_pass(
    ...
    G_applied  = np.ones(fft_bins, dtype=np.float64)  # BUG-V13-04
    ...
    for i in range(n_frames):
        ...
        # KB-12-06 + KB-13-02: Hams breathiness + Tafasshi spectral guard (v13)
        if is_voiced:
            _frame_seg = audio[i * hop_n: i * hop_n + frame_n]
            if len(_frame_seg) > 4:
                _zcr_v13 = float(np.sum(np.abs(np.diff(np.sign(_frame_seg)))) /
                                 (2 * max(len(_frame_seg) - 1, 1)))
                # KB-12-06: Hams — any high-ZCR voiced frame → cap at 0.50
                if _zcr_v13 > 0.22:
                    G_applied = np.maximum(G_applied, 0.50)
                # KB-13-02: Tafasshi (shin) — spectral centroid in 4-8kHz + high ZCR
                if _zcr_v13 > _TAFASSHI_ZCR_THRESH:
                    try:
                        _spec_v13  = np.abs(rfft(_frame_seg.astype(np.float64))) ** 2
                        _fq_v13    = rfftfreq(len(_frame_seg), 1.0 / sr)
                        _band_v13  = (_fq_v13 >= 700.0) & (_fq_v13 <= 9500.0)
                        if _band_v13.any():
                            _cent = float(np.sum(_fq_v13[_band_v13] * _spec_v13[_band_v13]) /
                                          (np.sum(_spec_v13[_band_v13]) + 1e-30))
                            if _TAFASSHI_CENTROID_LO <= _cent <= _TAFASSHI_CENTROID_HI:
                                G_applied = np.maximum(G_applied, _TAFASSHI_GAIN_FLOOR)
                    except Exception:
                        pass
...
def _v16_type_c_preecho_suppress(
    ...
    """
    C-1: Pre-echo suppression + KB-13-03 Idgham + KB-14-02 emphatic guard.

    KB-13-03: voiced frames with monotone rising energy (idgham ramp) are
    marked IDGHAM_PROTECTED and excluded from pre-echo attenuation.
    All other logic from v12 is preserved (OLA smoothing fix included).
    """
    if not NUMPY_OK or len(audio) < sr * 0.5:
        return audio
    ...
    rms_db_arr  = _v16_frame_rms_db(audio, sr, 0.020, 0.010)
    n_f         = len(rms_db_arr)

    # ZCR per frame for idgham detection
    zcr_arr = np.zeros(n_f, dtype=np.float32)
    for i in range(n_f):
        seg = audio[i * hop_n: i * hop_n + frame_n]
        if len(seg) > 4:
            zcr_arr[i] = float(np.sum(np.abs(np.diff(np.sign(seg)))) /
                               (2 * max(len(seg) - 1, 1)))

    # KB-13-03: Idgham-protected frames
    idgham_protected = set()
    n_look_idg = int(0.030 / 0.010)   # 30ms = 3 frames
    for i in range(n_look_idg, n_f):
        if zcr_arr[i] > _IDGHAM_ZCR_CEIL:
            continue
        window_rms = rms_db_arr[i - n_look_idg: i + 1]
        if len(window_rms) < n_look_idg + 1:
            continue
        diffs = np.diff(window_rms)
        if np.all(diffs > 0.5) and float(window_rms[-1] - window_rms[0]) < 8.0:
            for pf in range(i - n_look_idg, i + 1):
                idgham_protected.add(pf)

    # Onset detection
    onset_mask = np.zeros(n_f, dtype=bool)
    for i in range(1, n_f):
        if rms_db_arr[i] - rms_db_arr[i - 1] > 8.0:
            onset_mask[i] = True
    ...
    for i in range(n_f):
        if not onset_mask[i]:
            continue
        masker_db = rms_db_arr[max(0, i - 1)]
        for j in range(max(0, i - n_look), i):
            if masker_db - rms_db_arr[j] >= _C_PREECHO_THRESH_DB:
                if j in idgham_protected:
                    continue   # KB-13-03: protect idgham ramp
                suppress_gain[j] = min(suppress_gain[j], atten_lin)

    # Smooth gain (v12 OLA fix preserved)
...
def _v16_type_c_bw_extend(
    ...
    # BUG-V13-01 FIX: true power-complementary LR HP.
    # H_HP = ratio^2 * H_LP = ratio^2/sqrt(1+ratio^4)
    # satisfies |H_LP|^2 + |H_HP|^2 = 1 at all frequencies.
    # The old formula H_HP = 1-H_LP gave H_HP(fc)=0.293 instead of 0.707.
    H_HP      = (ratio.astype(np.float32) ** 2 * H_LP).astype(np.float32)
...
def enhance_tier2(input_wav: str, state: 'InputState',
    ...
    if df_avail and df_trigger:
        ...
        if df3_ok and Path(df3_out).exists() and Path(df3_out).stat().st_size > 0:
            ...
            input_wav            = df3_out
            # v20 V-8: track for QARA audit
            if hasattr(state, 'deepfilter3_applied'):
                state.deepfilter3_applied = True
...
def _apply_iec2_shelf_correction(nodes: list, state: InputState) -> list:
    """KB-13-05: prepend +3.5dB shelf at 7kHz for IEC Type II chrome tape."""
    if not getattr(state, 'tape_iec2_suspected', False):
        return nodes
    if getattr(state, 'tape_iec2_shelf_applied', False):
        return nodes
    state.tape_iec2_shelf_applied = True
    L('  [KB-13-05/IEC2] +3.5dB shelf @ 7kHz — chrome tape HF compensation')
    return [(7000.0, +3.5, 0.9)] + list(nodes)
...
def design_eq(post_nr_spectrum: Dict, ref: ReferenceModel, state: InputState,
    ...
    # v18-FIX-2a: reduce HF EQ max boost for TIER_DEGRADED.
    # At 6dB cap, the optimizer can place +6dB nodes at 7kHz and 11kHz to flatten
    # the natural -14.9dB/oct mosque rolloff toward the reference slope.  Stacked
    # with TYPE_C BWE and J-1 shimmer this produced +34dB at 5kHz (metallic ring).
    # With BWE and shimmer now also fixed, keep the EQ cap at 3dB for TIER_DEGRADED
    # to match natural rolloff character rather than fight it.
    _max_eq_db = 3.0 if state.source_tier == 'TIER_DEGRADED' else 6.0

    eq_nodes = optimize_eq(
        post_nr_spectrum, biased_target,
        n_nodes=12, max_db=_max_eq_db, sib_cap=sib_cap,
    ...
    # v19-FIX-A: dedup BEFORE per-band clamp.
    # The L-BFGS-B optimizer can split a large gain correction into N nearby
    # nodes each ≤ the per-band clamp limit (e.g., two +2.5dB peaks at 400Hz
    # and 500Hz to get an effective +5dB where only ±3dB is allowed).  Running
    # _dedup_eq_nodes AFTER the clamp left both nodes through; their summed
    # effect (+5dB combined) silently overrode the ±3dB identity guard.
    # Fix: merge first so the summed gain is subject to the clamp below.
    ...
    _tier_degraded     = (state.source_tier == 'TIER_DEGRADED')
    _clamped = []
    for f_node, g_node, q_node in eq_nodes:
        ...
        # v18-FIX-2b: hard +3dB boost ceiling above 3kHz for TIER_DEGRADED
        elif f_node > 3000 and _tier_degraded:
            g_node = float(np.clip(g_node, -5.0, 3.0))
    ...
    if _tier_degraded:
        L('  [v18-FIX-2b/DegradedHFClamp] >3kHz boosts capped at +3dB '
          '(TIER_DEGRADED natural HF rolloff -- prevent metallic plateau)')
    ...
    eq_nodes = _apply_iec2_shelf_correction(eq_nodes, state)
    eq_nodes = _apply_iec2_shelf_correction(eq_nodes, state)
    return eq_nodes
...
def _measure_true_peak_ebur128(path: str) -> float:
    """
    FIX-15 (Plan Part I, MISS-03): Measure True Peak via ffmpeg ebur128.
    Uses EBU R128 4x oversampled measurement (catches inter-sample peaks
    that sample-domain max() misses by up to 3dB).
    Returns dBTP value, or -99.0 if measurement fails.
    """
    sp, tc = _safe(path)
    r = subprocess.run(
        ['ffmpeg', '-i', sp, '-af', 'ebur128=peak=true',
         '-f', 'null', '-', '-loglevel', 'info'],
        capture_output=True, text=True)
    if tc:
        try: os.remove(tc)
        except: pass
    # Parse "True peak:" from ebur128 summary output
    for line in r.stderr.split('\n'):
        s = line.strip()
        if 'True peak:' in s or 'true peak:' in s.lower():
            try:
                return float(s.split()[-2])
            except (ValueError, IndexError):
                pass
        # Alternative: "TP:" format in some ffmpeg versions
        if s.startswith('TP:') and 'dB' in s:
            try:
                return float(s.split('TP:')[1].strip().split()[0])
            except (ValueError, IndexError):
                pass
    return -99.0
...
def run_pass_encode(best_wav: str, output_path: str,
    ...
    for attempt in range(3):
        ...
        # FIX-15 (Plan Part I, MISS-03): attack=5ms, release=50ms per plan spec.
        # Slower attack/release = less pumping while still catching inter-sample peaks.
        # level=0 disables input level correction (preserves our explicit volume trim).
        # asc=1 adaptive release prevents loudness pumping on dense recitation.
        parts.append(f'alimiter=limit={limiter_threshold:.4f}:level=0:attack=5:release=50:asc=1:asc_level=0.5')
    ...
    # FIX-15 (Plan Part I, MISS-03): Post-encode ebur128 True Peak verification.
    # Sample-domain max() above misses inter-sample peaks (up to +3dB).
    # ebur128 uses 4x oversampling per EBU R128 — the authoritative TP standard.
    # If verified TP > -1.0 dBTP → apply gain correction + re-encode once.
    _tp_ebur128 = _measure_true_peak_ebur128(output_path)
    if _tp_ebur128 > -1.0 and _tp_ebur128 > -90.0:
        _tp_excess = _tp_ebur128 - (-1.0)
        L(f'  [FIX-15] ebur128 TP={_tp_ebur128:.2f}dBTP > -1.0 — '
          f'correcting {_tp_excess:.2f}dB excess')
        import uuid as _uu_tp15
        _tp_fixed = output_path + f'_tp15_{_uu_tp15.uuid4().hex[:6]}.mp3'
        _tp_af = (f'volume=-{_tp_excess + 0.1:.2f}dB,'
                  f'alimiter=limit=0.8913:level=0:attack=5:release=50')
        _tp_r = subprocess.run(
            ['ffmpeg', '-y', '-i', output_path, '-af', _tp_af,
             '-b:a', '320k', '-loglevel', 'error', _tp_fixed],
            capture_output=True)
        if _tp_r.returncode == 0 and os.path.exists(_tp_fixed):
            shutil.move(_tp_fixed, output_path)
            true_peak_db = _tp_ebur128 - _tp_excess - 0.1
            n_retries += 1
            L(f'  [FIX-15] ✓ TP corrected → {true_peak_db:.2f}dBTP')
        else:
            L(f'  [FIX-15] correction encode failed — TP={_tp_ebur128:.2f}dBTP remains')
    elif _tp_ebur128 > -90.0:
        L(f'  [FIX-15] ebur128 TP={_tp_ebur128:.2f}dBTP ✓ (≤-1.0)')
        true_peak_db = _tp_ebur128  # use the authoritative measurement
...
def measure_pass(wav_path: str, ref: ReferenceModel, state: InputState,
    ...
    # v20 V-9: track in state for QARA Dimension 5
    if hasattr(state, 'sib_snr'):
        state.sib_snr = result.sib_snr
...
def enhance(input_path: str, output_path: str,
            ...
            force_tier: str = None,
            quran_prefix: str = '',
            nr_backend: str = 'deepfilter') -> Dict:
    t0 = time.time()
    ...
    L(f'╔════════════════════════════════════════════════════════════╗')
    L(f'║  الاسترداد v20 — All Gaps Closed (Parts I–IV)              ║')
    L(f'║  QARA 7-dim · Audit · GPU-ready · MUJAWWAD-correct         ║')
    L(f'║  المرجع: الشيخ ياسر الدوسري — 1425H  (KB v15)             ║')
    L(f'╚════════════════════════════════════════════════════════════╝')
    ...
    # ── PHASE 0: الوقاية — Stereo-to-Mono + DC Drift (MISS-01/02, Part I) ──
    # MUST run before analyze_input. Phase correlation guard before any sum.
    L('\nPhase 0 — الوقاية (Stereo-to-Mono + DC Drift Correction)')
    _chk('phase_0_wiqaya')

    # Pre-probe duration for الوقاية (we don't have state yet)
    _pre_probe = probe_file(input_path)
    _pre_total_s = float(_pre_probe.get('format', {}).get('duration', 0.0))
    _pre_state   = InputState()          # temporary state for Phase 0
    _pre_state.total_s = max(_pre_total_s, 1.0)

    # Phase 0.1: Phase-safe stereo-to-mono
    working_path = _stereo_to_mono_wiqaya(input_path, _pre_state)

    # Phase 0.2: DC offset drift correction (on the mono file)
    working_path = _correct_dc_drift(working_path, _pre_state)

    L(f'  الوقاية: method={_pre_state.wiqaya_method}  '
      f'r={_pre_state.wiqaya_correlation:.3f}  '
      f'delay={_pre_state.wiqaya_delay_ms:.2f}ms  '
      f'makeup={_pre_state.wiqaya_makeup_db:.1f}dB  '
      f'dc_corrected={_pre_state.dc_drift_corrected}')
    ...
    # analyze_input now receives the Phase-0-processed mono file
    state = analyze_input(working_path, ref)

    # Transfer Phase 0 state into main state
    state.wiqaya_applied     = _pre_state.wiqaya_applied
    state.wiqaya_method      = _pre_state.wiqaya_method
    state.wiqaya_correlation = _pre_state.wiqaya_correlation
    state.wiqaya_delay_ms    = _pre_state.wiqaya_delay_ms
    state.wiqaya_makeup_db   = _pre_state.wiqaya_makeup_db
    state.dc_drift_corrected = _pre_state.dc_drift_corrected
    state.dc_poly_degree     = _pre_state.dc_poly_degree
    state.dc_max_offset_db   = _pre_state.dc_max_offset_db
    state.is_mono = True   # working_path is guaranteed mono after Phase 0

    # KB-14-02: quran_prefix and nr_backend now passed as enhance() params (ARCH FIX)
    if quran_prefix:
        state.quran_prefix_set  = True
        state.quran_prefix_text = _build_whisper_prefix(quran_prefix)
        L(f'  [KB-14-02] Quran prefix set ({len(state.quran_prefix_text)} chars)')
    state.nr_backend      = nr_backend
    state._input_wav_path = working_path
    _warn_no_quran_prefix(state)

    if force_tier:
        L(f'  [force_tier] overriding {state.source_tier} → {force_tier}')
        state.source_tier = force_tier

    # Phase A.8: Opus/WhatsApp Detection (after tier, before logging)
    L('  [A.8] Opus/WhatsApp tier detection...')
    _chk('phase_A8_opus')
    _a8_clip = load_audio_fast(working_path, state.skip_s, min(state.dur_s, 30))
    if len(_a8_clip) > SR * 2:
        _detect_opus_whatsapp(_a8_clip, state)
        if state.opus_detected:
            # Re-compute achievable after tier override
            state.achievable_lufs, state.achievable_crest, state.achievable_lra = (
                _compute_achievable(state.source_tier, state.codec_cutoff,
                                    state.src_sr, state.clip_ratio,
                                    state.noise_type, state.smear_score))
    ...
    if state.opus_detected:
        L(f'  [A.8] Opus confirmed: ~{state.opus_bitrate_estimate}kbps  '
          f'SILK_repair_pending=True  '
          f'phoneme_unreliable_3to8k={state.phoneme_unreliable_3to8k}')

    # Phase A.9: Congregation Noise Taxonomy
    L('  [A.9] Congregation noise taxonomy...')
    _chk('phase_A9_noise_taxonomy')
    _a9_clip = load_audio_fast(working_path, state.skip_s, min(state.dur_s, 30))
    if len(_a9_clip) > SR * 2:
        _classify_congregation_noise(_a9_clip, state)
    del _a9_clip

    # Silence data for NR (loaded from working_path = mono Phase-0 output)
    clip = load_audio_fast(working_path, state.skip_s, state.dur_s)
    ...
    # NOTE: working_path is already set from Phase 0 above — do NOT reset.
    if state.clip_ratio > 0.0005 and state.source_tier in ('TIER_DAMAGED', 'TIER_DEGRADED'):
        ...
        _chk('phase_declip')
        _declip_wav = _declip_pass(working_path, state)
        if _declip_wav != working_path:
            working_path = _declip_wav
        if working_path != input_path:
            L(f'  [declip] OK — continuing with declipped audio')
            _log_stage_hash('declip', working_path)
            # KB-14-06: CQDV1 domain adaptation note (§73.3, v6)
            # Comprehensive Quranic Dataset v1: 114 suras × 35 reciters,
            # Hafs Asim riwaya, WAV 44.1kHz. Use for SFMSE/FRCRN Arabic
            # domain adaptation. Resample to 48kHz mono before use.
            # Download: search "CQDV1 Comprehensive Quranic Dataset"
            # in Bulletin of Computer Science Research (2025).
    else:
    ...
    # M-3 gate: noise_map from A.9 — only apply NR when M-3 murmur detected
    # If noise_map has NO M-3 entries, broadband NR has no valid target.
    # M-1 (Ameen) SKIP_NR gate — full timestamp gating in Part II Phase B.
    _has_m3_target = any(e['type'] == 'M-3_murmur' for e in state.noise_map)
    if state.noise_map and not _has_m3_target:
        L(f'  [A.9/NR gate] noise_map has no M-3 murmur — broadband NR has no valid target')
        L(f'  [A.9/NR gate] types present: '
          + ', '.join(set(e["type"] for e in state.noise_map)))
        # Reduce NR confidence to avoid attacking non-murmur noise with broadband NR
        if state.nr_confidence > 0.5:
            state.nr_confidence = min(state.nr_confidence, 0.35)
            L(f'  [A.9/NR gate] nr_confidence reduced to {state.nr_confidence:.2f} '
              f'(no M-3 murmur target, broadband NR inappropriate)')
    if state.ameen_detected:
        L(f'  [A.9/NR gate] M-1 Ameen detected — SKIP_NR gate active '
          f'(full timestamp gating implemented in Part II Phase B)')
    ...
    _log_stage_hash('nr_pass', nr_wav)
    # v20 V-2: Track DF3 applied, ornament-frame chunking for MUJAWWAD
    state.deepfilter3_applied = nr_report.get('deepfilter_applied',
                                               nr_report.get('applied', False))

    # ── v16 K-2: الاستمرار amplitude drift correction (Phase A.11) ────────────
    if state.amplitude_drift_detected and state.total_s >= 900.0:
        L('  [الاستمرار] applying amplitude drift correction...')
        _chk('phase_amplitude_drift')
        try:
            _drift_corrected = _correct_amplitude_drift(working_path, working_path, state)
            if _drift_corrected != working_path:
                working_path = _drift_corrected
                L('  [الاستمرار] ✓ amplitude drift corrected')
            else:
                L('  [الاستمرار] drift correction failed — continuing')
        except Exception as _adce:
            L(f'  [الاستمرار] error: {_adce}')
    elif state.total_s >= 900.0:
        L('  [الاستمرار] amplitude drift: not detected — skip')

    # ── v17 [P3-02] nara_wpe True WPE Dereverberation (Phase B.3) ─────────────
    L('\nPass 2A — WPE (nara_wpe True Dereverberation)')
    _chk('phase_B_wpe')
    try:
        _wpe_out = _nara_wpe_derev(nr_wav, state, ref, log_fn=L)
        if _wpe_out != nr_wav:
            nr_wav = _wpe_out
            _log_stage_hash('nara_wpe', nr_wav)
            L(f'  WPE ✓  RT60_reduction≈{state.wpe_rt60_reduction_s:.2f}s')
        else:
            flags = []
            if state.wpe_lra_guard_fired:    flags.append('LRA_guard')
            if state.wpe_madd_guard_fired:   flags.append('Madd_guard')
            if state.wpe_idgham_guard_fired: flags.append('Idgham_guard')
            L(f'  WPE bypass — {"/".join(flags) or "unavailable_or_skipped"}')
    except Exception as _wpe_e:
        L(f'  WPE error: {_wpe_e}')

    # ── PHASE B1.5: SILK Artifact Repair (A.8 follow-up, MISS-04) ────────────
    # Runs only when Opus detected at <32kbps. Applies formant bandwidth
    # widening to undo SILK LPC over-smoothing. After NR (which may have
    # partially helped) but before Tier 2 (which needs clean formants).
    if state.opus_detected and state.opus_bitrate_estimate < 32:
        L(f'\nPass 2A — SILK Artifact Repair (~{state.opus_bitrate_estimate}kbps)')
        _chk('phase_B1_5_silk')
        _silk_wav = _apply_silk_repair(nr_wav, state)
        if state.silk_repair_applied:
            nr_wav = _silk_wav
            _log_stage_hash('silk_repair', nr_wav)
    else:
        if state.opus_detected:
            L(f'  [B1.5] SILK repair bypass (bitrate={state.opus_bitrate_estimate}kbps ≥ 32)')
    ...
    # ── v16 K-5: الاستمرار spectral tilt drift correction (post-NR) ───────────
    if (state.total_s >= 900.0 and
            abs(state.spectral_tilt_drift_dB_per_min) >= 0.5):
        L(f'  [الاستمرار] spectral tilt correction '
          f'({state.spectral_tilt_drift_dB_per_min:.2f}dB/min) — post-NR...')
        _chk('phase_tilt_drift')
        try:
            _tilt_corr = _correct_spectral_tilt_drift(nr_wav, nr_wav, state)
            if _tilt_corr != nr_wav:
                nr_wav = _tilt_corr
                L('  [الاستمرار] ✓ spectral tilt corrected')
            else:
                L('  [الاستمرار] tilt correction failed — continuing')
        except Exception as _tce:
            L(f'  [الاستمرار] tilt error: {_tce}')

    # ── v20 V-5: Per-segment multilingual processing (replaces v16 K-6 binary gate) ─
    _is_arabic_dominant = (
        not state.has_multilingual_content or
        (sum((e - s) for (s, e, l, m) in state.language_segment_map
             if m == 'arabic') >=
         sum((e - s) for (s, e, l, m) in state.language_segment_map) * 0.6
         if state.language_segment_map else True)
    )
    if state.has_multilingual_content and state.language_segment_map:
        L(f'\n  [التعدد/v20] per-segment multilingual processing...')
        _chk('phase_multilingual_seg')
        def _arabic_seg_fn(cw, st, rf):
            return _run_deepfilter3_adaptive(cw, st) if (DEEPFILTER_OK or DEEPFILTER_CLI_OK) else cw
        _ml_out = _process_multilingual_segments(
            nr_wav, state, ref, _arabic_seg_fn, _non_arabic_segment_process)
        if _ml_out != nr_wav and os.path.exists(_ml_out):
            nr_wav = _ml_out
            L('  [التعدد/v20] ✓ per-segment processing complete')
    elif not _is_arabic_dominant:
        L('  [التعدد] WARNING: Non-Arabic dominant — Tajweed-specific phases gated')

    # ══════════════════════════════════════════════════════════════════════════
    # ── v17 PHASE B-GEN: الحكيم — DriftSE + SIPS Generative Corrector ────────
    L('\nPass 2D — الحكيم (Generative Corrector: DriftSE + SIPS)')
    _chk('phase_B_gen_hakim')
    if _HAKIM_OK:
        try:
            _hakim_in   = nr_wav
            _hakim_out, _hakim_res = apply_hakim(_hakim_in, state, ref, log_fn=L)
            state.hakim_status              = _hakim_res.status
            state.hakim_driftse_applied     = _hakim_res.driftse_applied
            state.hakim_driftse_accepted    = _hakim_res.driftse_accepted
            state.hakim_sips_applied        = _hakim_res.sips_applied
            state.hakim_sips_accepted       = _hakim_res.sips_accepted
            state.hakim_emphatic_delta      = _hakim_res.emphatic_delta_db
            state.hakim_madd_delta          = _hakim_res.madd_delta_frac
            state.hakim_style_gate          = _hakim_res.style_gate
            state.hakim_driftse_backend     = _hakim_res.driftse_backend
            state.hakim_dnsmos_ovrl_before  = _hakim_res.dnsmos_ovrl_before
            state.hakim_dnsmos_ovrl_after   = _hakim_res.dnsmos_ovrl_after
            if _hakim_res.status == 'OK' and _hakim_out != _hakim_in:
                nr_wav = _hakim_out
                _log_stage_hash('hakim_gen', nr_wav)
                L(f'  الحكيم ✓  B1={_hakim_res.driftse_backend}({_hakim_res.driftse_accepted})  '
                  f'B2=MetricGAN+({_hakim_res.sips_accepted})  '
                  f'emp_Δ={_hakim_res.emphatic_delta_db:.3f}dB  '
                  f'DNSMOS={_hakim_res.dnsmos_ovrl_before:.2f}→{_hakim_res.dnsmos_ovrl_after:.2f}')
            else:
                L(f'  الحكيم {_hakim_res.status} — {_hakim_res.reason}')
        except Exception as _hk_e:
            L(f'  الحكيم error: {_hk_e}')
    else:
        L('  الحكيم bypass — hakim_gen_v2.py not found')

    # ── v17 PHASE B-REF: المرآة — AnyEnhance Reference-Guided ────────────────
    L('\nPass 2E — المرآة (Reference-Guided: AnyEnhance)')
    _chk('phase_B_ref_miraat')
    if _MIRAAT_OK:
        try:
            _miraat_in   = nr_wav
            # v21: pass REF_FILES (raw paths) — v2 needs them for ECAPA + ref EQ
            _miraat_out, _miraat_res = apply_miraat(_miraat_in, state,
                                                     REF_FILES, log_fn=L)
            state.miraat_status       = _miraat_res.status
            state.miraat_applied      = (_miraat_res.status == 'OK')
            state.miraat_accepted     = (_miraat_res.status == 'OK')
            # v21: reference quality = ECAPA speaker cosine similarity
            state.miraat_ref_quality  = _miraat_res.speaker_sim_after
            state.miraat_gates_passed = sum([
                int(_miraat_res.gate_speaker_pass),
                int(_miraat_res.gate_emphatic_pass),
                int(_miraat_res.gate_madd_pass),
                int(_miraat_res.gate_lufs_pass),
            ])
            if _miraat_res.status == 'OK' and _miraat_out != _miraat_in:
                nr_wav = _miraat_out
                _log_stage_hash('miraat_ref', nr_wav)
                L(f'  المرآة ✓  speaker_sim={_miraat_res.speaker_sim_after:.3f}  '
                  f'ref_eq={_miraat_res.ref_eq_applied}  '
                  f'gates={state.miraat_gates_passed}/4')
            else:
                L(f'  المرآة {_miraat_res.status} — {_miraat_res.reason}')
        except Exception as _mr_e:
            L(f'  المرآة error: {_mr_e}')
    else:
        L('  المرآة bypass — miraat_ref_v2.py not found')

    # ── v17 PHASE B-TEXT: الإدراك — FlowSE Text-Conditioned ──────────────────
    L('\nPass 2F — الإدراك (Text-Conditioned: FlowSE)')
    _chk('phase_B_text_idrak')
    if _IDRAK_OK:
        try:
            _idrak_in   = nr_wav
            # v21: apply_idrak no longer takes ref — Whisper-conditioned, not ref-guided
            _idrak_out, _idrak_res = apply_idrak(_idrak_in, state, log_fn=L)
            state.idrak_status           = _idrak_res.status
            state.idrak_applied          = (_idrak_res.status == 'OK')
            state.idrak_accepted         = (_idrak_res.status == 'OK')
            # v21: text_conditioned = True when Whisper produced a transcript
            state.idrak_text_conditioned = bool(_idrak_res.transcript_arabic)
            state.idrak_fallback_path    = ''   # v21 has no fallback path concept
            if _idrak_res.status == 'OK' and _idrak_out != _idrak_in:
                nr_wav = _idrak_out
                _log_stage_hash('idrak_text', nr_wav)
                L(f'  الإدراك ✓  '
                  f'segs={_idrak_res.n_segments_enhanced}/{_idrak_res.n_low_conf_segments}  '
                  f'conf={_idrak_res.transcript_confidence:.3f}  '
                  f'lang={_idrak_res.whisper_language}'
                  f'({_idrak_res.whisper_language_prob:.0%})')
            else:
                L(f'  الإدراك {_idrak_res.status} — {_idrak_res.reason}')
        except Exception as _id_e:
            L(f'  الإدراك error: {_id_e}')
    else:
        L('  الإدراك bypass — idrak_text_v2.py not found')
    # ══════════════════════════════════════════════════════════════════════════
    ...
    # ── PHASE B6: الجلال — Voice Transcendence Engine ─────────────────────
    L('\nPass 2G — الجلال (Voice Transcendence: Shimmer · Transient · Formant · Sub · Width)')
    _chk('phase_B6_jalal')
    if state.source_tier not in ('TIER_CRITICAL',) and not _ihyaa_applied:
        try:
            _jalal_in = nr_wav
            # ── v16 K-4: MUJAWWAD style gates for Phase B6 الجلال ─────────────
            # J-1 shimmer: DISABLED — redundant with natural harmonic ornaments
            # J-4 widener: DISABLED — creates comb artifacts at vibrato rate
            _mujawwad_gate = (state.style_class == 'MUJAWWAD')
            _jalal_params_backup = {}
            if _mujawwad_gate:
                L('  [v16/K-4] MUJAWWAD: J-1 shimmer and J-4 widener gated OFF')
                import copy as _copy
                for _tk in _JALAL_TIER_PARAMS:
                    _jalal_params_backup[_tk] = _JALAL_TIER_PARAMS[_tk].copy()
                    _JALAL_TIER_PARAMS[_tk]['shimmer_blend'] = 0.0
                    _JALAL_TIER_PARAMS[_tk]['widener_mix']   = 0.0
            _jalal_out, _jalal_rep = apply_jalal(
                _jalal_in, state, ref, log_fn=L)
            if _mujawwad_gate:
                for _tk, _bk in _jalal_params_backup.items():
                    _JALAL_TIER_PARAMS[_tk] = _bk
    ...
    if state.source_tier not in ('TIER_CRITICAL',) and not _ihyaa_applied:
        try:
            ...
            # v20 V-3: MUJAWWAD نداء style gate — warn if N1/N3 ran on MUJAWWAD
            try:
                _nidaa_rep = _nidaa_apply_style_gate(state, _nidaa_rep)
                if _nidaa_rep.get('style_gate_warning'):
                    L(f'  [v20/V-3] نداء style gate fired — see apply_nidaa() patch')
            except Exception:
                pass
        except Exception as _ne:
    ...
    # ── v20 V-4: GPU generative path (GAP-URGENet, before Phase C EQ) ──────────
    L('\nPass 2H — GPU الاسترداد (GAP-URGENet Generative Recovery)')
    _chk('phase_B_gpu')
    if _gpu_path_available():
        _gpu_out, _gpu_rep = _run_gpu_enhance(nr_wav, state, ref)
        if _gpu_rep.get('status') == 'OK' and _gpu_out != nr_wav:
            nr_wav = _gpu_out
            _log_stage_hash('gpu_enhance', nr_wav)
            L(f'  GPU ✓  emphatic_Δ={_gpu_rep["emphatic_delta"]:+.3f}dB  '
              f'gates={_gpu_rep["gates_passed"]}')
        else:
            L(f'  GPU {_gpu_rep["status"]} — {_gpu_rep.get("reason","")}')
    else:
        L('  GPU path unavailable — see Section L-1 (GAP_URGENET_WEIGHTS not set)')
    ...
    # ── PHASE E1: الفضاء الصوتي — Room Presence (P3-03 pyroomacoustics / aecho) ──
    # P3-03: Physics-based Shoebox RIR (PRISTINE/CLEAN only).
    # Fallback: aecho chain for all other tiers (unchanged behaviour).
    if state.source_tier in _ROOM_TIER_PARAMS:
        L('  [الفضاء] applying room presence...')
        _room_applied = False

        # Try pyroomacoustics RIR first (PRISTINE/CLEAN)
        if state.source_tier in ('TIER_PRISTINE', 'TIER_CLEAN'):
            _pyroom_result = _apply_pyroomacoustics_sadaa(best_wav, state, ref, log_fn=L)
            if _pyroom_result != best_wav and os.path.exists(_pyroom_result):
                best_wav = _pyroom_result
                state.room_reverb_applied = True
                state.room_reverb_wet_db  = round(20 * math.log10(0.20 + 1e-9), 1)
                _room_applied = True
                L(f'  الفضاء ✓  pyroomacoustics RIR  RT60={state.sadaa_rir_rt60:.1f}s  '
                  f'mix=0.20 (−14dB)  tier={state.source_tier}')

        # aecho chain fallback (existing behaviour for all tiers or pyroom miss)
        if not _room_applied:
            ...
            if _room_ok and Path(_room_out).exists() and Path(_room_out).stat().st_size > 1000:
                best_wav = _room_out
                state.room_reverb_applied = True
                _wet_linear = 1.0 - _rp['out_gain']
                state.room_reverb_wet_db  = round(20 * math.log10(max(_wet_linear, 1e-6)), 1)
                L(f'  الفضاء ✓  aecho  tier={state.source_tier}  '
                  f'delays=[{_rp["delays"]}]ms  ~wet={state.room_reverb_wet_db:.1f}dB')
            else:
    ...
    lines = [
        f'isteidad-v17 | {state.source_tier} | {elapsed:.0f}s',
    ...
    # ── v20 V-6: QARA 7-Dimension Tajweed Quality Report ─────────────────────
    L('\nQARA — Tajweed Quality Assessment')
    _chk('phase_qara')
    _qara = QARAReport()
    try:
        _qara = _score_qara(output_path, state, ref)
        state.qara_report = _qara
        L(f'  QARA composite={_qara.composite_score:.1f}  grade={_qara.overall_grade}  '
          f'pass={_qara.composite_pass}  ({_qara.dimensions_passed}/7 dimensions)')
        L(f'  الماد={_qara.madd_score:.0f}  الغنة={_qara.ghunnah_score:.0f}  '
          f'القلقلة={_qara.qalqalah_score:.0f}  الإطباق={_qara.emphatic_score:.0f}  '
          f'الصفير={_qara.sibilant_score:.0f}  DNA={_qara.voice_dna_score:.0f}  '
          f'صحة={_qara.health_score:.0f}')
        if _qara.fail_reasons:
            L(f'  QARA below threshold: {_qara.fail_reasons}')
    except Exception as _qe:
        L(f'  QARA error: {_qe}')

    return {
        'engine_version':    'v20.0-الاسترداد-KB15-PartIV',
        ...
        # ── v15 Part I — الوقاية + الأذن الحساسة ─────────────────────────
        'wiqaya_applied':           state.wiqaya_applied,
        'wiqaya_method':            state.wiqaya_method,
        'wiqaya_correlation':       state.wiqaya_correlation,
        'wiqaya_delay_ms':          state.wiqaya_delay_ms,
        'wiqaya_makeup_db':         state.wiqaya_makeup_db,
        'dc_drift_corrected':       state.dc_drift_corrected,
        'dc_poly_degree':           state.dc_poly_degree,
        'dc_max_offset_db':         state.dc_max_offset_db,
        'opus_detected':            state.opus_detected,
        'opus_bitrate_estimate':    state.opus_bitrate_estimate,
        'codec_type':               state.codec_type,
        'silk_repair_applied':      state.silk_repair_applied,
        'phoneme_unreliable_3to8k': state.phoneme_unreliable_3to8k,
        'noise_map_count':          len(state.noise_map),
        'noise_map_types':          list(set(e['type'] for e in state.noise_map)),
        'ameen_detected':           state.ameen_detected,
        'pa_feedback_detected':     state.pa_feedback_detected,
        'congregation_murmur_ms':   state.congregation_murmur_ms,
        'noise_map_has_printthrough': state.noise_map_has_printthrough,
        # ── v16 Part II: Arabic DNA ──────────────────────────────────────────
        'riwaya_detected':          state.riwaya_detected,
        'riwaya_confidence':        round(state.riwaya_confidence, 3),
        'riwaya_uncertain':         state.riwaya_uncertain,
        'style_class':              state.style_class,
        'style_confidence':         round(state.style_confidence, 3),
        'style_deepfilter_atten':   state.style_deepfilter_atten_lim,
        'ornament_frames':          state.ornament_frames_detected,
        'amplitude_drift_detected': state.amplitude_drift_detected,
        'amplitude_drift_range_LU': round(state.amplitude_drift_range_LU, 2),
        'amplitude_drift_corrected':state.amplitude_drift_corrected,
        'tilt_drift_dB_per_min':    round(state.spectral_tilt_drift_dB_per_min, 3),
        'tilt_correction_applied':  state.tilt_correction_applied,
        'harakah_ms_calibrated':    round(state._harakah_ms_calibrated, 1),
        'madd_events_v2_count':     len(state.madd_events_v2),
        'madd_events_v2':           state.madd_events_v2,
        'has_multilingual_content': state.has_multilingual_content,
        # ── v17 Part III: Generative Intelligence ───────────────────────────
        'part3_generative': {
            'silero_vad_available':     state.silero_vad_available,
            'wpe_applied':              state.wpe_applied,
            'wpe_lra_guard_fired':      state.wpe_lra_guard_fired,
            'wpe_madd_guard_fired':     state.wpe_madd_guard_fired,
            'wpe_idgham_guard_fired':   state.wpe_idgham_guard_fired,
            'wpe_rt60_reduction_s':     round(state.wpe_rt60_reduction_s, 2),
            'hakim_status':             state.hakim_status,
            'hakim_driftse_backend':    state.hakim_driftse_backend,
            'hakim_driftse_accepted':   state.hakim_driftse_accepted,
            'hakim_sips_accepted':      state.hakim_sips_accepted,
            'hakim_emphatic_delta_db':  round(state.hakim_emphatic_delta, 3),
            'hakim_madd_delta_frac':    round(state.hakim_madd_delta, 3),
            'hakim_style_gate':         state.hakim_style_gate,
            'hakim_dnsmos_ovrl':        [round(state.hakim_dnsmos_ovrl_before, 2),
                                         round(state.hakim_dnsmos_ovrl_after,  2)],
            'miraat_status':            state.miraat_status,
            'miraat_accepted':          state.miraat_accepted,
            'miraat_speaker_sim':       round(state.miraat_ref_quality, 3),
            'miraat_gates_passed':      state.miraat_gates_passed,
            'idrak_status':             state.idrak_status,
            'idrak_accepted':           state.idrak_accepted,
            'idrak_text_conditioned':   state.idrak_text_conditioned,
            'idrak_fallback_path':      state.idrak_fallback_path,
            'sadaa_pyroomacoustics':    state.sadaa_pyroomacoustics,
            'sadaa_rir_rt60':           round(state.sadaa_rir_rt60, 1),
        },
        # ── v20 Part IV ──────────────────────────────────────────────────────
        'module_audit':   _build_module_audit(state),
        'qara':           _qara.to_dict() if hasattr(_qara, 'to_dict') else {},
        'arabic_wavlm':   getattr(state, '_arabic_wavlm_confirmed', False),
        'gpu_path_used':  getattr(state, 'gpu_enhance_applied', False),
        'summary':           summary,
    }
    ...
    if os.path.exists(_REF_CACHE):
        try:
            with open(_REF_CACHE) as f:
                d = json.load(f)
            if (d.get('cache_version') == 'v20.0'  # bumped — invalidates earlier caches
...
def main() -> int:
    ...
    p.add_argument('--quran-prefix', default='',
        metavar='TEXT',
        help='KB-14-02: Quran reference text as Whisper decoder prefix. '
             'Reduces emphatic consonant (emphatic sibilant) misclassification '
             'in noisy recordings. Supply the relevant ayah text (§73.4, v6).')
    p.add_argument('--nr-backend', default='deepfilter',
        choices=['deepfilter', 'frcrn', 'sfmse'],
        help='KB-14-04: NR backend for A/B testing (§68.5/69.5, v6). '
             '"deepfilter" = current afftdn/DeepFilterNet pipeline (default). '
             '"frcrn" = ClearerVoice-Studio FRCRN (pip install clearvoice-studio). '
             '"sfmse" = SFMSE one-step flow-matching (experimental).')
    ...
    try:
        ...
        r = enhance(args.input, args.output, args.iterations, args.target,
                   ...
                   quran_prefix=getattr(args, 'quran_prefix', ''),
                   nr_backend=getattr(args, 'nr_backend', 'deepfilter'))
