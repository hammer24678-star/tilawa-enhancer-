...
def enhance(input_path: str, output_path: str,
    ...
    # ── Force tier override ───────────────────────────────────────────────────
    if force_tier and force_tier in ('TIER_PRISTINE', 'TIER_PRISTINE_NOISY',
                                     'TIER_COMPRESSED', 'TIER_DAMAGED', 'TIER_CRITICAL'):
        L(f'  [FORCE_TIER] overriding auto-classified tier={tier} → {force_tier}')
        tier = force_tier
        state.source_tier = tier
        # Re-measure background noise if forcing noisy
        if force_tier == 'TIER_PRISTINE_NOISY' and samples is not None and NUMPY_OK:
            _bg_floor, _snr_proxy, _ = _measure_bg_noise_itiqan(samples, SR)
            state.noise_floor_db = _bg_floor
            state.snr_proxy_db   = _snr_proxy
            L(f'  [FORCE_TIER] noise_floor={_bg_floor:.1f}dBFS snr_proxy={_snr_proxy:.1f}dB')
    ...
    # ── Phase K: Harshness Softening (TIER_PRISTINE_NOISY only) ─────────────
    # Gate: tier is TIER_PRISTINE_NOISY AND VoiceDNA detected upper-mid harshness
    # (vocal_effort_ratio > 1.30). DF3 denoising can expose harsh upper-mids
    # that were previously masked by noise. This post-pass corrects that.
    # Softening chain (calibrated from listening tests):
    #   −3.5dB @ 2.5kHz  — cuts bite/edge in presence zone
    #   −4.5dB @ 3.15kHz — main harshness zone
    #   −4.0dB @ 5.0kHz  — sibilance brightness
    #   −5.0dB high shelf @ 7kHz — rolls off everything above aggressively
    #   +2.0dB @ 300Hz   — body warmth to fill the space
    #   +1.5dB @ 600Hz   — lower-mid warmth
    _chk('phase_K_harshness_softening')
    _k_ver    = getattr(state, 'vocal_effort_ratio', 0.0)
    _k_noisy  = state.source_tier == 'TIER_PRISTINE_NOISY'
    _k_harsh  = _k_ver > 1.30
    state.harshness_softened = False
    if _k_noisy and _k_harsh:
        L(f'  [K] TIER_PRISTINE_NOISY + vocal_effort_ratio={_k_ver:.3f} > 1.30 — applying softening')
        _k_tmp = _tmp_mp3('soften')
        _k_eq  = (
            'equalizer=f=2500:width_type=o:width=1.0:g=-3.5,'
            'equalizer=f=3150:width_type=o:width=1.0:g=-4.5,'
            'equalizer=f=5000:width_type=o:width=1.0:g=-4.0,'
            'highshelf=f=7000:width_type=s:width=0.7:g=-5.0,'
            'equalizer=f=300:width_type=o:width=1.0:g=2.0,'
            'equalizer=f=600:width_type=o:width=1.0:g=1.5'
        )
        _k_rc, _, _k_err = _run_ffmpeg([
            'ffmpeg', '-y', '-i', output_path,
            '-af', _k_eq,
            '-b:a', '320k', '-q:a', '0', _k_tmp
        ])
        if _k_rc == 0 and os.path.exists(_k_tmp):
            os.replace(_k_tmp, output_path)
            state.harshness_softened = True
            L('  [K] ✓ harshness softening applied')
        else:
            L(f'  [K] softening failed: {_k_err[:80]}')
            _cleanup(_k_tmp)
    elif _k_noisy and not _k_harsh:
        L(f'  [K] TIER_PRISTINE_NOISY but vocal_effort_ratio={_k_ver:.3f} ≤ 1.30 — no softening needed')
    else:
        L('  [K] not TIER_PRISTINE_NOISY — skipping')
    ...
    if getattr(state, 'harshness_softened', False):
        summary_lines.append(f'Phase K: harshness softened (VER={getattr(state, "vocal_effort_ratio", 0):.2f})')
    if encode_retries > 0:
    ...
    return {
        ...
        'itiqan_harshness_softened':  getattr(state, 'harshness_softened', False),
        'itiqan_vocal_effort_ratio':  getattr(state, 'vocal_effort_ratio', 0.0),
...
def main() -> int:
    ...
    p.add_argument('--force-tier',        default='', help='Override auto-classified source tier')
    args = p.parse_args()
    ...
    try:
        r = enhance(args.input, args.output, args.iterations, args.target, force_sadaa=args.force_sadaa, aggressive=args.aggressive, force_tier=args.force_tier)
        if 'error' in r:
            print(f'\n  ERROR: {r["error"]}')
            if r.get('routing'):
